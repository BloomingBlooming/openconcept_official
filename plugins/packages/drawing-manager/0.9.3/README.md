...
SQLiteの旧スキーマから移行するとき、同名の作業テーブル（接頭辞付きの `plugin_dwg_documents_schema_v5`）が既に存在する場合は、そのテーブルを削除せず移行を停止します。大文字・小文字だけが異なる名前も同様です。停止時は既存の図面・添付関連・作業テーブル内のデータを保持し、トランザクションと外部キー設定を元に戻します。衝突したテーブルは管理者がバックアップと内容を確認し、必要に応じて別名に保全してから移行を再実行してください。

プラグイン修正は下位互換を維持します。この修正ではPlugin API 1.0.0、既存のAPI・設定キー・ファイル形式・DBスキーマv7、権限・承認・通知・画面操作を維持しています。既存データの作り直しは不要です。SQLiteの通常移行で行う旧テーブルの置き換えと、MySQLの空の移行管理テーブルの片付けは従来の処理を維持します。検査ツールは引き続きこれらを削除SQL候補として表示します。

HTML描画の4箇所は文字列のエスケープと数値変換を確認し、悪意ある文字列を渡す回帰テストを追加しています。動的なHTML描画そのものは残るため、検査レポートの要確認項目は消えません。

Version 0.9.3 preserves the existing APIs, schema, settings, file formats and UI behavior. SQLite upgrades now stop on an existing migration work table instead of deleting it. Normal legacy upgrades remain supported. The remaining SQL and HTML inspection findings describe reviewed operations, not findings suppressed by an exception list.
