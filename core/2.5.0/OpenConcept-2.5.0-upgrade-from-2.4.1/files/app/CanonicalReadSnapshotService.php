...
/**
 * Canonical content export contract v1, deliberately distinct from DB transfer
...
final class CanonicalReadSnapshotService
{
    ...
    public function capabilities(array $actor, string $issuer): array
    {
        ...
        return [
            ...
            'providers' => $providerDescriptions, 'excluded' => $this->exclusions(self::CORE_TABLES, [], true),
            'original_file_version_scope' => 'current_files_table_only',
            'scopes' => ['all_core', 'selected_tables'], 'attachments_optional' => true, 'providers_require_explicit_selection' => true,
    ...
    public function begin(array $actor, string $issuer, array $options = []): array
    {
        ...
        $owners = $this->selection($options['providers'] ?? [], array_keys($this->providers));
    ...
    private function exclusions(array $tables, array $providers, bool $attachments): array
    {
        $categories = ['rag_snapshots_chunks_embeddings_indexes_sync', 'automatic_summaries_and_search_metadata',
            ...
            'notifications_audit_and_ai_conversation_history', 'generated_public_site_artifacts',
            'superseded_original_file_versions_and_recovery_blobs'];
        ...
        return [
            ...
            'core_records' => [['table' => 'extension_records', 'scope' => 'core:sources', 'collection' => 'file-versions',
