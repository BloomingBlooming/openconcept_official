...
final class PluginCatalog
{
    ...
    public function entries(): array
    {
        ...
        foreach ($catalog['plugins'] as $item) {
            ...
            if (!is_array($requires)
                || (isset($requires['plugin_api']) && (!is_string($requires['plugin_api']) || !PluginManager::isValidVersion($requires['plugin_api'])))
                || !is_array($requires['capabilities'] ?? []) || !array_is_list($requires['capabilities'] ?? [])) {
                throw new RuntimeException('Invalid catalog Plugin API requirements.');
            }
            foreach ($requires['capabilities'] ?? [] as $capability) {
                if (!is_string($capability) || $capability === '' || strlen($capability) > 120) {
                    throw new RuntimeException('Invalid catalog capability.');
                }
            }
    ...
    /** Reuse the complete package validation in an isolated, non-discoverable directory. */
    public function stageUpdate(string $pluginId, string $version, string $sha256, string $stagingRoot): array
    {
        $entries = $this->entries();
        $entry = null;
        foreach ($entries as $candidate) { if ($candidate['id'] === $pluginId) { $entry = $candidate; break; } }
        if ($entry === null || $entry['version'] !== $version || !hash_equals($entry['sha256'], $sha256)) {
            throw new RuntimeException('更新候補が変更されています。プラグイン一覧を再読み込みしてください。', 409);
        }
        $stagedCatalog = new self($this->catalogUrl, $stagingRoot, $this->fetcher);
        $stagedCatalog->entries = $entries;
        return $stagedCatalog->install($pluginId);
    }
