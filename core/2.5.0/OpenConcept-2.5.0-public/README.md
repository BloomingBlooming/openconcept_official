# OpenConcept 2.5.0 公開配布版

このフォルダーは、不特定多数への配布を目的としたOpenConceptの実行パッケージです。個別環境のホスト名、Database接続情報、APIキー、メールアカウント、SSH設定、利用者データ、ログ、添付ファイルは含まれていません。

V2.5.0では共通プラグインAPIを追加しました。PDF・Word・Excelを読み取る任意プラグイン、受信ボックスの汎用モーダル、管理者ログイン時のGitHub更新通知、将来のバックアッププラグイン向け正本読み取り専用APIを同梱しています。受理した抽出本文は通常のAI検索で利用できます。V2.4.1以前の修正と公式5言語の案内も含みます。

Version 2.5.0 adds the common Plugin API, optional PDF/Word/Excel reading for regular AI Search, reusable inbox dialogs, GitHub update notifications after administrator login, and a read-only canonical API for future backup plugins. Earlier fixes and all five interface languages are retained.

AI File Readerと自動読み取りは初期状態では無効です。「設定 > プラグイン」で有効にして、サイドバーからLLM接続を設定・確認してください。詳細は[変更点と利用方法](docs/release-notes-2.5.0.md)、開発用契約は[Plugin API](docs/plugin-api-reference.md)を参照してください。SQL/TXT等のバックアッププラグインと復元機能は今回の同梱対象外です。

AI File Reader and automatic reading are disabled by default. Enable the plugin, then configure and test its LLM connection from the sidebar. See the [release notes](docs/release-notes-2.5.0.md) and [Plugin API reference](docs/plugin-api-reference.md). SQL/text backup formatting plugins and restore functionality are not included.
