# Plugin API 1.0.0（OpenConcept V2.5.0）
...
`plugin.php` の戻り値のコールバックに `$context['plugin_api']`（PluginApiClient）を渡す。旧Hook・DB Adapter・RAG Provider・Translation Providerの契約は継続利用できる。実際の有効化には以下の本体側の対応表を満たす必要がある。

## 本体側の対応版と取り外し

`app/PluginCompatibility.php` が本体公開時点のID・最低対応版を保持する。V2.5.0は以下の版以上を許可する。更新版は必ず従来より大きい番号にし、プラグイン作者が下位互換性を保つか、本体を停止させず非対応機能を扱うことを契約とする。
...
プラグインのmanifestでこの表を上書きできない。表にないID・最低対応版未満は非対応とし、必要APIのmajor・最低版・能力も照合する。旧版のファイルや保存データを本体更新で削除しないが、最低対応版未満の導入済みプラグインは実効OFFになる。既知IDの新版を使うための本体側の版追加は不要。バージョンはSemVerの順序で比較し、ビルド情報は比較に含めず、同じ数値版では先行版を正式版より古く扱う。
...
既定の出力はCore正本（ページ、手動タグ、ACL、改訂、コメント、必要なユーザー属性、原本ファイル）。RAGのチャンク・Embedding・索引・再生成可能なスナップショットは対象外。providersに `extracted-content` を明示すると受理済み補助本文を追加し、manifestにもその区分を記録する。

SQL／JSONL／TXT／MDなどへの整形は将来のバックアッププラグインで実装する。TXT・MDだけではACL・参照関係・添付バイトを復元できないため、完全なコンテンツ移送では構造データと原本ファイルも保存する。

初期の原本バイト出力範囲は `original_file_version_scope=current_files_table_only`。差し替え前の復旧用ファイル履歴（`core:sources/file-versions`）と旧バイトは出力対象外としてmanifestに明記する。履歴のすべてを含む環境復旧には従来の環境全体バックアップを用いる。
