...
SQL);
        ...
        $affectedPublications = $meaningfulUpdate
            ...
            : [];
        $extensionEvents = [...$extensionPriorEvents, ...$pluginFileService->journalPageSources([$id, ...$inheritedPageIds], ['operation' => 'save', 'change_id' => bin2hex(random_bytes(16)), 'actor_user_id' => (int) $user['id']], $accessChanged)];
        $pdo->commit();
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
