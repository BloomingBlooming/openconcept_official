...
final class CustomRagHttpConnector implements RagRetrieverInterface
{
    ...
    public function __construct(array $configuration, ?SafeOutboundHttpClient $http = null)
    {
        ...
        if (strlen($apiKey) > 4096 || preg_match('/[\r\n\x00]/', $apiKey) === 1) {
            throw new InvalidArgumentException('Custom RAG API key is invalid.');
        }
        if ($secretEnv !== '' && preg_match('/^[A-Z][A-Z0-9_]{1,127}$/D', $secretEnv) !== 1) {
    ...
    private function jsonRequest(
        ...
        if ($authMode !== 'none') {
            $secret = array_key_exists('api_key', $this->configuration)
                ...
                : (string) getenv((string) $this->configuration['secret_env']);
            if ($secret === '') {
                throw new RuntimeException('Custom RAG API key is unavailable.');
            }
