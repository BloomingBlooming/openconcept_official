# OpenConcept 2.6.0 公開配布版

このフォルダーは、不特定多数への配布を目的としたOpenConceptの実行パッケージです。個別環境のホスト名、Database接続情報、APIキー、メールアカウント、SSH設定、利用者データ、ログ、添付ファイルは含まれていません。

V2.6.0では、会話・ページ旧版・旧添付を保持して参照する「記録と履歴」を追加しました。訂正とAI変更案の判断結果は元の記録を残して追記し、根拠不足時の一般知識への自動切替を停止します。共通JSON形式の会話取込、正本と添付の出力、隔離SQLiteへの復元・照合を提供します。

Version 2.6.0 adds records and history, immutable corrections and action results, common-format conversation imports, retained attachment originals, and portable knowledge export with verified isolated SQLite restoration. Unsupported answers no longer automatically fall back to general knowledge. Core schema generation 6 is unchanged.

詳しくは [変更点と利用方法](docs/release-notes-2.6.0.md) と [Plugin API](docs/plugin-api-reference.md) を参照してください。正本アーカイブは資格情報や環境設定を含まないため、環境全体バックアップも継続します。

See the [release notes](docs/release-notes-2.6.0.md) and [Plugin API reference](docs/plugin-api-reference.md). Knowledge archives omit credentials and environment settings; continue complete environment backups as well.
