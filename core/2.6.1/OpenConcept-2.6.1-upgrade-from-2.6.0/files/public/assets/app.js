(() => {
    ...
    const appVersion = window.OPENCONCEPT_BOOT?.app_version || '2.6.1';
    ...
    async function api(action, options = {}) {
        ...
        if (typeof FormData !== 'undefined' && options.body instanceof FormData) {
            ...
            if (state.user && state.wafCompatibilityEnabled && config.method.toUpperCase() === 'POST'
                && !['login', 'logout', 'initialize', 'waf-compatibility-settings'].includes(action)) {
    ...
    async function showSettings(tab = 'general', updateHash = true) {
        ...
        if (state.capabilities.can_manage_system_settings) {
            await Promise.all([loadPlugins(false), loadAiProviderSettings(false), loadTranslationProviderSettings(false), loadWafCompatibilitySettings()]);
