# OpenConcept 2.6.1 公開配布版

アプリのダークモード、カバー13種類（なしを含む）、アイコン96種類、受信トレイ下の三点アイコンによるAI検索・記録と履歴の開閉を含みます。フロートNavi 1.2.2は個別プラグインとして導入・更新できます。

Includes app-wide dark mode, 13 cover choices including No cover, 96 page icons, and the three-dot toggle below Inbox for AI search and Records and history. Float Navi 1.2.2 is installed or updated as a separate plugin.
...
詳しくは [変更点と利用方法](docs/release-notes-2.6.1.md)、[Plugin API](docs/plugin-api-reference.md)、[プラグイン開発](docs/plugin-development.md) を参照してください。V2.6.0の記録と履歴・正本出力を継続利用できます。正本アーカイブは資格情報や環境設定を含まないため、環境全体バックアップも継続します。

See the [release notes](docs/release-notes-2.6.1.md) and [Plugin API reference](docs/plugin-api-reference.md). Records, history, and portable knowledge archives from v2.6.0 remain available. Knowledge archives omit credentials and environment settings; continue complete environment backups as well.
