...
final class BgeM3EmbeddingProvider implements EmbeddingProviderInterface
{
    ...
    public function __construct(array $configuration = [], ?SafeOutboundHttpClient $http = null)
    {
        ...
        if ($configuration['api_key_env'] !== ''
            && preg_match('/^[A-Z][A-Z0-9_]{1,127}$/D', $configuration['api_key_env']) !== 1) {
    ...
    public function embed(array $texts): array
    {
        ...
        if (($this->configuration['use_shared_api_key'] ?? false) === true && $secret === '') {
            throw new EmbeddingServiceException('embedding_credentials_missing', 'The shared AI API key is not configured.');
        }
        if ($secret !== '') {
