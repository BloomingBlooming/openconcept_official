...
$pdo = $database->pdo();
$aiProviderSettings = new AiProviderSettings(dirname(__DIR__) . '/storage');
...
$pluginManager = new PluginManager(dirname(__DIR__) . '/plugins');
...
function notificationsForUser(PDO $pdo, array $user, int $limit = 50): array
{
    ...
    // Fetch a wider window before applying page ACLs so hidden notifications do
    // not crowd visible ones out of the requested result set.
    $scanLimit = min(300, $limit * 3);
    $statement = $pdo->prepare(<<<SQL
SELECT n.*
FROM notifications n
LEFT JOIN pages p ON p.id = n.page_id
WHERE n.user_id = ? AND (n.page_id IS NULL OR p.archived_at IS NULL)
...
SQL);
    ...
    foreach ($statement->fetchAll() as $notification) {
        $pageId = $notification['page_id'] === null ? null : (int) $notification['page_id'];
        if ($pageId !== null && !canViewPage($pdo, $user, ['id' => $pageId])) {
            continue;
        }
...
{
    $statement = $pageId === null
        ? $pdo->prepare('SELECT id FROM notifications WHERE user_id = ? AND type = ? AND page_id IS NULL AND is_read = 0 ORDER BY created_at DESC, id DESC LIMIT 1')
        : $pdo->prepare('SELECT id FROM notifications WHERE user_id = ? AND type = ? AND page_id = ? AND is_read = 0 ORDER BY created_at DESC, id DESC LIMIT 1');
