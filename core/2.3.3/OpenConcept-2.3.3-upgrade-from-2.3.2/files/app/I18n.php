...
final class I18n
{
    ...
    /** Resolve an anonymous UI locale without overriding an explicit saved choice. */
    public function selectGuestLocale(?string $remembered, string $acceptLanguage = ''): string
    {
        $available = array_column($this->availableLocales(), 'code');
        if (in_array($remembered, $available, true)) {
            return $remembered;
        }

        $preferences = [];
        foreach (array_slice(explode(',', substr($acceptLanguage, 0, 8192)), 0, 100) as $position => $entry) {
            if (preg_match('/^\s*([a-z]{1,8}(?:-[a-z0-9]{1,8})*|\*)\s*(?:;\s*q\s*=\s*(0(?:\.\d{0,3})?|1(?:\.0{0,3})?))?\s*$/iD', $entry, $matches) !== 1) {
                continue;
            }
            $preferences[] = ['range' => strtolower($matches[1]), 'quality' => isset($matches[2]) ? (float) $matches[2] : 1.0, 'position' => $position];
        }
        usort($preferences, static fn (array $a, array $b): int => ($b['quality'] <=> $a['quality']) ?: ($a['position'] <=> $b['position']));
        $fallback = $this->selectLocale(null);
        foreach ($preferences as $preference) {
            ...
            do {
                $candidates = array_values(array_filter($available, static fn (string $locale): bool => $range === '*'
                    || strtolower($locale) === $range || str_starts_with(strtolower($locale), $range . '-')));
                // Prefer an exact pack, then English when a broad range permits it.
                usort($candidates, static fn (string $a, string $b): int =>
                    ((strtolower($b) === $range ? 2 : 0) + ($b === $fallback ? 1 : 0))
                    <=> ((strtolower($a) === $range ? 2 : 0) + ($a === $fallback ? 1 : 0)));
                foreach ($candidates as $candidate) {
                    $excluded = false;
                    foreach ($preferences as $other) {
                        if ($other['quality'] === 0.0 && strlen($other['range']) >= strlen($range)
                            && ($other['range'] === '*' || strtolower($candidate) === $other['range']
                                || str_starts_with(strtolower($candidate), $other['range'] . '-'))) {
                            $excluded = true;
                            break;
                        }
                    }
                ...
                // Accept base language preferences and regional variants, e.g. en-GB -> en-US.
                $separator = strrpos($range, '-');
                $range = $separator === false ? '' : substr($range, 0, $separator);
            } while ($range !== '');
        }
        return $fallback;
    }
