(() => {
    ...
    async function request(action, options = {}) {
        ...
        if (!response.ok || data.error || data.ok === false) {
            ...
            const errorMessage = code === 'pgvector_missing'
                ? t('ui.pgvectorRequired', {}, 'The canonical database is PostgreSQL, but pgvector is not installed. Standard RAG cannot be saved until the vector extension is available.')
                : code === 'postgresql_required'
                ? t('ui.postgresqlRequired', {}, 'OpenConcept Standard RAG requires PostgreSQL to be the canonical database. Prepare and connect PostgreSQL before selecting Standard RAG.')
    ...
    function generationRows(status) {
        ...
        if (!generations.length) return `<p>${esc(t('ui.noGenerations', {}, 'No index generations yet.'))}</p>`;
    ...
    function configurationDiagnostics(body) {
        ...
        if (body.retrieval_mode === 'standard') {
            if (!standardReady) {
                ...
                issues.push(configurationIssue(deploymentCode, 'error', 'retrieval', 'database',
                    pgvectorMissing
                        ...
                        : t('ui.issueDatabase', {}, 'Standard RAG cannot run on the current database'),
                    pgvectorMissing
                        ...
                        : t('ui.issueDatabaseHelp', {}, 'Open the database settings and make PostgreSQL with pgvector the canonical database, or select Custom RAG.')));
            }
            ...
            if (!validSecretEnvironment(body.standard.api_key_env)) {
            ...
            if (body.custom.auth_mode !== 'none' && !validSecretEnvironment(body.custom.secret_env)) {
            ...
            issues.push(configurationIssue('rag_disabled', 'warning', 'retrieval', 'retrieval',
                t('ui.issueRagDisabled', {}, 'RAG retrieval is disabled'),
                t('ui.issueRagDisabledHelp', {}, 'AI Search will not use the RAG evidence pipeline. Select Standard RAG or Custom RAG to enable it.')));
        }

        if (body.answer.mode === 'shared') {
            ...
            if (shared.authMode !== 'none' && !shared.secretAvailable) {
                issues.push(configurationIssue('shared_secret_missing', 'error', 'answer', 'shared-ai',
        ...
        } else if (body.answer.mode === 'openai_compatible') {
            issues.push(...endpointConfigurationIssues('answer', 'answer', body.answer.base_url, body.answer.model_id,
                body.answer.allow_private_network, body.answer.allow_http));
            const answerEnvironment = String(body.answer.api_key_env || '').trim();
            const answerSecretAvailable = payload?.answer_secret?.available === true;
            if (answerEnvironment !== '' && !validSecretEnvironment(answerEnvironment)) {
                issues.push(configurationIssue('answer_secret_reference_invalid', 'error', 'answer', 'answer',
                    t('ui.issueAuthentication', {}, 'AI authentication is incomplete'),
                    t('ui.issueDirectAuthenticationHelp', {}, 'Enter an API key or a valid environment-variable name that contains it.')));
            } else if (body.answer.auth_mode !== 'none' && answerEnvironment === '' && !String(body.answer.api_key || '').trim() && !answerSecretAvailable) {
                issues.push(configurationIssue('answer_secret_missing', 'error', 'answer', 'answer',
                    t('ui.issueAuthentication', {}, 'AI authentication is incomplete'),
            ...
            issues.push(configurationIssue('legacy_answer_provider', 'error', 'answer', 'answer',
        ...
        if (runtimeIssue) issues.unshift(runtimeIssue);
        return issues;
    }
    ...
    function configurationFlow(body, issues) {
        ...
        const retrievalValue = body.retrieval_mode === 'standard'
            ? t('ui.standard', {}, 'OpenConcept Standard RAG')
        ...
        const retrievalDetail = body.retrieval_mode === 'standard'
            ...
            : t('ui.ragBypassHelp', {}, 'The request bypasses the RAG evidence pipeline.');
        const retrievalProcess = body.retrieval_mode === 'standard'
            ...
            : t('ui.flowRetrievalDisabledHelp', {}, 'RAG is bypassed and the saved common AI Search route is used. The RAG answer setting below is not used on this route.');
        const retrievalImpact = body.retrieval_mode === 'custom'
            ...
            : t('ui.impactRagDisabled', {}, 'Saving switches to common AI Search without building an index.');
        ...
        const configuredAnswerValue = body.answer.mode === 'shared'
            ...
            ? t('ui.separateAi', {}, 'Separate OpenAI-compatible API')
        ...
        const retrievalState = flowState(issues, 'retrieval', body.retrieval_mode === 'unconfigured' ? 'warning' : 'ready');
        const answerState = flowState(issues, 'answer', body.answer.mode === 'none' ? 'warning' : 'ready');
        ...
        const routeBlocked = retrievalBlocked || (ragEnabled && answerBlocked);
        const answerValue = ragEnabled
            ? configuredAnswerValue
            : t('ui.flowAnswerFallbackValue', {}, 'Common AI connection (RAG answer setting is not used)');
        ...
        const answerProcess = !ragEnabled
            ? t('ui.flowAnswerFallbackHelp', {}, 'The saved common AI Search route generates the answer instead of the RAG answer setting.')
            : retrievalBlocked
        ...
        const answerImpact = !ragEnabled
            ? t('ui.flowAnswerFallbackImpact', {}, 'Changing the RAG answer setting does not affect user answers until RAG is enabled.')
        ...
        const runtimeRoute = runtimeRagActive
            ...
            : t('ui.effectiveFallbackHelp', {}, 'No active RAG index is serving questions. AI Search continues through the saved common AI connection.');
        return `<section class="rag-flow" aria-label="${esc(t('ui.flowTitle', {}, 'Question-to-answer configuration flow'))}">
            <header><div><strong>${esc(t('ui.flowTitle', {}, 'Question-to-answer configuration flow'))}</strong><span>${esc(t('ui.flowHelp', {}, 'This chart previews the route selected in the form. Unsaved changes do not affect AI Search until they are saved. Retrieval changes take effect after the new index generation is activated.'))}</span></div><em class="${isDraft ? 'draft' : 'saved'}">${esc(isDraft ? t('ui.flowDraft', {}, 'Editing (unsaved)') : t('ui.flowSaved', {}, 'Saved route'))}</em></header>
            ...
            <div class="rag-flow-track">
                ...
                <span class="rag-flow-arrow" aria-hidden="true">&rarr;</span>
                ${flowStage({ stage: 'evidence', title: t('ui.flowEvidence', {}, '3. Verify evidence'), value: ragEnabled ? t('ui.flowEvidenceValue', {}, 'ACL and revision checks') : t('ui.disabled', {}, 'Disabled'), process: ragEnabled ? t('ui.flowEvidenceHelp', {}, 'OpenConcept rechecks current permissions, document revisions, and source identity for every candidate, removing inaccessible or stale evidence.') : t('ui.flowEvidenceSkippedHelp', {}, 'RAG-specific evidence verification is skipped. The permission-aware search in the common AI route is used instead.'), impact: ragEnabled ? t('ui.impactFixed', {}, 'Always enabled in RAG; no setting') : t('ui.impactRagDisabled', {}, 'Saving switches to common AI Search without building an index.'), destination: ragEnabled ? t('ui.flowEvidenceDestination', {}, 'Only verified evidence proceeds to answer generation and later appears under Sources.') : t('ui.flowEvidenceSkippedDestination', {}, 'This step has no direct screen and is skipped when RAG is disabled.'), state: retrievalBlocked || !ragEnabled ? 'blocked' : 'ready' })}
                <span class="rag-flow-arrow" aria-hidden="true">&rarr;</span>
                ${flowStage({ stage: 'answer', title: t('ui.flowAnswer', {}, '4. Generate answer'), value: answerValue, detail: answerDetail, process: answerProcess, impact: answerImpact, destination: answerDestination, state: retrievalBlocked || !ragEnabled ? 'blocked' : answerState, target: !ragEnabled || body.answer.mode === 'shared' ? 'shared-ai' : 'answer' })}
                <span class="rag-flow-arrow" aria-hidden="true">&rarr;</span>
    ...
    function currentConfiguration(settings, deploymentRequirement) {
        ...
        const answerName = answer.mode === 'none'
            ...
            : usesShared
            ? t('ui.sharedAi', {}, 'Use common AI connection')
        ...
        const databaseStatus = deploymentRequirement.ready === true
            ? t('ui.standardAvailable', {}, 'Standard RAG is available')
            : deploymentRequirement.code === 'pgvector_missing'
            ? t('ui.standardPgvectorMissing', {}, 'Standard RAG unavailable: pgvector is not installed')
            : t('ui.standardUnavailable', {}, 'Standard RAG unavailable on the current database');
        ...
        const routeName = ragActive
            ...
            : t('ui.effectiveFallback', {}, 'Common AI route (RAG fallback)');
    ...
    function render() {
        ...
        const answerSecretAvailable = answerSecret.available === true;
        const embeddingSecret = payload.standard_embedding_secret || {};
        const embeddingSecretAvailable = embeddingSecret.available === true;
        const answerMode = payload.configuration_saved !== true && !answer.mode
            ? 'shared'
            : answer.mode === 'provider' && answer.provider_id === 'openai-compatible' && answerConnection.use_shared_ai_provider === true
            ? 'shared'
        ...
        const deploymentNotice = !standardReady
            ? `<p class="notice error" data-standard-requirement="${esc(deploymentCode)}">${esc(deploymentCode === 'pgvector_missing'
                ? t('ui.pgvectorRequired', {}, 'The canonical database is PostgreSQL, but pgvector is not installed. Standard RAG cannot be saved until the vector extension is available.')
                : t('ui.postgresqlRequired', {}, 'OpenConcept Standard RAG requires PostgreSQL to be the canonical database. Prepare and connect PostgreSQL before selecting Standard RAG.'))} <button class="btn secondary compact" type="button" data-focus-setting="database">${esc(t('ui.openDatabaseSettings', {}, 'Open database settings'))}</button></p>`
            : '';
        ...
        overlay.innerHTML = `<div class="rag-core-workspace" aria-label="${esc(t('core.name', {}, 'RAG Core'))}">
                ...
                <form class="rag-core-grid" data-settings-form>
                    <fieldset class="rag-core-section" id="ragCoreRetrievalSection">
                        <legend><span>1</span><div><strong>${esc(t('ui.retrievalEngine', {}, 'RAG retrieval engine'))}</strong><small>${esc(t('ui.retrievalEngineHelp', {}, 'Finds evidence in your documents. Standard and Custom connection values are retained separately; changing the selection creates a new index generation.'))}</small></div></legend>
                        <div class="rag-core-availability ${retrievalMode === 'unconfigured' ? 'pending' : ''}"><strong>${esc(t('ui.coreAlwaysAvailable', {}, 'RAG Core is always available'))}</strong><span>${esc(retrievalMode === 'unconfigured'
                            ? t('ui.providerUnconfigured', {}, 'RAG retrieval is disabled. AI Search continues through the common AI route without RAG evidence.')
                            : t('ui.providerConfigured', { provider: retrievalMode === 'standard' ? t('ui.standard', {}, 'OpenConcept Standard RAG') : t('ui.custom', {}, 'Custom RAG') }, '{provider} is configured as the retrieval provider.'))}</span></div>
                        ${choice('retrieval_mode', 'standard', retrievalMode === 'standard', 'ui.standard', 'OpenConcept Standard RAG', 'ui.standardChoiceHelp', 'OpenConcept manages chunking, embeddings, and search. PostgreSQL with pgvector is required.', true)}
                        ...
                        ${choice('retrieval_mode', 'unconfigured', retrievalMode === 'unconfigured', 'ui.disableRetrieval', 'Do not use RAG retrieval', 'ui.disableRetrievalHelp', 'Continue using the common AI search route without the RAG evidence pipeline.')}
                        ${deploymentNotice}
                        <div data-mode-panel="standard" class="rag-core-advanced">
                            <details${retrievalMode === 'standard' ? ' open' : ''}><summary>${esc(t('ui.standardDetails', {}, 'Standard RAG connection and advanced settings'))}</summary>
                                <div class="rag-core-fields">
                                    ...
                                    <label>${esc(t('ui.apiKeyEnv', {}, 'API key environment variable'))}<input class="input" name="standard_api_key_env" value="${esc(standard.api_key_env || 'OPENCONCEPT_BGE_M3_API_KEY')}"><small>${esc(t('ui.secretEnvHelp', {}, 'Enter the environment-variable name, never the secret value.'))}</small></label>
                                    <label>${esc(t('ui.embeddingApiKey', {}, 'Embedding API key'))}<input class="input" type="password" name="standard_api_key" value="" autocomplete="new-password" maxlength="4096" placeholder="${embeddingSecretAvailable ? esc(embeddingSecret.hint || '••••') : ''}"><small>${esc(embeddingSecretAvailable
                                        ? t('ui.embeddingApiKeyConfigured', { source: embeddingSecret.source || 'stored' }, 'Configured ({source}); enter a value only to replace it.')
                                        : t('ui.embeddingApiKeyOptional', {}, 'Optional for unauthenticated local services. The environment variable above remains the fallback.'))}</small></label>
                                    <label class="rag-core-inline-check"><input type="checkbox" name="standard_clear_api_key"> <span>${esc(t('ui.clearStoredEmbeddingKey', {}, 'Remove the API key stored in OpenConcept'))}</span></label>
                        ...
                        <div data-mode-panel="custom" class="rag-core-advanced">
                            <div class="rag-core-fields">
                                ...
                                <label>${esc(t('ui.secretEnv', {}, 'Secret environment variable'))}<input class="input" name="custom_secret_env" value="${esc(custom.secret_env || 'OPENCONCEPT_CUSTOM_RAG_API_KEY')}"><small>${esc(t('ui.secretEnvHelp', {}, 'Enter the environment-variable name, never the secret value.'))}</small></label>
                    ...
                    <fieldset class="rag-core-section" id="ragCoreAnswerSection">
                        <legend><span>2</span><div><strong>${esc(t('ui.answerGeneration', {}, 'Answer generation'))}</strong><small>${esc(t('ui.answerGenerationHelp', {}, 'Turns verified evidence into a readable answer. Changes in this section do not rebuild the index.'))}</small></div></legend>
                        ${choice('answer_mode', 'shared', answerMode === 'shared', 'ui.sharedAi', 'Use common AI connection', 'ui.sharedAiHelp', 'Use the saved AI provider shown above. Changes there also apply to RAG answer generation.', true)}
                        ${choice('answer_mode', 'openai_compatible', answerMode === 'openai_compatible', 'ui.separateAi', 'Use a separate OpenAI-compatible API', 'ui.separateAiHelp', 'Use this only when RAG answers must use a different endpoint or model from the common AI connection.')}
                        ...
                        <div data-answer-mode-panel="openai_compatible" class="rag-core-advanced"><div class="rag-core-fields">
                            ...
                            <label>${esc(t('ui.apiKeyEnv', {}, 'API key environment variable'))}<input class="input" name="llm_api_key_env" value="${esc(answerConnection.api_key_env || 'OPENAI_API_KEY')}"></label>
                            <label>${esc(t('ui.apiKey', {}, 'API key'))}<input class="input" type="password" name="llm_api_key" value="${answerSecretAvailable ? '＊＊＊＊' : ''}" data-secret-masked="${answerSecretAvailable ? 'true' : 'false'}" autocomplete="new-password" maxlength="4096"><small>${esc(answerSecretAvailable ? t('ui.apiKeyConfigured', {}, '＊＊＊＊ (configured; enter a value only to replace it)') : t('ui.apiKeyMissing', {}, 'Not configured'))}</small></label>
    ...
    function formValues() {
        ...
        const apiKeyInput = form.querySelector('[name="llm_api_key"]');
        const apiKey = apiKeyInput?.disabled || apiKeyInput?.dataset.secretMasked === 'true' ? '' : String(apiKeyInput?.value || '');
        const embeddingApiKey = String(form.querySelector('[name="standard_api_key"]')?.value || '');
        return {
            form,
            data,
            custom: {
                name: data.get('custom_name'), base_url: data.get('custom_base_url'), auth_mode: data.get('custom_auth_mode'),
                secret_env: data.get('custom_secret_env'), index_id: data.get('custom_index_id'), connect_timeout: Number(data.get('custom_connect_timeout') || 10),
            ...
            llm: {
                base_url: data.get('llm_base_url'), api_key_env: data.get('llm_api_key_env'), model_id: data.get('llm_model_id'),
                api_key: apiKey,
    ...
    function settingsBody(requireRetrieval = true) {
        ...
        return {
            ...
            standard: {
                base_url: data.get('standard_base_url'), api_key_env: data.get('standard_api_key_env'), model_id: data.get('standard_model_id'),
                api_key: embeddingApiKey, clear_api_key: data.has('standard_clear_api_key'),
    ...
    function updatePanels() {
        ...
        const directNoAuth = overlay.querySelector('[name="llm_auth_mode"]')?.value === 'none';
        overlay.querySelectorAll('[name="llm_api_key"], [name="llm_api_key_env"]').forEach(input => { input.disabled = directNoAuth; });
        const customNoAuth = overlay.querySelector('[name="custom_auth_mode"]')?.value === 'none';
        overlay.querySelectorAll('[name="custom_secret_env"]').forEach(input => { input.disabled = customNoAuth; });
    }
    ...
    function bind() {
        ...
        const apiKeyInput = overlay.querySelector('[name="llm_api_key"]');
        if (apiKeyInput) {
            apiKeyInput.onfocus = () => {
                if (apiKeyInput.dataset.secretMasked === 'true') {
                    apiKeyInput.value = '';
                    apiKeyInput.dataset.secretMasked = 'false';
                }
            };
            apiKeyInput.oninput = () => { apiKeyInput.dataset.secretMasked = 'false'; };
        }
        overlay.querySelector('[data-settings-form]').onsubmit = async event => {
            ...
            try {
                ...
                connectionChecks.retrieval = null;
                connectionChecks.answer = null;
                notice = result.waiting_for_embedding === true
