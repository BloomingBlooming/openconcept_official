...
最新版 / Latest: [本体ZIP / Application ZIP](OpenConcept-2.3.2-public.zip) · [SHA-256](OpenConcept-2.3.2-public.zip.sha256) · [展開済み / Unpacked](OpenConcept-2.3.2-public/)

図面管理 / Drawing Manager: [個別パッケージ / Standalone package](../../plugins/packages/drawing-manager/0.9.1/drawing-manager-0.9.1.oc-plugin.json) · [公式導入説明 / Official installation guide](https://github.com/BloomingBlooming/openconcept_official/tree/main/plugins/packages/drawing-manager/0.9.1)
...
```text
core/2.3.2/
├── README.md                           本書 / This guide, displayed on GitHub
...
同梱の[`docs/nginx.conf`](OpenConcept-2.3.2-public/docs/nginx.conf)は`http` contextから読み込むサイト別`server`設定例です。global `nginx.conf`の置換用ではありません。アプリの絶対パス、`server_name`、`listen`とTLS、`fastcgi_pass`を変更し、アップロード上限とタイムアウトを調整してください。Nginxは`.htaccess`を解釈しません。

The bundled [`docs/nginx.conf`](OpenConcept-2.3.2-public/docs/nginx.conf) is a virtual-host `server` example included from the `http` context, not a replacement for the global configuration. Adjust the application path, `server_name`, `listen` and TLS, `fastcgi_pass`, upload limits, and timeouts. Nginx does not interpret `.htaccess`.
...
操作方法 / User operations: [日本語操作説明書 / Japanese operation manual](OpenConcept-2.3.2-public/docs/openconcept-operation-manual.ja.md).
