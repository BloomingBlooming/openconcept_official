...
SQL);
    ...
    ): array
    {
        return $this->consistentRead(function () use ($conversationId, $userId, $expectedPasswordFingerprint): array {
            ...
            $user = $this->authorizationUser($userId, $expectedPasswordFingerprint);
            if ($user === null) {
                throw new InvalidArgumentException('指定されたAIチャットを開けません。');
            }
    ...
    ): array
    {
        ...
        try {
            ...
            if ($turnLock->fetchColumn() === false) {
                throw new InvalidArgumentException('更新するAIチャットの応答が見つかりません。');
            }
...
SQL);
        ...
        if ($pageIds === []) {
            ...
            return empty($row['sufficient']) || (string) ($row['mode'] ?? '') === 'general-knowledge';
        }
