...
final class AiPageActionService
{
    ...
    public function handle(
        ...
        if (in_array($operation, ['rename_page', 'create_page', 'update_focused_block', 'insert_after_focus'], true)) {
            ...
            if ($operation === 'create_page' && !canCreatePage($user)) {
                throw new InvalidArgumentException('新しいページを作成する権限がありません。');
            }
            ...
            $blockId = (string) ($editorContext['block_id'] ?? '');
            if ($blockId === '') {
                throw new InvalidArgumentException('戻り先の本文ブロックが記録されていません。対象ブロックへカーソルを置いてからAIを開いてください。');
            }
            $actions[] = [
                'type' => 'focus',
                'page_id' => (int) $page['id'],
    ...
    public function executePending(array $pending, array $user, string $expectedPasswordFingerprint): array
    {
        if ((int) ($pending['user_id'] ?? 0) !== (int) ($user['id'] ?? 0)) {
            throw new InvalidArgumentException('このAI変更案を実行できません。');
        }
        ...
        if ($page === null) {
            throw new InvalidArgumentException('変更対象のページを確認できません。');
        }
        ...
        if (preg_match('/^[a-f0-9]{64}$/D', $expectedFingerprint) !== 1
            ...
            throw new InvalidArgumentException('ページが別の操作で更新されました。最新の内容を開き直してください。');
        }
        $this->assertEditable($page, $user);
        ...
        if ($operation === 'rename_page') {
            if ($plan['title'] === '') {
                throw new InvalidArgumentException('変更後のページタイトルを確認できませんでした。');
            }
            ...
            $blockId = (string) ($editorContext['block_id'] ?? '');
            if ($blockId === '') {
                throw new InvalidArgumentException('編集する本文ブロックを確認できません。');
            }
            $blocks = json_decode((string) ($page['blocks_json'] ?? '[]'), true);
            $blocks = is_array($blocks) ? $blocks : [];
            $index = $this->blockIndex($blocks, $blockId);
            if ($index < 0) {
                throw new InvalidArgumentException('編集対象のブロックが更新されています。もう一度対象へカーソルを置いてください。');
            }
            if ($operation === 'update_focused_block') {
                $type = (string) ($blocks[$index]['type'] ?? 'paragraph');
                if (!in_array($type, ['paragraph', 'lead', 'heading1', 'heading2', 'heading3', 'bullet', 'number', 'todo', 'quote', 'code', 'callout'], true)) {
                    throw new InvalidArgumentException('この種類のブロックはAIから直接書き換えられません。');
                }
                ...
                if ($inserted === []) {
                    throw new InvalidArgumentException('挿入する内容を確認できませんでした。');
                }
            ...
            throw new InvalidArgumentException('このAI変更案は実行できません。');
        }
...
SQL);
    ...
    ): array
    {
        $liveUser = preg_match('/^[a-f0-9]{64}$/D', (string) $expectedPasswordFingerprint) === 1
            ...
            : null;
        if ($liveUser === null) {
            throw new InvalidArgumentException('ページを編集する権限がありません。');
        }
        ...
        if ($lockedHierarchy['reason'] !== null || !is_array($livePage)) {
            throw new InvalidArgumentException('ページの階層が更新されました。最新の内容を開き直してください。');
        }
    ...
    private function assertEditable(array $page, array $user): void
    {
        if (!canEditPage($this->pdo, $user, $page)) {
            throw new InvalidArgumentException('このページをAIから編集する権限がありません。');
        }
    }
    ...
    private function assertCurrentVersion(array $page, string $expectedUpdatedAt): void
    {
        ...
        if ($expectedUpdatedAt !== '' && !hash_equals((string) ($page['updated_at'] ?? ''), $expectedUpdatedAt)) {
            throw new InvalidArgumentException('ページが別の操作で更新されました。最新の内容を開き直してから、もう一度依頼してください。');
        }
    }
    ...
    ): array
    {
        ...
        if ($title === '') {
            throw new InvalidArgumentException('新しいページのタイトルを生成できませんでした。');
        }
        $blocks = $this->plannedBlocks((array) ($plan['blocks'] ?? []), (string) ($source['plain_text'] ?? ''));
        if ($blocks === []) {
            throw new InvalidArgumentException('新しいページの本文を生成できませんでした。');
        }
