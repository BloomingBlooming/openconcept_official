# OpenConcept 2.4.1 HTTPサーバー配置・必要環境 / HTTP Server Layout and Requirements
...
新規設置には、上記のv2.4.1公開パッケージを使用してください。ZIP、SHA-256チェックサム、展開済みファイル一式を掲載しています。

Use the v2.4.1 public package above for a new installation. The ZIP, SHA-256 checksum, and unpacked application files are available through the links above.

v2.4.1の公開配布物は、v2.4.0までの修正に加え、音声アシストのプラグインON/OFF連動と、ページツリーの現在ページのハイライト・祖先の自動展開・表示位置へのスクロール、「すべて開く」「すべて閉じる」を含みます。v2.4.0までの共通APIキーの明示選択、個別キーの保持・再利用・明示削除、未保存キー入力の保持、RAGとベクトル検索の説明修正、横並び設定タブも含みます。

The v2.4.1 public distribution includes all fixes through v2.4.0, plus voice-assist synchronization with the plugin's enabled state and a page tree that highlights the current page, expands its ancestors, and scrolls it into view, with Expand all and Collapse all controls. Earlier fixes include explicit common API-key selection, retained individual keys and explicit deletion, preservation of unsaved key input, corrected RAG/vector-search guidance, and horizontal settings tabs.
...
```text
dist/
├── OpenConcept-2.4.1-public.zip          配布ZIP / Distribution archive
├── OpenConcept-2.4.1-public.zip.sha256   ZIP検証値 / Archive checksum
└── OpenConcept-2.4.1-public/             展開済み配布物 / Unpacked distribution
...
`OpenConcept-2.4.1-public/`の**内容全体**を、設置先の専用アプリルートへ転送します。`dist/`全体やZIPをDocument Rootにする必要はありません。`public/`だけを単独でコピーすると、親フォルダーのPHPコードや保存領域を参照できなくなります。隠しファイルも保持してください。

Transfer the **entire contents** of `OpenConcept-2.4.1-public/` into a dedicated application root. Neither `dist/` nor the ZIP is the document root. Copying only `public/` breaks access to PHP code and storage in its parent directory. Preserve hidden files as well.
...
同梱の[`docs/nginx.conf`](OpenConcept-2.4.1-public/docs/nginx.conf)は`http` contextから読み込むサイト別`server`設定例です。global `nginx.conf`の置換用ではありません。アプリの絶対パス、`server_name`、`listen`とTLS、`fastcgi_pass`を変更し、アップロード上限とタイムアウトを調整してください。Nginxは`.htaccess`を解釈しません。

The bundled [`docs/nginx.conf`](OpenConcept-2.4.1-public/docs/nginx.conf) is a virtual-host `server` example included from the `http` context, not a replacement for the global configuration. Adjust the application path, `server_name`, `listen` and TLS, `fastcgi_pass`, upload limits, and timeouts. Nginx does not interpret `.htaccess`.
...
操作方法 / User operations: [日本語操作説明書 / Japanese operation manual](OpenConcept-2.4.1-public/docs/openconcept-operation-manual.ja.md).
