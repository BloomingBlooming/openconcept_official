# OpenConcept 2.4.1 公開配布版

このフォルダーは、不特定多数への配布を目的としたOpenConceptの実行パッケージです。個別環境のホスト名、Database接続情報、APIキー、メールアカウント、SSH設定、利用者データ、ログ、添付ファイルは含まれていません。

V2.4.1では、AI検索の音声アシスト表示がプラグインのON/OFFに連動するよう修正しました。別タブにも設定変更を反映し、切り替え時の入力内容を保持します。ページを開くと左ツリーの祖先を展開して現在ページをハイライトし、見える位置までスクロールします。検索欄の下に「すべて開く」「すべて閉じる」も追加しました。公式5言語の案内を同梱しています。

Version 2.4.1 fixes AI Search voice assist visibility when its plugin is enabled or disabled. Plugin changes also reach other tabs while preserving pending input. Opening a page expands its ancestors in the sidebar, highlights the current page, and scrolls it into view. Expand all and Collapse all controls are available below the sidebar search field. Guidance is included in all five supported interface languages.
