...
final class AiProviderSettings
{
    ...
    public const RAG_CUSTOM_PROFILE = 'rag-custom-provider';
    ...
    public function __construct(string $storageRoot, string $profile = self::DEFAULT_PROFILE)
    {
        ...
        if (!in_array(
            $profile,
            [self::DEFAULT_PROFILE, 'translation-provider', self::RAG_ANSWER_PROFILE, self::RAG_EMBEDDING_PROFILE, self::RAG_CUSTOM_PROFILE],
            true
        )) {
        ...
        $this->aad = match ($profile) {
            ...
            self::RAG_CUSTOM_PROFILE => 'OpenConcept RAG custom provider settings v1',
        };
    }
    ...
    public function validate(array $input): void
    {
        ...
        $this->validateApiKey((bool) ($input['clear_api_key'] ?? false) ? '' : (string) ($input['api_key'] ?? ''));
    }

    public function storedApiKeyFor(string $baseUrl, string $authMode): string
    {
        $record = $this->readRecord();
        if (!is_array($record['secret'] ?? null)
            || !$this->bindingMatches($this->secretBinding($record), $baseUrl, $authMode)) {
            return '';
        }
        return trim($this->unseal($record['secret']));
    }

    /** Resolve only the selected credential source; callers own service-specific environment fallbacks. */
    public function apiKeyFor(string $baseUrl, string $authMode, bool $useSharedApiKey): string
    {
        if (strtolower(trim($authMode)) === 'none') {
            return '';
        }
        if ($useSharedApiKey && $this->profile !== self::DEFAULT_PROFILE) {
            return trim((string) ((new self($this->storageRoot))->resolved()['api_key'] ?? ''));
        }
        return $this->storedApiKeyFor($baseUrl, $authMode);
    }
    ...
    public function resolved(): array
    {
        ...
        $record = $this->readRecord();
        if ($record === null) {
            if ($this->profile !== self::DEFAULT_PROFILE) {
                $shared = (new self($this->storageRoot))->resolved();
        ...
        } elseif ($settings['use_shared_api_key']) {
            $settings['api_key'] = (string) ((new self($this->storageRoot))->resolved()['api_key'] ?? '');
            $settings['api_key_source'] = 'shared';
        } elseif ($storedSecret !== '' && $this->bindingMatches($this->secretBinding($record), $settings['base_url'], $settings['auth_mode'])) {
    ...
    public function publicState(): array
    {
        ...
        $individualKey = is_array($record['secret'] ?? null) ? trim($this->unseal($record['secret'])) : '';
        $individualAvailable = $individualKey !== ''
            && $this->bindingMatches($this->secretBinding($record), (string) $resolved['base_url'], (string) $resolved['auth_mode']);
        return [
            'settings' => $resolved,
            'secret' => [
            ...
            'individual_secret' => [
                'configured' => $individualKey !== '',
                'available' => $individualAvailable,
                'source' => $individualKey !== '' ? 'stored' : 'none',
                'hint' => $individualKey === '' ? '' : ('••••' . substr($individualKey, -4)),
            ],
    ...
    public function save(array $input): array
    {
        ...
        $newSecret = (bool) ($input['clear_api_key'] ?? false) ? '' : trim((string) ($input['api_key'] ?? ''));
        ...
        try {
            ...
            $binding = $secret !== null ? $this->secretBinding($current) : null;
            if ((bool) ($input['clear_api_key'] ?? false)) {
                $secret = null;
                $binding = null;
            } elseif ($newSecret !== '') {
                $secret = $this->seal($newSecret);
                $binding = ['base_url' => $settings['base_url'], 'auth_mode' => $settings['auth_mode']];
            }
            $record = [
    ...
    private function normalizeSettings(array $input): array
    {
        ...
        if ($this->profile !== self::DEFAULT_PROFILE && array_key_exists('use_shared_api_key', $input)
            && !is_bool($input['use_shared_api_key'])) {
            throw new InvalidArgumentException('AI provider shared API key selection must be a boolean.');
        }
        ...
        return [
            ...
            'auth_mode' => $authMode,
            'use_shared_api_key' => $this->profile !== self::DEFAULT_PROFILE && ($input['use_shared_api_key'] ?? false) === true,
    ...
    /** @param array<string, mixed>|null $record @return array{base_url: string, auth_mode: string} */
    private function secretBinding(?array $record): array
    {
        // Older encrypted records were bound implicitly to their saved
        // connection. Preserve that original destination on the next save.
        $binding = is_array($record['secret_binding'] ?? null)
            ? $record['secret_binding']
            : (is_array($record['settings'] ?? null) ? $record['settings'] : []);
        return [
            'base_url' => rtrim(trim((string) ($binding['base_url'] ?? '')), '/'),
            'auth_mode' => strtolower(trim((string) ($binding['auth_mode'] ?? 'bearer'))),
        ];
    }

    /** @param array{base_url: string, auth_mode: string} $binding */
    private function bindingMatches(array $binding, string $baseUrl, string $authMode): bool
    {
        return $binding['base_url'] === rtrim(trim($baseUrl), '/')
            && $binding['auth_mode'] === strtolower(trim($authMode))
            && strtolower(trim($authMode)) !== 'none';
    }
