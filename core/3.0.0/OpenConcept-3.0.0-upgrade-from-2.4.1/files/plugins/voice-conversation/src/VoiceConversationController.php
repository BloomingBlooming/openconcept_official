...
final class VoiceConversationController
{
    ...
    ): void
    {
        ...
        $this->uiLocale = function_exists('uiLocale') ? uiLocale($user) : (string) ($user['ui_locale'] ?? 'en-US');
        try {
    ...
    {
        // Streaming errors bypass Core's JSON response boundary. Never translate deltas or conversation content.
        if ($event === 'error' && class_exists('SystemMessageLocalizer')) {
            $data = SystemMessageLocalizer::response($data, $this->uiLocale);
        }
        echo 'event: ' . $event . "\n";
