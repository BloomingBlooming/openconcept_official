...
class SimpleXLS
{
    // OpenConcept hardening: bound malformed compound-file chains and BIFF loops.
    private $openConceptSteps = 0;
    private $openConceptDeadline = null;
    protected function openConceptGuard()
    {
        if ($this->openConceptDeadline === null) {
            $this->openConceptDeadline = microtime(true) + 5;
        }
        if (++$this->openConceptSteps > 500000 || microtime(true) > $this->openConceptDeadline) {
            throw new \RuntimeException('workbook_parse_limit');
        }
    }
    const BIFF8 = 0x600;
    const BIFF7 = 0x500;
...
*/
        ...
        $openConceptSeen = array();
        while ($sbdBlock !== - 2) {
            $this->openConceptGuard();
            if (isset($openConceptSeen[$sbdBlock])) { throw new \RuntimeException('workbook_chain_cycle'); }
            $openConceptSeen[$sbdBlock] = true;
...
//      return ($value > 0x7FFFFFFF) ? $value - 0x100000000 : $value;
    ...
    protected function readData($bl)
    {
        ...
        while ($block !== - 2) {
            $this->openConceptGuard();
            if (isset($openConceptSeen[$block])) { throw new \RuntimeException('workbook_chain_cycle'); }
            $openConceptSeen[$block] = true;
            $pos  = ( $block + 1 ) * self::BIG_BLOCK_SIZE;
