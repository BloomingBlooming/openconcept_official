...
SQL);
        foreach ($lockedRows as $pageId => $page) {
            ...
            if ((int) $page['author_id'] !== (int) $actor['id']) {
                $locale = notificationRecipientLocale($pdo, (int) $page['author_id']);
                $actorName = cleanText((string) $actor['name'], 120) ?: SystemMessageLocalizer::message('notification.member', [], $locale);
                createLocalizedNotification($pdo, (int) $page['author_id'], 'update',
                    'notification.update', ['name' => $actorName, 'title' => (string) $page['title']], $pageId, 'system-messages', true);
            }
            $changedIds[] = $pageId;
        }
