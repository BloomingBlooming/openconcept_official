...
final class PluginApiRegistry
{
    ...
    /** Add structured localization only to a notification owned by this plugin and recipient. */
    public function localizeNotification(string $scope, int $id, int $userId, string $message, string $key, array $parameters): void
    {
        $this->mutate($scope, function () use ($scope, $id, $userId, $message, $key, $parameters): void {
            $metadata = $this->notificationMetadata($id, ['id' => $userId]);
            if (($metadata['scope'] ?? null) !== $scope) { return; }
            if ($this->repository->get('core:notifications', 'localization', (string) $id) !== null) { return; }
            $storedMessage = function_exists('cleanText') ? cleanText($message, 500) : $message;
            $this->repository->put('core:notifications', 'localization', (string) $id, [
                'domain' => $scope, 'key' => $key, 'parameters' => $parameters,
                'source_sha256' => hash('sha256', $storedMessage),
            ], 0);
        });
    }
