...
SQL);
    ...
    /**
     * Append an action decision/result while retaining the referenced original question and proposal.
     *
     * @return array{conversation: array<string, mixed>, turn: array<string, mixed>}
     */
    ...
    ): array
    {
        ...
        try {
            ...
            $conversationSql = 'SELECT id, turn_count FROM ai_conversations WHERE id = ? AND user_id = ? AND archived_at IS NULL';
            $turnSql = 'SELECT * FROM ai_chat_turns WHERE id = ? AND conversation_id = ?';
            if ((string) $this->pdo->getAttribute(PDO::ATTR_DRIVER_NAME) !== 'sqlite') {
            ...
            $conversationRow = $conversationLock->fetch(PDO::FETCH_ASSOC);
            if (!is_array($conversationRow)) {
                throw new AiValidationException('ai.openChatError');
            }
            $turnLock = $this->pdo->prepare($turnSql);
            $turnLock->execute([$turnId, $conversationId]);
            $original = $turnLock->fetch(PDO::FETCH_ASSOC);
            if (!is_array($original)) {
                throw new AiValidationException('ai.chatResponseMissing');
            }

            // A decision is a new event. The proposal text and its original
            // evidence remain immutable, including when the decision is refusal.
            $previousTurnId = $turnId;
            $nextIndex = (int) $conversationRow['turn_count'] + 1;
            if ($sourceResult === null) {
                $pageId = (int) ($original['page_id'] ?? 0);
                $pageTitle = (string) $original['page_title'];
                $sources = json_decode($original['sources_json'], true, 64, JSON_THROW_ON_ERROR);
            }
            $insert = $this->pdo->prepare(<<<'SQL'
INSERT INTO ai_chat_turns
    (conversation_id, turn_index, mode, question, answer, page_id, page_title, sufficient,
     sources_json, candidate_count, model, reasoning_effort, response_id, latency_ms, degraded)
VALUES (?, ?, 'page-action', ?, ?, ?, ?, ?, ?, ?, ?, ?, '', 0, 0)
SQL);
            $insert->execute([
                $conversationId, $nextIndex, '変更案に対する判断・実行結果', $answer,
                $pageId > 0 ? $pageId : null, $pageTitle,
                $sourceResult === null ? (int) $original['sufficient'] : (!empty($sourceResult['sufficient']) ? 1 : 0),
                json_encode($sources, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR),
                $sourceResult === null ? (int) $original['candidate_count'] : max(0, (int) ($sourceResult['candidate_count'] ?? count($sources))),
                (string) $original['model'], (string) $original['reasoning_effort'],
            ]);
            $turnId = (int) $this->pdo->lastInsertId();
            (new KnowledgeRecords($this->pdo))->turnRelation($turnId, $previousTurnId, $userId,
                $sourceResult === null ? 'decision' : 'execution_result');
            $this->pdo->prepare(<<<'SQL'
UPDATE ai_conversations
SET turn_count = ?, last_page_id = ?, last_page_title = ?, updated_at = CURRENT_TIMESTAMP
WHERE id = ? AND user_id = ?
SQL)->execute([$nextIndex, $pageId > 0 ? $pageId : null, $pageTitle, $conversationId, $userId]);
            $this->pdo->commit();
        } catch (Throwable $exception) {
            if ($this->pdo->inTransaction()) {
                $this->pdo->rollBack();
...
SQL);
    ...
    private function turnIsCurrentlyAccessible(array $row, array $user): bool
    {
        ...
        foreach ($sources as $source) {
            ...
            if ($sourcePageId > 0 && ($source['source_scope'] ?? '') !== 'knowledge_history') {
                $pageIds[$sourcePageId] = $sourcePageId;
            }

            $scope = (string) ($source['source_scope'] ?? '');
            if (in_array($scope, ['extension_content', 'knowledge_history'], true)) {
