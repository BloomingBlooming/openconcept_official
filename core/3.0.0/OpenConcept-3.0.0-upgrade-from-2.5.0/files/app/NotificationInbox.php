...
final class NotificationInbox
{
    ...
    {
        $storedRows = array_column($rows, null, 'id');
        $runtime = $GLOBALS['pluginApiRuntime'] ?? null;
        $rows = $runtime instanceof PluginApiRuntime ? $runtime->decorateNotifications($rows, $user) : $rows;
        if (class_exists(SystemMessageLocalizer::class, false)) {
            $locale = isset($user['ui_locale']) ? (string) $user['ui_locale'] : notificationRecipientLocale($this->pdo, (int) $user['id']);
            foreach ($rows as &$row) {
                $stored = (string) ($storedRows[$row['id']]['message'] ?? $row['message']);
                $metadata = $this->records->get('core:notifications', 'localization', (string) $row['id']);
                $translated = $metadata === null ? null : SystemMessageLocalizer::storedNotification($stored, $metadata['value'], $locale);
                if ($translated !== null) { $row['message'] = $translated; continue; }
                // Extension messages are owned by their plugin, not inferred from a core type.
                if (empty($row['extension_action'])) {
                    $row['message'] = SystemMessageLocalizer::notification((string) $row['message'], (string) $row['type'], $locale);
                }
            }
            unset($row);
        }
