...
require_once dirname(__DIR__) . '/app/SystemMessageLocalizer.php';
...
if (in_array($requestedActionBeforeBootstrap, $databaseMigrationControlActions, true)) {
    ...
    if ($controlUserId < 1
        ...
        echo json_encode(['error' => SystemMessageLocalizer::guestError('Authentication is required.')], JSON_UNESCAPED_SLASHES);
        exit;
    }
    $maintenanceMode = new DatabaseMaintenanceMode(Environment::storagePath(dirname(__DIR__)));
    try {
        ...
        echo json_encode([
            'error' => SystemMessageLocalizer::guestError($exception->getMessage()),
...
if (!in_array($requestedActionBeforeBootstrap, $databaseMigrationSetupActions, true)) {
    ...
    try {
        ...
        echo json_encode([
            'error' => SystemMessageLocalizer::guestError('Database migration maintenance is active. Please retry shortly.'),
            'maintenance' => true,
...
if ($action === 'session') {
    ...
    if (($user !== null || requestedUiLocale() !== null) && ($_COOKIE['openconcept_ui_locale'] ?? null) !== uiLocale($user)) {
...
if ($action === 'guest-ui-locale' && $method === 'POST') {
    requireCsrf();
    $body = bodyJson();
    $requestedLocale = $body['locale'] ?? null;
    if (!is_string($requestedLocale) || !I18n::isValidLocale($requestedLocale)
        || $i18n->selectLocale($requestedLocale) !== $requestedLocale) {
        jsonResponse(['error' => localizedAuthError('利用可能な画面言語を選択してください。')], 422);
    }
...
if ($action === 'initialize' && $method === 'POST') {
    ...
    if (isInitialized($pdo)) {
        jsonResponse(['error' => localizedAuthError('初期セットアップは完了しています。')], 409);
    }

    $activeUsers = (int) $pdo->query("SELECT COUNT(*) FROM users WHERE active = 1 AND role NOT IN ('system', 'suspended')")->fetchColumn();
    if ($activeUsers > 0) {
        jsonResponse(['error' => localizedAuthError('有効なユーザーが存在するため初期化できません。')], 409);
    }
    ...
    if ($name === '') {
        jsonResponse(['error' => localizedAuthError('管理者名を入力してください。')], 422);
    }
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        jsonResponse(['error' => localizedAuthError('正しいメールアドレスを入力してください。')], 422);
    }
    if ($password !== $confirmation) {
        jsonResponse(['error' => localizedAuthError('確認用パスワードが一致しません。')], 422);
    }
    if ($error = passwordError($password)) {
        jsonResponse(['error' => localizedAuthError($error)], 422);
    }
    ...
    try {
        ...
        $pdo->prepare('UPDATE users SET ui_locale = ? WHERE id = ?')->execute([uiLocale(), $adminId]);
        ...
        if ($exception instanceof PDOException && (str_contains($exception->getMessage(), 'UNIQUE') || str_contains($exception->getMessage(), 'Duplicate'))) {
            ...
            jsonResponse(['error' => localizedAuthError('このメールアドレスは既に使用されています。')], 422);
        }
        throw $exception;
    }
...
if ($action === 'login' && $method === 'POST') {
    ...
    // An explicit login-screen choice follows the member through their first
    // password change. Automatic browser detection never overwrites saved UI settings.
    $loginLocale = requestedUiLocale();
    if ($loginLocale !== null && $loginLocale !== ($user['ui_locale'] ?? null)) {
        $pdo->prepare('UPDATE users SET ui_locale = ? WHERE id = ?')->execute([$loginLocale, (int) $user['id']]);
    ...
    $_SESSION['extension_login_ticket'] = ['user_id' => (int) $user['id'], 'expires' => time() + 600];
    ...
    jsonResponseWithDeferredWork(
        ...
        static function () use ($pdo, $database, $deploymentWriteGatePath): void {
            (new RagLoginWorker(
                $pdo,
                new OpenAIClient(),
...
if ($action === 'change-password' && $method === 'POST') {
    ...
    $confirmation = (string) ($body['password_confirmation'] ?? '');
    if ($password !== $confirmation) {
        jsonResponse(['error' => localizedAuthError('確認用パスワードが一致しません。', $user)], 422);
    }
    if ($error = passwordError($password)) {
        jsonResponse(['error' => localizedAuthError($error, $user)], 422);
    }
    ...
    try {
        ...
        $status = in_array((int) $exception->getCode(), [401, 422], true)
            ...
            : 500;
        jsonResponse(['error' => $status === 500 ? $i18n->translate('common.operationFailed', [], uiLocale($user)) : localizedAuthError($exception->getMessage(), $user)], $status);
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
        }
...
if (!empty($user['must_change_password']) && $action !== 'update-ui-locale') {
    jsonResponse(['error' => localizedAuthError('続行するにはパスワードを変更してください。', $user), 'code' => 'password_change_required'], 403);
}

$pluginApiController = new PluginApiController($pluginApiRuntime, $pdo, $i18n);
require_once dirname(__DIR__) . '/app/KnowledgeHistoryController.php';
$knowledgeController = new KnowledgeHistoryController($database, dirname(__DIR__));
if ($knowledgeController->handles($action)) {
    $knowledgeController->dispatch($action, $method, $user, $expectedPasswordFingerprint);
}
if ($pluginApiController->handles($action)) {
    $pluginApiController->dispatch($action, $method, $user);
}
...
if ($action === 'mail-settings' && $method === 'GET') {
    requireRole($user, ['admin']);
    try {
        jsonResponse((new MailSettings($database->storagePath()))->publicState());
    } catch (Throwable) {
        jsonResponse(['error' => $i18n->translate('mail.error.settingsUnavailable', [], uiLocale($user))], 500);
    }
}

if ($action === 'mail-settings' && $method === 'POST') {
    requireCsrf();
    requireRole($user, ['admin']);
    try {
        beginPageWriteTransaction($pdo);
        $liveAdmin = requireLockedApplicationAdminForWrite($pdo, $user, $expectedPasswordFingerprint);
        $state = (new MailSettings($database->storagePath()))->save(bodyJson());
        audit($pdo, (int) $liveAdmin['id'], 'mail_settings_updated', 'system', 1, [
        ...
        $key = str_starts_with($exception->getMessage(), 'mail.error.') ? $exception->getMessage() : 'mail.error.invalidSettings';
        jsonResponse(['error' => $i18n->translate($key, [], uiLocale($user))], 422);
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) { $pdo->rollBack(); }
        $status = in_array((int) $exception->getCode(), [401, 403], true) ? (int) $exception->getCode() : 500;
        jsonResponse(['error' => $status === 500
            ? $i18n->translate('mail.error.settingsUnavailable', [], uiLocale($user))
            : localizedMemberError($exception->getMessage(), $user)], $status);
    }
}

if ($action === 'mail-test' && $method === 'POST') {
    requireCsrf();
    requireRole($user, ['admin']);
    try {
        beginPageWriteTransaction($pdo);
        $liveAdmin = requireLockedApplicationAdminForWrite($pdo, $user, $expectedPasswordFingerprint);
        // Persist the throttle under the administrator write lock, across browser sessions.
        $throttleKey = 'mail_test_last_' . (int) $liveAdmin['id'];
        if ((int) setting($pdo, $throttleKey, '0') > time() - 60) {
            throw new RuntimeException('mail.error.rateLimited', 429);
        }
        setSetting($pdo, $throttleKey, (string) time());
        $pdo->commit();
        $mail = Mailer::sendTest((string) $liveAdmin['email'], baseUrl(), $database->storagePath(), uiLocale($liveAdmin));
        audit($pdo, (int) $liveAdmin['id'], 'mail_test_sent', 'system', 1, [
            'transport' => $mail['transport'], 'sent' => $mail['sent'],
        ]);
        jsonResponse(['ok' => $mail['sent'], 'mail' => $mail]);
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) { $pdo->rollBack(); }
        $status = in_array((int) $exception->getCode(), [401, 403, 429], true) ? (int) $exception->getCode() : 500;
        jsonResponse(['error' => $status === 429
            ? $i18n->translate('mail.error.rateLimited', [], uiLocale($user))
            : ($status === 500 ? $i18n->translate('mail.error.settingsUnavailable', [], uiLocale($user)) : localizedMemberError($exception->getMessage(), $user))], $status);
    }
}
...
if ($action === 'translation-provider-test' && $method === 'POST') {
    ...
    try {
        ...
        foreach (['error', 'message'] as $messageField) {
            if (is_string($result[$messageField] ?? null)) {
                $result[$messageField] = SystemMessageLocalizer::translate($result[$messageField], uiLocale($user));
            }
        }
...
if ($action === 'update-ui-locale' && $method === 'POST') {
    requireCsrf();
    $body = bodyJson();
    $requestedLocale = is_string($body['locale'] ?? null) ? trim($body['locale']) : '';
    if (!I18n::isValidLocale($requestedLocale) || $i18n->selectLocale($requestedLocale) !== $requestedLocale) {
        jsonResponse(['error' => localizedAuthError('利用可能な画面言語を選択してください。', $user)], 422);
    }
    beginPageWriteTransaction($pdo);
    try {
        ...
        jsonResponse(['error' => localizedAuthError($status === 500 ? '画面言語を更新できませんでした。' : $exception->getMessage(), $user)], $status);
    }
...
if ($action === 'profile-avatar' && $method === 'GET') {
    ...
    if (!validStoredProfilePhoto($memberId, $storedName)) {
        jsonResponse(['error' => $i18n->translate('profile.photoNotFound', [], uiLocale($user))], 404);
    }
    ...
    if ($mime === '') {
        jsonResponse(['error' => $i18n->translate('profile.photoInvalid', [], uiLocale($user))], 404);
    }
    session_write_close();
    $path = ProfilePhoto::deliveryPath(uploadStoragePath(), $storedName);
...
if ($action === 'update-profile' && $method === 'POST') {
    ...
    if ($avatarKind === 'photo') {
        if (is_array($photo) && ($photo['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
            if (($photo['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
                $message = in_array($photo['error'], [UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE], true)
                    ? 'profile.photoServerLimit'
                    : 'profile.photoUploadFailed';
                jsonResponse(['error' => $i18n->translate($message, [], uiLocale($user))], 422);
            }
            $size = (int) ($photo['size'] ?? 0);
            if ($size < 1 || $size > profilePhotoLimitBytes()) {
                $limitMb = max(1, (int) floor(profilePhotoLimitBytes() / 1024 / 1024));
                jsonResponse(['error' => $i18n->translate('profile.photoSizeError', ['size' => $limitMb], uiLocale($user))], 422);
            }
            $temporaryPath = (string) ($photo['tmp_name'] ?? '');
            if (!is_uploaded_file($temporaryPath)) {
                jsonResponse(['error' => $i18n->translate('profile.photoInvalid', [], uiLocale($user))], 422);
            }
            ...
            try {
                $optimizedPhoto = ProfilePhoto::optimize($temporaryPath, $originalName);
            } catch (RuntimeException $exception) {
                jsonResponse(['error' => $i18n->translate($exception->getMessage(), [], uiLocale($user))], (int) $exception->getCode());
            }
            $uploadDirectory = uploadStoragePath();
            if (!is_dir($uploadDirectory) && !mkdir($uploadDirectory, 0770, true) && !is_dir($uploadDirectory)) {
                jsonResponse(['error' => $i18n->translate('profile.saveFailed', [], uiLocale($user))], 500);
            }
            ...
            if (@file_put_contents($destination, $optimizedPhoto['bytes'], LOCK_EX) !== strlen($optimizedPhoto['bytes'])) {
                @unlink($destination);
                jsonResponse(['error' => $i18n->translate('profile.saveFailed', [], uiLocale($user))], 500);
            }
        ...
        jsonResponse(['error' => $i18n->translate('profile.photoSelectRequired', [], uiLocale($user))], 422);
    }
    ...
    try {
        ...
        if ($avatarKind === 'photo' && $newPhoto === null) {
            ...
            if (($liveUser['avatar_kind'] ?? '') !== 'photo'
                ...
                throw new RuntimeException($i18n->translate('profile.photoSelectRequired', [], uiLocale($user)), 422);
            }
    ...
    try {
        ...
        if (empty($body['confirm'])) {
            $saved = $repository->updateTurnAnswer(
                ...
                $turnId,
                (int) $user['id'],
                SystemMessageLocalizer::message('error.aiNotApplied', [], uiLocale($user)),
                $expectedPasswordFingerprint
            );
            $saved['turn']['actions'] = [];
        ...
        $saved = $repository->updateTurnAnswer(
            ...
            $turnId,
            (int) $user['id'],
            (string) ($result['answer'] ?? SystemMessageLocalizer::message('error.aiApplied', [], uiLocale($user))),
            $expectedPasswordFingerprint,
            $result
        );
...
if ($action === 'upload-file' && $method === 'POST') {
    ...
    try {
        ...
        $extensionSource = $pluginFileService->fileSource($fileId, $user);
        $extensionEvents = is_array($extensionSource) ? [$pluginFileService->journalEvent('source.created', $extensionSource, ['actor_user_id' => (int) $user['id']])] : [];
    ...
    $pluginFileService->dispatchEvents($extensionEvents);
...
if ($action === 'delete-file' && $method === 'POST') {
    ...
    try {
        $deleted = deleteFileRecord($pdo, $user, $id, $expectedPasswordFingerprint, $pluginFileService);
    } catch (RuntimeException $exception) {
...
if ($action === 'invite-member' && $method === 'POST') {
    ...
    if ($name === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        jsonResponse(['error' => localizedMemberError('氏名と正しいメールアドレスを入力してください。', $user)], 422);
    }
    if ($department === '') {
        jsonResponse(['error' => localizedMemberError('部署を選択するか、新しい部署名を入力してください。', $user)], 422);
    }
    $memberLocale = $body['ui_locale'] ?? 'en-US';
    if (!is_string($memberLocale) || !in_array($memberLocale, array_column($i18n->availableLocales(), 'code'), true)) {
        jsonResponse(['error' => $i18n->translate('settings.invitation.invalidLocale', [], uiLocale($user))], 422);
    }
    ...
    try {
        ...
        $insert = $pdo->prepare('INSERT INTO users (name, email, password_hash, avatar_color, role, department, ui_locale, active, must_change_password, invited_at) VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1, CURRENT_TIMESTAMP)');
        $insert->execute([$name, $email, $passwordHash, $color, $role, $department, $memberLocale]);
        ...
        jsonResponse(['error' => localizedMemberError($status === 500 || $exception instanceof PDOException ? 'メンバーを招待できませんでした。' : $exception->getMessage(), $user)], $status);
    }

    $mail = Mailer::sendInvitation($name, $email, $password, baseUrl(), (string) $liveAdmin['name'], $database->storagePath(), false, $memberLocale, uiLocale($liveAdmin));
    jsonResponse([
        'ok' => true,
        'mail_sent' => $mail['sent'],
        ...
        'member' => ['id' => $memberId, 'name' => $name, 'email' => $email, 'avatar_color' => $color, 'avatar_kind' => 'initials', 'avatar_value' => '', 'ui_locale' => $memberLocale, 'role' => $role, 'department' => $department, 'must_change_password' => true, 'invited_at' => date('Y-m-d H:i:s'), 'last_login_at' => null, 'initial_login_pending' => true],
    ], 201);
}
...
if ($action === 'resend-invitation' && $method === 'POST') {
    ...
    $currentPassword = (string) ($body['current_password'] ?? '');
    if ($memberId < 1) {
        jsonResponse(['error' => localizedMemberError('再送するメンバーを確認してください。', $user)], 422);
    }
    ...
    try {
        ...
        $member['ui_locale'] = $lockedMember['ui_locale'] ?? 'en-US';
        ...
        jsonResponse(['error' => localizedMemberError($status === 500 ? '招待を再送できませんでした。' : $exception->getMessage(), $user)], $status);
    }
...
if ($action === 'reactivate-member' && $method === 'POST') {
    ...
    $allowedRoles = ['admin', 'content_admin', 'editor', 'author', 'commenter', 'viewer'];
    if ($memberId < 1 || !in_array($role, $allowedRoles, true)) {
        jsonResponse(['error' => localizedMemberError('復活するアカウントと権限を確認してください。', $user)], 422);
    }
    ...
    try {
        ...
        jsonResponse(['error' => localizedMemberError($status === 500 ? 'アカウントを復活できませんでした。' : $exception->getMessage(), $user)], $status);
    }
...
if ($action === 'reset-member-password' && $method === 'POST') {
    ...
    $currentPassword = (string) ($body['current_password'] ?? '');
    if ($memberId < 1 || $memberId === (int) $user['id']) {
        jsonResponse(['error' => localizedMemberError('パスワードを再設定するメンバーを確認してください。', $user)], 422);
    }
    ...
    try {
        ...
        jsonResponse(['error' => localizedMemberError($status === 500 ? 'パスワードを再設定できませんでした。' : $exception->getMessage(), $user)], $status);
    }
...
if ($action === 'plugins' && $method === 'GET') {
    requireRole($user, ['admin']);
    require_once dirname(__DIR__) . '/app/PluginDetachment.php';
    $pluginDetachment = new PluginDetachment(dirname(__DIR__), $database->storagePath());
    $pluginUpdates = new PluginUpdateService(dirname(__DIR__), $database->storagePath(), $pluginCompatibility);
    ...
    $adapterStates = $database->adapterStateStore()->load()['adapters'] ?? [];
    ...
    foreach ($installed as &$installedPlugin) {
        ...
        $installedPlugin += $pluginDetachment->managementState($installedPlugin, $database->backendId(), (array) ($adapterStates[$adapterId ?? ''] ?? []));
        $installedPlugin['update'] = $pluginUpdates->availableUpdate($installedPlugin, $catalogEntries);
        $installedPlugin['update_block_reason'] = $installedPlugin['installation_token'] === '' ? 'unsafe_plugin_path' : null;
        if ($adapterId !== null && !in_array((string) ($adapterStates[$adapterId]['lifecycle'] ?? 'INSTALLED'), ['INSTALLED', 'DISABLED', 'ERROR', 'ACTIVE'], true)) {
            $installedPlugin['update_block_reason'] = 'database_adapter_in_use';
        }
    }
    ...
    foreach ($available as &$availablePlugin) { $availablePlugin['compatibility'] = $pluginCompatibility->check($availablePlugin); }
    unset($availablePlugin);
...
if ($action === 'download-plugin' && $method === 'POST') {
    ...
    require_once dirname(__DIR__) . '/app/PluginDetachment.php';
    $installationService = new PluginDetachment(dirname(__DIR__), $database->storagePath());
    ...
    try {
        $plugin = $catalog->install(
            ...
            static function (array $_plugin) use (
                ...
                $installationLock = $installationService->acquireLock($pluginId);
                beginPageWriteTransaction($pdo);
                $liveAdmin = requireLockedApplicationAdminForWrite(
                    $pdo,
                    $user,
        ...
        if (is_resource($installationLock)) { flock($installationLock, LOCK_UN); fclose($installationLock); }
...
if ($action === 'update-plugin' && $method === 'POST') {
    requireCsrf();
    requireRole($user, ['admin']);
    $body = bodyJson();
    $pluginId = $body['plugin_id'] ?? null;
    $version = $body['version'] ?? null;
    $token = $body['installation_token'] ?? null;
    $targetVersion = $body['target_version'] ?? null;
    $packageHash = $body['package_sha256'] ?? null;
    if (!is_string($pluginId) || !PluginManager::isValidId($pluginId)
        || !is_string($version) || !PluginManager::isValidVersion($version)
        || !is_string($targetVersion) || !PluginManager::isValidVersion($targetVersion)
        || !is_string($token) || preg_match('/^[a-f0-9]{64}$/D', $token) !== 1
        || !is_string($packageHash) || preg_match('/^[a-f0-9]{64}$/D', $packageHash) !== 1
        || PluginCompatibility::compareVersions($targetVersion, $version) <= 0) {
        jsonResponse(['error' => '更新するプラグインを一覧から選び直してください。'], 422);
    }
    $updates = new PluginUpdateService(dirname(__DIR__), $database->storagePath(), $pluginCompatibility);
    $installation = new PluginDetachment(dirname(__DIR__), $database->storagePath());
    $installationLock = null; $exclusive = false; $result = null;
    try {
        // Authenticate first, then perform network work outside the exclusive writer barrier.
        $prepared = $updates->prepare(PluginCatalog::official(dirname(__DIR__) . '/plugins'), $pluginId, $targetVersion, $packageHash);
        $installationLock = $installation->acquireLock($pluginId);
        $database->beginAdapterCutover(['operation' => 'plugin_update', 'adapter' => $pluginId]);
        $exclusive = true;
        beginPageWriteTransaction($pdo);
        $liveAdmin = requireLockedApplicationAdminForWrite($pdo, $user, $expectedPasswordFingerprint);
        $database->throwIfAdapterCutoverCancelled();
        $adapterId = match ($pluginId) { 'database-mysql-adapter' => 'mysql', 'database-postgresql-adapter' => 'postgresql', default => '' };
        $adapterState = $database->adapterStateStore()->load()['adapters'][$adapterId] ?? [];
        if ($adapterId !== '' && !in_array((string) ($adapterState['lifecycle'] ?? 'INSTALLED'), ['INSTALLED', 'DISABLED', 'ERROR', 'ACTIVE'], true)) {
            throw new RuntimeException('DBプラグインは設定・移行処理中のため更新できません。完了後に再度お試しください。', 409);
        }
        $currentManager = new PluginManager(dirname(__DIR__) . '/plugins', false, $pluginCompatibility);
        $current = $currentManager->plugin($pluginId);
        if ($current === null) { throw new RuntimeException('インストール済みのプラグインが見つかりません。', 404); }
        // Preserve the latest stored state, including a concurrent ON/OFF that finished while downloading.
        $storedState = setting($pdo, 'plugin.' . $pluginId . '.enabled', null);
        $requestedEnabled = $storedState === null ? (bool) $current['requested_enabled'] : $storedState === '1';
        $database->updateAdapterCutoverStage('updating_plugin', [], false);
        $result = $updates->apply($prepared, $version, $token);
        setSetting($pdo, 'plugin.' . $pluginId . '.enabled', $requestedEnabled ? '1' : '0');
        audit($pdo, (int) $liveAdmin['id'], 'plugin_updated', 'plugin', 0, [
            'plugin_id' => $pluginId, 'previous_version' => $version, 'version' => $targetVersion,
            'enabled' => $requestedEnabled, 'data_retained' => true, 'retained_code' => $result['backup'],
        ]);
        $pdo->commit();
        // If this durable marker cannot be written, restore only the code; user data was never changed.
        $updates->commit();
        $result['enabled'] = $requestedEnabled;
        unset($result['backup']);
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) { $pdo->rollBack(); }
        try { $updates->rollback(); } catch (Throwable $restoreError) { $exception = $restoreError; }
        if ($exclusive) { try { $database->finishAdapterCutover(false); } catch (Throwable) {} }
        if (is_resource($installationLock)) { flock($installationLock, LOCK_UN); fclose($installationLock); }
        $status = $exception instanceof DatabaseAdapterException ? 409
            : (in_array((int) $exception->getCode(), [401, 403, 404, 409, 422], true) ? (int) $exception->getCode() : 422);
        if ($status === 401) { unset($_SESSION['user_id'], $_SESSION['user_password_fingerprint']); }
        jsonResponse(['error' => $exception instanceof DatabaseAdapterException
                ? 'ほかの処理が実行中のため更新できませんでした。完了後に再度お試しください。'
                : ($exception->getCode() !== 0 ? $exception->getMessage() : '新版の取得または検証に失敗しました。旧版を維持します。'),
            'code' => 'plugin_update_failed'], $status);
    }
    $database->finishAdapterCutover(false);
    if (is_resource($installationLock)) { flock($installationLock, LOCK_UN); fclose($installationLock); }
    jsonResponse(['ok' => true, 'plugin' => $result, 'data_retained' => true, 'reload_required' => true]);
}
...
if ($action === 'toggle-plugin' && $method === 'POST') {
    ...
    require_once dirname(__DIR__) . '/app/PluginDetachment.php';
    $installationService = new PluginDetachment(dirname(__DIR__), $database->storagePath());
    $installationLock = null; $activationFailure = null; $currentPlugins = null;
    try {
        $installationLock = $installationService->acquireLock($pluginId);
        beginPageWriteTransaction($pdo);
        $liveAdmin = requireLockedApplicationAdminForWrite($pdo, $user, $expectedPasswordFingerprint);
        // Re-read the installed manifest under the same lock used by install/detach.
        $currentPlugins = new PluginManager(dirname(__DIR__) . '/plugins', false, $pluginCompatibility);
        $plugin = null;
        foreach ($currentPlugins->summaries() as $candidate) { if ($candidate['id'] === $pluginId) { $plugin = $candidate; break; } }
        if ($plugin === null) { throw new RuntimeException('インストール済みのプラグインが見つかりません。', 404); }
        $activationFailure = $currentPlugins->activationError($pluginId) ?? $pluginManager->activationError($pluginId);
        if ($enabled && ($activationFailure !== null || !$currentPlugins->canEnable($pluginId))) {
            throw new RuntimeException('このプラグインは有効化できません。対応バージョンとエラー内容を確認してください。', 409);
        }
        ...
        if ($pdo->inTransaction()) { $pdo->rollBack(); }
        if (is_resource($installationLock)) { flock($installationLock, LOCK_UN); fclose($installationLock); }
        $status = in_array((int) $exception->getCode(), [401, 403, 404, 409], true) ? (int) $exception->getCode() : 500;
        if ($status === 401) { unset($_SESSION['user_id'], $_SESSION['user_password_fingerprint']); }
        jsonResponse(['error' => $status === 500 ? 'プラグイン設定を更新できませんでした。' : $exception->getMessage(),
            'code' => $activationFailure['code'] ?? 'plugin_state_update_failed',
            'compatibility' => $currentPlugins?->compatibilityFor($pluginId)], $status);
    }
    if (is_resource($installationLock)) { flock($installationLock, LOCK_UN); fclose($installationLock); }
    jsonResponse([
        'ok' => true,
...
if ($action === 'detach-plugin' && $method === 'POST') {
    requireCsrf();
    requireRole($user, ['admin']);
    $body = bodyJson();
    $pluginId = $body['plugin_id'] ?? null;
    $version = $body['version'] ?? null;
    $token = $body['installation_token'] ?? null;
    if (!is_string($pluginId) || !PluginManager::isValidId($pluginId) || !is_string($version)
        || !is_string($token) || preg_match('/^[a-f0-9]{64}$/D', $token) !== 1) {
        jsonResponse(['error' => '取り外すプラグインを一覧から選び直してください。'], 422);
    }
    require_once dirname(__DIR__) . '/app/PluginDetachment.php';
    $detachment = new PluginDetachment(dirname(__DIR__), $database->storagePath());
    $installationLock = null; $detachmentReason = null;
    try {
        $installationLock = $detachment->acquireLock($pluginId);
        beginPageWriteTransaction($pdo);
        $liveAdmin = requireLockedApplicationAdminForWrite($pdo, $user, $expectedPasswordFingerprint);
        $currentPlugins = new PluginManager(dirname(__DIR__) . '/plugins', false, $pluginCompatibility);
        $plugin = null;
        foreach ($currentPlugins->summaries() as $candidate) { if ($candidate['id'] === $pluginId) { $plugin = $candidate; break; } }
        if ($plugin === null) { throw new RuntimeException('プラグインが見つかりません。', 404); }
        if ($plugin['version'] !== $version || !hash_equals($detachment->installationToken($pluginId), $token)) {
            throw new RuntimeException('プラグインが更新されています。一覧を再読み込みしてください。', 409);
        }
        // Preserve the boot diagnostic produced by this request, without executing the plugin again.
        $plugin['error'] ??= $pluginManager->activationError($pluginId);
        $adapterId = match ($pluginId) { 'database-mysql-adapter' => 'mysql', 'database-postgresql-adapter' => 'postgresql', default => '' };
        $adapterState = $database->adapterStateStore()->load()['adapters'][$adapterId] ?? [];
        $detachState = $detachment->managementState($plugin, $database->backendId(), (array) $adapterState);
        if (!$detachState['can_detach']) {
            $detachmentReason = $detachState['detach_block_reason'];
            throw new RuntimeException('このプラグインは取り外せません。エラー状態とデータベースの使用状況を確認してください。', 409);
        }
        // This setting row is also the common API scope write fence. Existing jobs cannot commit after this change.
        setSetting($pdo, 'plugin.' . $pluginId . '.enabled', '0');
        $retainedCode = $detachment->move($pluginId, $token);
        audit($pdo, (int) $liveAdmin['id'], 'plugin_detached', 'plugin', 0, [
            'plugin_id' => $pluginId, 'version' => $version, 'data_retained' => true, 'retained_code' => $retainedCode,
        ]);
        $pdo->commit();
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) { $pdo->rollBack(); }
        try { $detachment->restoreAfterRollback(); }
        catch (Throwable $restoreError) { $exception = $restoreError; }
        if (is_resource($installationLock)) { flock($installationLock, LOCK_UN); fclose($installationLock); }
        $status = in_array((int) $exception->getCode(), [401, 403, 404, 409], true) ? (int) $exception->getCode() : 500;
        if ($status === 401) { unset($_SESSION['user_id'], $_SESSION['user_password_fingerprint']); }
        jsonResponse(['error' => $status === 500 ? 'プラグインを取り外せませんでした。' : $exception->getMessage(),
            'code' => $detachmentReason ?? 'plugin_detach_failed'], $status);
    }
    if (is_resource($installationLock)) { flock($installationLock, LOCK_UN); fclose($installationLock); }
    jsonResponse(['ok' => true, 'plugin_id' => $pluginId, 'data_retained' => true, 'reload_required' => true]);
}
...
SQL)->fetchAll();
    ...
    $notificationInbox = (new NotificationInbox($pdo))->page($user);
    $notificationRows = $notificationInbox['notifications'];
    $files = $pdo->query(<<<'SQL'
...
SQL)->fetchAll();
    foreach ($files as &$file) {
        ...
        $extensionSource = $pluginApiRuntime->fileSource((int) $file['id'], $user);
        $file['extension_states'] = $extensionSource === null ? [] : $pluginApiRegistry->fileStates($extensionSource, $user);
        $file['extension_actions'] = $extensionSource === null ? [] : $pluginApiRegistry->fileActions($extensionSource, $user);
        $file['source_reference'] = $extensionSource === null ? null : array_intersect_key($extensionSource, array_flip(['source_type','source_id','source_version','content_hash']));
        $file['can_replace'] = $extensionSource !== null && ($user['role'] === 'admin' || (int) $file['uploaded_by'] === (int) $user['id']);
...
SQL)->fetchAll();
    ...
    $bootstrapResponse = [
        'user' => $user,
        'csrf' => $_SESSION['csrf'],
        'plugin_state' => $pluginManager->clientState($user),
        'pages' => $pages,
        'trash' => $trash,
        'files' => $files,
        ...
        'notification_counts' => $notificationInbox['counts'],
        'notification_pagination' => $notificationInbox['pagination'],
        'members' => $members,
...
if ($action === 'create-page' && $method === 'POST') {
    ...
    try {
        ...
        $statement->execute([$parentId, $sortOrder, $i18n->translate('page.untitled', [], uiLocale($user)), '📄', 'none', $status, $i18n->translate('page.defaultCategory', [], uiLocale($user)), '[]', '[]', json_encode($blocks, JSON_UNESCAPED_UNICODE), '', $visibility, $accessDepartment, $languageCode, (int) $user['id'], (int) $user['id']]);
...
if ($action === 'move-page' && $method === 'POST') {
    ...
    try {
        ...
        $extensionPriorEvents = $pluginFileService->journalPageSources($movingSubtree['ids'], ['operation' => 'before-move', 'change_id' => bin2hex(random_bytes(16))]);
        ...
        $extensionEvents = [...$extensionPriorEvents, ...$pluginFileService->journalPageSources($movingSubtree['ids'], ['operation' => 'move', 'change_id' => bin2hex(random_bytes(16)), 'actor_user_id' => (int) $user['id']], true)];
        $pdo->commit();
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
...
if ($action === 'save-page' && $method === 'POST') {
    ...
    $title = cleanText((string) ($body['title'] ?? ''), 240) ?: $i18n->translate('page.untitled', [], uiLocale($user));
    ...
    $applyStatusToDescendants = !empty($body['apply_status_to_descendants']);
    if ($applyStatusToDescendants && ($expectedAccessState === null
        || !in_array($body['status'] ?? null, ['draft', 'review', 'published', 'private'], true))) {
        jsonResponse(['error' => '一括変更する状態と現在のページ設定を確認してください。', 'code' => 'invalid_subtree_status_request'], 422);
    }
    $statusSubtree = null;
    $statusChangedPageIds = [];
    ...
    try {
        ...
        if ($applyStatusToDescendants) {
            if ($publicRequestBlocked || $status !== $requestedStatus) {
                $abortSave(['error' => '親ページの状態または共有設定が変更されています。最新の設定を確認してください。',
                    'code' => 'subtree_status_conflict'], 409);
            }
            $statusSubtree = lockedPageSubtreeForUpdate($pdo, $id);
            if ($statusSubtree['reason'] !== null) {
                $abortSave(['error' => 'ページ階層が別の操作で更新されました。もう一度お試しください。',
                    'code' => 'page_hierarchy_conflict'], 409);
            }
            if (count($statusSubtree['ids']) < 2) {
                $abortSave(['error' => '子ページがありません。ページ一覧を更新してください。', 'code' => 'page_has_no_children'], 409);
            }
            foreach ($statusSubtree['rows'] as $descendant) {
                if (!canEditPage($pdo, $user, $descendant)) {
                    $abortSave(['error' => '配下に編集できないページがあるため、一括変更できません。',
                        'code' => 'subtree_status_forbidden'], 403);
                }
            }
            $publicationScopeIds = $statusSubtree['ids'];
        }
        if (($applyStatusToDescendants && $status !== 'published')
            || ((string) ($existing['status'] ?? '') === 'published' && $status !== 'published')
            || $status === 'private') {
            $publicationScopeIds = $statusSubtree['ids'] ?? [$id];
            if ($status === 'private' && $statusSubtree === null) {
        ...
        $meaningfulUpdate = $contentChanged
            ...
            || (in_array($body['cover'] ?? '', ['none', 'mint', 'blue', 'sand', 'coral', 'lavender', 'night', 'black', 'navy', 'midnight', 'red', 'primary-blue', 'yellow'], true) ? $body['cover'] : 'none') !== (string) $existing['cover']
            || $status !== (string) $existing['status']
            || $visibility !== (string) $existing['visibility']
            || $accessChanged
            || cleanText((string) ($body['category'] ?? $existing['category'] ?? $i18n->translate('page.defaultCategory', [], uiLocale($user))), 80) !== (string) $existing['category']
        ...
        $meta = KnowledgeFileVersions::revisionMetadata($pdo, $existing, $meta);
        ...
        $extensionPriorEvents = $pluginFileService->journalPageSources([$id, ...($publicationScopeIds ?? [])], ['operation' => 'before-save', 'change_id' => bin2hex(random_bytes(16))]);
        $statement = $pdo->prepare(<<<'SQL'
...
SQL);
        $statement->execute([
            ...
            in_array($body['cover'] ?? '', ['none', 'mint', 'blue', 'sand', 'coral', 'lavender', 'night', 'black', 'navy', 'midnight', 'red', 'primary-blue', 'yellow'], true) ? $body['cover'] : 'none',
            $status,
            cleanText((string) ($body['category'] ?? $existing['category'] ?? $i18n->translate('page.defaultCategory', [], uiLocale($user))), 80),
        ...
        if ($applyStatusToDescendants) {
            $statusChangedPageIds = PageSubtreeStatus::apply($pdo, $statusSubtree['rows'], $id, $status, $user);
            $inheritedPageIds = $statusChangedPageIds;
        ...
        if ($meaningfulUpdate && (int) $existing['author_id'] !== (int) $user['id']) {
            $recipientLocale = notificationRecipientLocale($pdo, (int) $existing['author_id']);
            $actorName = cleanText((string) $user['name'], 120) ?: SystemMessageLocalizer::message('notification.member', [], $recipientLocale);
            createLocalizedNotification(
                $pdo,
                (int) $existing['author_id'],
                'update',
                'notification.update', ['name' => $actorName, 'title' => $title],
                $id, 'system-messages', true
            );
        }
        audit($pdo, (int) $user['id'], 'updated', 'page', $id, ['title' => $title]);
        $affectedPublications = ($meaningfulUpdate || $statusChangedPageIds !== [])
            ? $affectedPublicationsForPageIds([$id, ...$inheritedPageIds])
            : [];
        $extensionEvents = [...$extensionPriorEvents, ...$pluginFileService->journalPageSources([$id, ...$inheritedPageIds], ['operation' => 'save', 'change_id' => bin2hex(random_bytes(16)), 'actor_user_id' => (int) $user['id']], $accessChanged || $statusChangedPageIds !== [])];
        $pdo->commit();
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
    ...
    foreach ($statusChangedPageIds as $changedPageId) {
        Hooks::action('article_updated', ['id' => $changedPageId], $statusSubtree['rows'][$changedPageId]);
        if ($status === 'published' && $statusSubtree['rows'][$changedPageId]['status'] !== 'published') {
            Hooks::action('article_published', ['id' => $changedPageId]);
        }
    }
    ...
    jsonResponse([
        ...
        'status_changed_pages' => count($statusChangedPageIds),
        'rag' => $rag,
        'affected_publications' => $affectedPublications,
    ]);
}
...
if ($action === 'archive-page' && $method === 'POST') {
    ...
    try {
        ...
        $extensionPriorEvents = $pluginFileService->journalPageSources($ids, ['operation' => 'before-archive', 'change_id' => bin2hex(random_bytes(16))]);
        $statement->execute($parameters);
        invalidateRagPageGenerations($pdo, $ids);
        audit($pdo, (int) $user['id'], 'archived', 'page', $id, ['page_count' => count($ids)]);
        $extensionEvents = [...$extensionPriorEvents, ...$pluginFileService->journalPageSources($ids, ['operation' => 'archive', 'change_id' => bin2hex(random_bytes(16))], true)];
        $pdo->commit();
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
...
if ($action === 'restore-page' && $method === 'POST') {
    ...
    try {
        ...
        $extensionEvents = $pluginFileService->journalPageSources($ids, ['operation' => 'restore', 'change_id' => bin2hex(random_bytes(16))], true);
        $pdo->commit();
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
...
if ($action === 'delete-page-permanently' && $method === 'POST') {
    ...
    try {
        ...
        $abortPermanentDelete(['error' => '正本と履歴を保持するため、ページはゴミ箱で保管します。', 'code' => 'canonical_retention_required'], 409);
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
        }
...
if ($action === 'restore-revision' && $method === 'POST') {
    ...
    try {
        ...
        $extensionPriorEvents = $pluginFileService->journalPageSources([(int) $page['id'], ...($publicationScopeIds ?? [])], ['operation' => 'before-restore-revision', 'change_id' => bin2hex(random_bytes(16))]);
        ...
        $update->execute([$snapshot['title'], $snapshot['icon'], $snapshot['blocks_json'], utf8Slice(blocksPlainText($restoredBlocks), 100000), $restoredStatus, $meta['category'] ?? $page['category'] ?? $i18n->translate('page.defaultCategory', [], uiLocale($user)), $restoredTagsJson, $restoredTagsJson, $restoredVisibility, $restoredAccessDepartment, $restoredStatus, (int) $user['id'], (int) $page['id']]);
        ...
        $extensionEvents = [...$extensionPriorEvents, ...$pluginFileService->journalPageSources([(int) $page['id'], ...$inheritedPageIds], ['operation' => 'restore-revision', 'change_id' => bin2hex(random_bytes(16))], true)];
        $pdo->commit();
    } catch (Throwable $exception) {
        if ($pdo->inTransaction()) {
            $pdo->rollBack();
...
if ($action === 'notifications' && $method === 'GET') {
    try {
        $notificationFilter = NotificationInbox::filter($_GET['filter'] ?? 'all');
        $notificationPage = NotificationInbox::pageNumber($_GET['page'] ?? 1);
    } catch (InvalidArgumentException $exception) {
        jsonResponse(['error' => $exception->getMessage()], 422);
    }
    $notificationFingerprint = sessionPasswordFingerprint() ?? '';
    $ownsPageSnapshot = beginConsistentPageRead($pdo);
    $snapshotUser = authenticatedApplicationUserForReadSnapshot($pdo, $user, $notificationFingerprint);
    if (!$snapshotUser || !empty($snapshotUser['must_change_password'])) {
        if ($ownsPageSnapshot && $pdo->inTransaction()) {
            $pdo->rollBack();
        }
        jsonResponse(['error' => 'ログインが必要です。'], 401);
    ...
    $user = $snapshotUser;
    $notificationResponse = (new NotificationInbox($pdo))->page($user, $notificationFilter, $notificationPage);
    $notificationResponse['plugin_state'] = $pluginManager->clientState($user);
    if ($ownsPageSnapshot) {
        $pdo->commit();
    }
...
if (in_array($action, ['notification-read', 'notification-archive', 'notification-delete', 'mark-notifications'], true) && $method === 'POST') {
    requireCsrf();
    $body = bodyJson();
    $id = filter_var($body['id'] ?? 0, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    if ($action !== 'mark-notifications' && !is_int($id)) {
        jsonResponse(['error' => '通知を選択してください。'], 422);
    }
    $isRead = !array_key_exists('is_read', $body) || !empty($body['is_read']);
    $operation = match ($action) {
        'notification-read' => 'read', 'notification-archive' => 'archive', 'notification-delete' => 'delete', default => 'mark-all',
    };
    try {
        $response = (new NotificationInbox($pdo))->mutate($operation, $user, sessionPasswordFingerprint() ?? '', (int) $id, $isRead);
    } catch (Throwable $exception) {
        $status = NotificationInbox::transientConflict($exception) ? 409 : (in_array((int) $exception->getCode(), [401, 403, 404, 409, 422], true) ? (int) $exception->getCode() : 500);
        jsonResponse(['error' => $status === 500 ? '通知を更新できませんでした。' : ($status === 409 && $exception instanceof PDOException ? '通知が別の操作で更新中です。再度お試しください。' : $exception->getMessage())], $status);
    }
