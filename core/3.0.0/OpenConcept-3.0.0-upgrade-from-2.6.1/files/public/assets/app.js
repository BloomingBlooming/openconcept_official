(() => {
    ...
    const state = {
        ...
        user: null,
        authLocaleChoice: window.OPENCONCEPT_BOOT?.auth_locale || '',
        initialized: false,
        pages: [],
        trash: [],
        files: [],
        ...
        inboxFilter: 'all',
        inboxPage: 1,
        notificationCounts: { all: 0, unread: 0, archived: 0 },
        inboxPagination: { page: 1, per_page: 10, total: 0, total_pages: 1 },
        inboxLoading: false,
        inboxError: '',
        inboxBusy: '',
        inboxOpeningId: 0,
        ...
        activePlugins: { ...(window.OPENCONCEPT_BOOT?.plugins || {}) },
        ...
        wafCompatibilityEnabled: false,
        wafCompatibilityKey: null,
        wafCompatibilityDraft: null,
        wafCompatibilityLoading: false,
        wafCompatibilitySaving: false,
        wafCompatibilityError: '',
    ...
    function applyI18n(bundle) {
        ...
        if (typeof bundle.messages?.['auth.pageTitle'] === 'string') document.title = `OpenConcept — ${bundle.messages['auth.pageTitle']}`;
    ...
    const aiSearchAssetLoads = new Set();
    let notificationRequest = 0;
    let notificationInFlight = 0;
    let notificationSession = 0;
    let notificationNavigation = 0;
    let pluginStateChannel = null;
    try {
        if (typeof BroadcastChannel === 'function') {
            pluginStateChannel = new BroadcastChannel(`openconcept:plugins:${new URL('.', window.location.href).pathname}`);
            pluginStateChannel.onmessage = () => {
                if (state.user && !databaseMigrationPollingPaused) refreshNotifications();
            };
        }
    } catch (_) { /* Visibility changes and polling also refresh plugin state. */ }

    function isPluginActive(id) {
        return Object.prototype.hasOwnProperty.call(state.activePlugins, id);
    }

    function unmountAiSearchExtension(id) {
        if (!mountedAiSearchExtensions.delete(id)) return;
        try { aiSearchExtensions.get(id)?.unmount?.(); }
        catch (error) { console.error(`[plugin:${id}] AI extension unmount failed`, error); }
    }

    function loadAiSearchAssets(assets = {}) {
        const aiPluginIds = new Set(state.pluginUi.filter(item => item.placement === 'ai-search').map(item => item.id));
        for (const type of ['styles', 'scripts']) {
            for (const asset of Array.isArray(assets[type]) ? assets[type] : []) {
                if (!isPluginActive(asset.plugin) || !aiPluginIds.has(asset.plugin)) continue;
                const url = new URL(`plugin-asset.php?plugin=${encodeURIComponent(asset.plugin)}&v=${encodeURIComponent(asset.version)}&file=${encodeURIComponent(asset.file)}`, window.location.href).href;
                const selector = type === 'styles' ? 'link[rel="stylesheet"]' : 'script[src]';
                const attribute = type === 'styles' ? 'href' : 'src';
                if (aiSearchAssetLoads.has(url) || [...document.querySelectorAll(selector)].some(element => element[attribute] === url)) continue;
                const element = document.createElement(type === 'styles' ? 'link' : 'script');
                if (type === 'styles') element.rel = 'stylesheet';
                else element.async = false;
                element[attribute] = url;
                aiSearchAssetLoads.add(url);
                element.onload = () => { aiSearchAssetLoads.delete(url); syncAiSearchExtensions(); };
                element.onerror = () => { aiSearchAssetLoads.delete(url); element.remove(); };
                document.head.appendChild(element);
            }
        }
    }

    function applyPluginState(snapshot) {
        if (!snapshot || !snapshot.plugins || !Array.isArray(snapshot.plugin_ui)) return;
        const activePlugins = { ...snapshot.plugins };
        const changed = JSON.stringify(state.activePlugins) !== JSON.stringify(activePlugins)
            || JSON.stringify(state.pluginUi) !== JSON.stringify(snapshot.plugin_ui);
        state.activePlugins = activePlugins;
        state.pluginUi = snapshot.plugin_ui.filter(item => isPluginActive(item.id));
        if (changed) {
            state.installedPlugins.forEach(plugin => {
                // Only runtime AI extensions are synchronized without a full settings reload.
                if (plugin.ui?.sidebar?.placement === 'ai-search') plugin.enabled = isPluginActive(plugin.id);
            });
        ...
        loadAiSearchAssets(snapshot.ai_search_assets);
    }
    ...
    window.OpenConceptPlugins = Object.freeze({
        ...
        unregister(pluginId) {
            const id = String(pluginId || '');
            window.OpenConceptPluginApi?.unregister(id);
            unmountAiSearchExtension(id);
    ...
    const icons = {
        ...
        moreCircle: '<circle cx="12" cy="12" r="9" fill="none" stroke-width="1.5"></circle><g fill="currentColor" stroke="none"><circle cx="7.5" cy="12" r="1.1"></circle><circle cx="12" cy="12" r="1.1"></circle><circle cx="16.5" cy="12" r="1.1"></circle></g>',
    ...
    const appVersion = window.OPENCONCEPT_BOOT?.app_version || '3.0.0';
    ...
    function avatarMarkup(profile = {}, size = '') {
        ...
        if (kind === 'photo' && id && value) {
            const src = `api.php?action=profile-avatar&id=${encodeURIComponent(id)}&v=${encodeURIComponent(value)}&thumb=1`;
            return `<span class="${classes} avatar-photo" style="background:${esc(color)}"><img src="${esc(src)}" alt="${esc(t('profile.photoAlt', { name }))}" loading="lazy" decoding="async"></span>`;
        }
        if (kind === 'emoji' && value) {
    ...
    const coverValues = ['none', 'mint', 'blue', 'sand', 'coral', 'lavender', 'night', 'black', 'navy', 'midnight', 'red', 'primary-blue', 'yellow'];
    ...
    function encryptionFailure(code, messageKey) {
    ...
    async function encryptJsonBody(json, action, method = 'POST') {
        if (!globalThis.crypto?.subtle || !globalThis.crypto?.getRandomValues) {
            throw encryptionFailure('waf_crypto_unavailable', 'common.wafCryptoUnavailable');
        }
        let rawKey;
        try {
            const encodedKey = state.wafCompatibilityKey;
            if (typeof encodedKey !== 'string' || encodedKey.length !== 44) throw new Error();
            const binaryKey = atob(encodedKey);
            if (binaryKey.length !== 32 || btoa(binaryKey) !== encodedKey) throw new Error();
            rawKey = Uint8Array.from(binaryKey, character => character.charCodeAt(0));
        } catch {
            throw encryptionFailure('waf_compatibility_key_missing', 'common.wafCryptoKeyMissing');
        }
        const plain = new TextEncoder().encode(json);
        if (plain.length > 16 * 1024 * 1024) {
            throw encryptionFailure('request_body_too_large', 'common.wafCryptoTooLarge');
        }
        try {
            const key = await globalThis.crypto.subtle.importKey('raw', rawKey, 'AES-GCM', false, ['encrypt']);
            const iv = globalThis.crypto.getRandomValues(new Uint8Array(12));
            const additionalData = new TextEncoder().encode(`OpenConcept:aes-256-gcm-v1:${method.toUpperCase()}:${action}`);
            const encrypted = new Uint8Array(await globalThis.crypto.subtle.encrypt({ name: 'AES-GCM', iv, additionalData, tagLength: 128 }, key, plain));
            const packet = new Uint8Array(iv.length + encrypted.length);
            packet.set(iv);
            packet.set(encrypted, iv.length);
            return JSON.stringify({ payload: bytesToBase64(packet) });
        } catch {
            throw encryptionFailure('waf_encryption_failed', 'common.wafCryptoFailed');
        }
    }
    ...
    async function readApiResponse(response) {
        ...
        const knownWafBlock = response.status === 403
            && /lolipop\.jp\/support\/faq\/support\/000804\/?/i.test(text)
            && /不正なパラメータ|アクセスをブロック/.test(text);
        const message = knownWafBlock ? t(state.wafCompatibilityEnabled ? 'common.wafBlockedEnabled' : 'common.wafBlocked')
            : response.status === 403 ? t('common.serverBlocked')
            : t('common.invalidServerResponseStatus', { status: response.status });
        const error = new Error(message);
        error.status = response.status;
        error.code = knownWafBlock ? 'waf_blocked' : 'invalid_server_response';
        error.data = null;
        throw error;
    }

    async function api(action, options = {}) {
        if (action === 'save-page' && state.saveSessionChanged) {
            throw encryptionFailure('save_session_changed', 'page.saveSessionChanged');
        }
        ...
        if (typeof FormData !== 'undefined' && options.body instanceof FormData) {
            ...
            if (state.user && state.wafCompatibilityEnabled && config.method.toUpperCase() === 'POST'
                && !['login', 'logout', 'initialize', 'guest-ui-locale', 'waf-compatibility-settings'].includes(action)) {
                config.headers['X-OpenConcept-Body-Encoding'] = 'aes-256-gcm-v1';
                config.body = await encryptJsonBody(config.body, action, config.method);
            }
        }
        const authLocaleQuery = state.authLocaleChoice && ['session', 'login', 'initialize', 'guest-ui-locale'].includes(action)
            ? `&lang=${encodeURIComponent(state.authLocaleChoice)}` : '';
        const response = await fetch(`api.php?action=${encodeURIComponent(action)}${options.query || ''}${authLocaleQuery}`, config);
        ...
        if (!response.ok) {
            ...
            error.status = response.status;
            error.data = data;
            if (['waf_encryption_unavailable', 'waf_compatibility_key_missing', 'encrypted_body_invalid'].includes(error.code)) error.retryable = false;
            throw error;
        }
        synchronizeCsrf(data?.csrf);
        if (typeof data.settings?.waf_compatibility_enabled === 'boolean') {
            state.wafCompatibilityEnabled = data.settings.waf_compatibility_enabled;
            state.wafCompatibilityKey = state.wafCompatibilityEnabled && typeof data.settings.waf_compatibility_key === 'string'
                ? data.settings.waf_compatibility_key : null;
        }
        mergeAffectedPublications(data?.affected_publications);
        return data;
    }

    window.OpenConceptPluginApi?.configure({
        request: api,
        changed: async () => {
            await refreshSummary();
            if (state.view === 'files') renderApp();
            else updateNotificationChrome();
        },
    });
    ...
    function clearWorkspaceState() {
        ...
        window.OpenConceptPluginApi?.closeModal();
        cleanupProfileDraft();
    ...
    function renderAuthLanguage() {
        return `<div class="form-field auth-language"><label for="authLanguage" lang="en" translate="no">${esc(t('auth.language'))}</label><select class="input" id="authLanguage" name="ui_locale" autocomplete="off">${availableContentLanguages().map(locale => `<option value="${esc(locale.code)}" ${locale.code === state.i18n.locale ? 'selected' : ''}>${esc(locale.name)}</option>`).join('')}</select></div>`;
    }

    function clearAuthLocaleChoice() {
        if (!state.authLocaleChoice) return;
        state.authLocaleChoice = '';
    ...
    function bindAuthLanguage(renderScreen) {
        const select = document.getElementById('authLanguage');
        select.addEventListener('change', async () => {
            const form = select.form;
            const values = Array.from(form.elements).filter(input => input.name && input !== select).map(input => [input.name, input.value]);
            ...
            try {
                ...
                const data = await api(state.user ? 'update-ui-locale' : 'guest-ui-locale', { method: 'POST', body: { locale } });
                applyI18n(data.i18n);
                if (state.user) {
                    state.user.ui_locale = locale;
                    clearAuthLocaleChoice();
                } else {
                    state.authLocaleChoice = locale;
                    const url = new URL(window.location.href);
                    url.searchParams.set('lang', locale);
                    window.history.replaceState(window.history.state, '', url);
                }
                renderScreen();
                const nextForm = document.getElementById('authLanguage').form;
                values.forEach(([name, value]) => { if (nextForm.elements[name]) nextForm.elements[name].value = value; });
                document.getElementById('authLanguage').focus();
            } catch (error) {
                select.value = state.i18n.locale;
    ...
    function renderSetup() {
        ...
        document.getElementById('setupForm').insertAdjacentHTML('afterbegin', renderAuthLanguage());
        bindAuthLanguage(renderSetup);
    }
    ...
    function renderPasswordChange() {
        ...
        document.getElementById('passwordChangeForm').insertAdjacentHTML('afterbegin', renderAuthLanguage());
        bindAuthLanguage(renderPasswordChange);
    }
    ...
    let extensionMaintenanceAt = 0;
    function scheduleExtensionMaintenance() {
        if (!state.user || extensionMaintenanceRunning || Date.now() - extensionMaintenanceAt < 60000) return;
        extensionMaintenanceRunning = true;
        extensionMaintenanceAt = Date.now();
        // A separate request starts after the application has rendered. Search never performs extraction.
        setTimeout(() => {
            if (!state.user) { extensionMaintenanceRunning = false; return; }
            api('extension-tick', { method: 'POST', body: {} })
                .then(() => { if (state.user) return refreshNotifications(); })
                .catch(() => { /* The durable queue resumes on a later visit. */ })
                .finally(() => { extensionMaintenanceRunning = false; });
        }, 250);
    }

    async function loadWorkspace({ restoreRoute = false } = {}) {
        extensionMaintenanceAt = 0;
        resetUndoHistory();
        state.page = null;
        state.view = 'home';
        state.panel = null;
        ...
        try {
            ...
            state.inboxPage = 1;
            applyNotificationData({ notifications: data.notifications, counts: data.notification_counts,
                pagination: data.notification_pagination, unread_count: data.unread_count });
            ...
            applyPluginState(data.plugin_state);
            startNotificationPolling();
            scheduleExtensionMaintenance();
            const hashMatch = location.hash.match(/^#page-(\d+)$/);
            const settingsMatch = location.hash.match(/^#settings(?:-(general|mail|ai|translation|plugins))?$/);
            if (settingsMatch) {
    ...
    function unreadCount() { return Math.max(0, Number(state.notificationCounts.unread) || 0); }
    ...
    function revealPageInSidebar(id) {
        const pageId = Number(id);
        pagePath(pageId).forEach(page => {
            if (Number(page.id) !== pageId) state.expanded.add(Number(page.id));
        });
        state.localSearchQuery = '';
        state.localSearchTag = '';
        state.sidebarCollapsed = false;
        state.sidebarRevealPageId = pageId;
    }

    function updateSidebarTree() {
        rememberSidebarScroll();
        updateLocalSearch(state.localSearchQuery, state.localSearchTag);
        const sidebarBody = root.querySelector('.sidebar-body');
        if (sidebarBody) scheduleSidebarScroll(sidebarBody, state.sidebarScrollTop);
    }

    function setAllTreeExpanded(expanded) {
        state.expanded.clear();
        if (expanded) {
            const visibleIds = new Set(state.pages.map(page => Number(page.id)));
            state.pages.forEach(page => {
                if (page.parent_id && visibleIds.has(Number(page.parent_id))) state.expanded.add(Number(page.parent_id));
            });
        }
        state.localSearchQuery = '';
        state.localSearchTag = '';
        state.sidebarRevealPageId = expanded && state.view === 'page' ? Number(state.page?.id || 0) : 0;
        updateSidebarTree();
    }

    function toggleTreePage(id) {
        const pageId = Number(id);
        state.expanded.has(pageId) ? state.expanded.delete(pageId) : state.expanded.add(pageId);
        state.sidebarRevealPageId = 0;
        updateSidebarTree();
    }

    function treePageTooltip(page) {
        const status = state.page?.id === page.id ? state.page.status ?? page.status : page.status;
        return ['published', 'review', 'draft', 'private', 'archived'].includes(status)
            ? `（${t(`page.status.${status}`)}）${page.title}`
            : page.title;
    }

    function treePageStatusMarkup(page) {
        if (!['admin', 'content_admin', 'editor', 'author'].includes(state.user?.role)) return '';
        const status = state.page?.id === page.id ? state.page.status ?? page.status : page.status;
        const mark = { review: '♦', draft: '■', private: '✖' }[status];
        if (typeof mark !== 'string') return '';
        const label = esc(t(`page.status.${status}`));
        return `<span class="tree-page-status is-${status}" role="img" aria-label="${label}" title="${esc(treePageTooltip(page))}">${mark}</span>`;
    }
    ...
    function renderTree(parentId = null, level = 0) {
        return childrenOf(parentId).map(page => {
            ...
            return `<li class="tree-node" data-tree-node-id="${page.id}">
                <div class="tree-row ${state.page?.id === page.id ? 'active' : ''} ${movable ? 'movable' : ''}" data-action="open-page" data-id="${page.id}" data-tree-page-id="${page.id}" ${state.page?.id === page.id ? 'aria-current="page"' : ''} draggable="${movable ? 'true' : 'false'}" style="padding-left:${Math.min(level * 14, 56)}px">
                    <button type="button" class="tree-toggle ${children.length ? '' : 'placeholder'} ${open ? 'open' : ''}" data-action="toggle-tree" data-id="${page.id}" aria-label="${esc(t('common.openClose'))}" ${children.length ? `aria-expanded="${open}"` : 'tabindex="-1" aria-hidden="true"'}>${svg('chevron', 'tiny')}</button>
                    <span class="tree-icon">${esc(page.icon)}</span>
                    ${treePageStatusMarkup(page)}
                    <span class="tree-label" title="${esc(treePageTooltip(page))}">${esc(page.title)}</span>
    ...
    function renderSidebar() {
        ...
        return `<aside class="sidebar ${state.sidebarCollapsed ? 'collapsed' : ''} ${state.mobileSidebar ? 'mobile-open' : ''}">
            <button type="button" class="workspace-switcher" data-action="workspace-menu" aria-haspopup="menu" aria-expanded="${state.context?.kind === 'workspace' ? 'true' : 'false'}">
                ...
                <div class="workspace-meta"><div class="workspace-name">${esc(state.workspaceName)}</div><div class="workspace-plan">${esc(t('workspace.brand'))}</div></div>
                ${svg('chevron', 'tiny')}
            </button>
            <div class="sidebar-body">
                ...
                <ul class="side-list">
                    ...
                    <li id="sidebarMoreItems" ${state.sidebarMoreOpen ? '' : 'hidden'}><ul class="side-list">
                        ...
                        ${aiSearchPluginItems}
                        <li><a class="side-item" href="history.php">${svg('search')}<span class="label">${esc(t('knowledge.title'))}</span></a></li>
                    </ul></li>
                    <li><button type="button" class="side-item sidebar-more-toggle" data-action="toggle-sidebar-more" aria-expanded="${state.sidebarMoreOpen ? 'true' : 'false'}" aria-controls="sidebarMoreItems" aria-label="${esc(t(state.sidebarMoreOpen ? 'sidebar.hideMore' : 'sidebar.showMore'))}" title="${esc(t(state.sidebarMoreOpen ? 'sidebar.hideMore' : 'sidebar.showMore'))}">${svg('moreCircle')}</button></li>
                </ul>
                ${pluginItems ? `<ul class="side-list plugin-side-list">${pluginItems}</ul>` : ''}
                ${favorites.length ? `<section class="sidebar-section"><div class="sidebar-title"><span>${esc(t('common.favorites'))}</span></div><ul class="page-tree">${favorites.map(page => `<li><div class="tree-row ${state.page?.id === page.id ? 'active' : ''}" data-action="open-page" data-id="${page.id}"><span class="tree-toggle placeholder"></span><span class="tree-icon">${esc(page.icon)}</span>${treePageStatusMarkup(page)}<span class="tree-label" title="${esc(treePageTooltip(page))}">${esc(page.title)}</span></div></li>`).join('')}</ul></section>` : ''}
                <section class="sidebar-section">
                    ...
                    <div class="tree-controls"><button type="button" data-action="expand-all-tree">${esc(t('sidebar.expandAll'))}</button><button type="button" data-action="collapse-all-tree">${esc(t('sidebar.collapseAll'))}</button></div>
    ...
    function renderTopbar() {
        ...
        return `<header class="topbar">
            ...
            <div class="topbar-actions">
                ${state.page ? `<div class="save-state ${state.saving ? 'saving' : ''}" title="${esc(state.saveErrorMessage)}" aria-live="polite"><span class="saved-dot"></span><span>${esc(state.saving ? t('common.saving') : state.dirty ? t('common.unsaved') : (state.lastSaved ? t('common.saved') : t('common.latest')))}</span></div>
    ...
    function renderPageSaveError() {
        if (!state.page || !state.dirty || !state.saveErrorMessage) return '';
        return `<aside class="page-save-error" role="alert"><div><strong>${esc(state.saveErrorMessage)}</strong><p>${esc(t('page.saveRecoveryHelp'))}</p></div><div class="page-save-error-actions">${state.capabilities.can_manage_system_settings ? `<a class="btn secondary compact" href="#settings" target="_blank" rel="noopener">${esc(t('page.settingsInNewTab'))}</a>` : ''}<button type="button" class="btn primary compact" data-action="retry-page-save" ${state.saving ? 'disabled' : ''}>${esc(t('page.retrySave'))}</button></div></aside>`;
    }

    function syncPageSaveError() {
        const host = document.getElementById('pageSaveError');
        if (host) host.innerHTML = renderPageSaveError();
    }

    async function retryPageSave() {
        if (!state.page || !state.dirty || state.saving) return;
        const page = state.page;
        const previousTransport = { enabled: state.wafCompatibilityEnabled, key: state.wafCompatibilityKey, csrf: state.csrf };
        try {
            // Reload the site setting without discarding the unsaved editor.
            const session = await api('session');
            if (!session.user || Number(session.user.id) !== Number(state.user?.id)) {
                state.wafCompatibilityEnabled = previousTransport.enabled;
                state.wafCompatibilityKey = previousTransport.key;
                synchronizeCsrf(previousTransport.csrf);
                state.saveSessionChanged = true;
                throw encryptionFailure('save_session_changed', 'page.saveSessionChanged');
            }
            state.saveSessionChanged = false;
            if (state.page !== page || state.saving) return;
            await savePage();
        } catch (error) {
    ...
    function scrollSidebarToCurrentPage(sidebarBody) {
        const pageId = Number(state.sidebarRevealPageId || 0);
        if (!pageId || state.view !== 'page' || Number(state.page?.id) !== pageId) return;
        const row = sidebarBody.querySelector(`.page-tree-main [data-tree-page-id="${pageId}"]`);
        if (!row) return;
        const rowRect = row.getBoundingClientRect();
        if (!rowRect.height || !sidebarBody.clientHeight) return;
        const viewportTop = sidebarBody.getBoundingClientRect().top + sidebarBody.clientTop;
        const viewportBottom = viewportTop + sidebarBody.clientHeight;
        const margin = Math.min(8, Math.max(0, (sidebarBody.clientHeight - rowRect.height) / 2));
        if (rowRect.top < viewportTop + margin) sidebarBody.scrollTop += rowRect.top - viewportTop - margin;
        else if (rowRect.bottom > viewportBottom - margin) sidebarBody.scrollTop += rowRect.bottom - viewportBottom + margin;
        state.sidebarScrollTop = sidebarBody.scrollTop;
        state.sidebarRevealPageId = 0;
    }

    function scheduleSidebarScroll(sidebarBody, scrollTop) {
        if (state.sidebarScrollFrame) cancelAnimationFrame(state.sidebarScrollFrame);
        sidebarBody.scrollTop = Math.max(0, Number(scrollTop) || 0);
        state.sidebarScrollFrame = requestAnimationFrame(() => {
            state.sidebarScrollFrame = 0;
            if (!sidebarBody.isConnected) return;
            sidebarBody.scrollTop = Math.max(0, Number(scrollTop) || 0);
            scrollSidebarToCurrentPage(sidebarBody);
        });
    }
    ...
    function restoreSidebarScroll() {
        ...
        scheduleSidebarScroll(sidebarBody, state.sidebarScrollTop);
    ...
    function renderApp() {
        ...
        root.innerHTML = `${renderSidebar()}<div class="mobile-overlay ${state.mobileSidebar ? 'visible' : ''}" data-action="close-mobile-sidebar"></div><main class="main-shell"><section class="main-view">${renderTopbar()}<div id="pageSaveError">${renderPageSaveError()}</div>${renderAffectedPublicationsBanner()}${view}</section>${state.panel ? renderPanel() : ''}</main>${aiLauncher}${renderFloating()}${renderDialog()}`;
        restoreSidebarScroll();
        afterRender();
    }
    ...
    function renderFileRow(file) {
        const extensionStates = (Array.isArray(file.extension_states) ? file.extension_states : []).map(item => {
            const label = ['queued', 'processing', 'review', 'registered', 'unregistered'].includes(item.state)
                ? window.OpenConceptI18n.t('plugin-api', `state.${item.state}`) : (item.label || item.state);
            return `<span class="extension-file-state">${esc(label)}</span>`;
        }).join('');
        const extensionActions = (Array.isArray(file.extension_actions) ? file.extension_actions : []).map((item, index) => `<button class="btn secondary compact" data-action="extension-file-action" data-id="${file.id}" data-index="${index}">${esc(item.label)}</button>`).join('');
        const replace = file.can_replace ? `<button class="btn secondary compact" data-action="extension-replace-file" data-id="${file.id}">${esc(window.OpenConceptI18n.t('plugin-api', 'replace'))}</button>` : '';
        return `<article class="file-library-row">${fileKindIcon(file.category)}<a class="file-library-link" href="api.php?action=file-content&id=${file.id}" target="_blank" rel="noopener"><strong>${esc(file.original_name)}</strong><span>${formatBytes(file.size_bytes)} · ${esc(file.uploader_name)} · ${timeAgo(file.created_at)}${file.page_title ? ` · ${esc(t('files.uploadSource', { page: file.page_title }))}` : ''}</span>${extensionStates}</a>${renderFileUsage(file)}<div class="file-library-actions">${extensionActions}${replace}${svg('external', 'small')}${file.can_delete ? `<button class="file-delete-button" data-action="delete-file" data-id="${file.id}" data-title="${esc(file.original_name)}" aria-label="${esc(t('files.deleteFile', { name: file.original_name }))}" title="${esc(t('common.delete'))}">${svg('trash', 'small')}</button>` : ''}</div></article>`;
    }

    function chooseFileReplacement(fileId) {
        const file = state.files.find(item => Number(item.id) === fileId);
        if (!file?.can_replace || !file.source_reference) return;
        const input = document.createElement('input');
        input.type = 'file'; input.hidden = true;
        input.setAttribute('aria-label', window.OpenConceptI18n.t('plugin-api', 'replace'));
        document.body.append(input);
        input.addEventListener('cancel', () => input.remove(), { once: true });
        input.addEventListener('change', async () => {
            const replacement = input.files?.[0]; input.remove();
            if (!replacement) return;
            const body = new FormData(); body.append('file', replacement); body.append('source', JSON.stringify(file.source_reference));
            try {
                await api('extension-file-replace', { method: 'POST', body });
                await refreshSummary(); renderApp();
                toast(window.OpenConceptI18n.t('plugin-api', 'replaced'));
                extensionMaintenanceAt = 0; scheduleExtensionMaintenance();
    ...
    function renderInboxItems() {
        if (state.inboxLoading) return `<div class="history-empty" role="status">${esc(t('inbox.loading'))}</div>`;
        const notifications = state.notifications;
        if (!notifications.length) {
            return `<div class="history-empty">${esc(t(state.inboxFilter === 'archived' ? 'inbox.noArchived' : state.inboxFilter === 'unread' ? 'inbox.noUnread' : 'inbox.noNotifications'))}</div>`;
        }
        const disabled = state.inboxBusy || state.inboxOpeningId ? ' disabled' : '';
        return notifications.map(item => {
            const isRead = Boolean(Number(item.is_read));
            const isArchived = Boolean(Number(item.is_archived));
            return `<article class="notification-item ${isRead ? '' : 'unread'}">
                <button type="button" class="notification-main" data-action="notification" data-id="${Number(item.id)}"${disabled}>
                ...
                <div class="notification-actions">
                    ${isArchived ? `<button type="button" class="notification-manage-button is-delete" data-action="delete-notification" data-id="${Number(item.id)}" aria-label="${esc(t('inbox.delete'))}"${disabled}>${esc(t('inbox.delete'))}</button>` : `
                    <button type="button" class="notification-read-toggle" data-action="toggle-notification-read" data-id="${Number(item.id)}" data-read="${isRead ? '0' : '1'}" title="${esc(t(isRead ? 'inbox.markUnread' : 'inbox.markRead'))}" aria-label="${esc(t(isRead ? 'inbox.markUnread' : 'inbox.markRead'))}"${disabled}>${isRead ? svg('bell', 'small') : svg('check', 'small')}</button>
                    ${isRead ? `<button type="button" class="notification-manage-button" data-action="archive-notification" data-id="${Number(item.id)}" aria-label="${esc(t('inbox.archive'))}"${disabled}>${esc(t('inbox.archive'))}</button>` : ''}`}
                </div>
            </article>`;
        }).join('');
    }
    ...
    function renderInboxBody() {
        const disabled = state.inboxBusy || state.inboxOpeningId ? ' disabled' : '';
        const tabs = ['all', 'unread', 'archived'].map(filter => `<button type="button" class="inbox-tab ${state.inboxFilter === filter ? 'active' : ''}" data-action="inbox-filter" data-filter="${filter}" role="tab" aria-selected="${state.inboxFilter === filter}"${disabled}>${esc(t(`inbox.${filter}`))} <span>${Number(state.notificationCounts[filter]) || 0}</span></button>`).join('');
        const pagination = state.inboxPagination;
        const pageDisabled = Boolean(state.inboxLoading || disabled);
        const pages = Number(pagination.total_pages) || 1;
        return `<div class="inbox-tabs" role="tablist" aria-label="${esc(t('inbox.filterAria'))}">${tabs}</div>
            ${state.inboxError ? `<p class="inbox-error" role="alert">${esc(t('inbox.loadFailed'))} ${esc(state.inboxError)}</p>` : ''}
            ${state.inboxBusy ? `<p class="inbox-progress" role="status">${esc(t('inbox.updating'))}</p>` : ''}
            <div class="inbox-list-container" aria-busy="${state.inboxLoading ? 'true' : 'false'}">${renderInboxItems()}</div>
            ${pages > 1 ? `<nav class="inbox-pagination" aria-label="${esc(t('inbox.pagination'))}"><button type="button" class="btn secondary compact" data-action="inbox-page" data-page="${state.inboxPage - 1}" ${pageDisabled || state.inboxPage <= 1 ? 'disabled' : ''}>${esc(t('inbox.previous'))}</button><span>${esc(t('inbox.pageSummary', { page: state.inboxPage, pages, total: Number(pagination.total) || 0 }))}</span><button type="button" class="btn secondary compact" data-action="inbox-page" data-page="${state.inboxPage + 1}" ${pageDisabled || state.inboxPage >= pages ? 'disabled' : ''}>${esc(t('inbox.next'))}</button></nav>` : ''}`;
    }
        ...
        return `<section class="ai-provider-settings" id="aiProviderSettingsCard">
            ...
            <details class="ai-provider-details" id="aiProviderDetails"${state.aiProviderExpanded ? ' open' : ''}>
                ...
                <div class="ai-provider-grid">
                    ...
                    <div class="form-field ai-provider-wide ai-provider-common-key"><label for="aiProviderApiKey"><strong>${esc(t('settings.aiProvider.commonApiKey'))}</strong></label><small id="aiProviderCommonKeyHelp">${esc(t('settings.aiProvider.commonApiKeyHelp'))}</small><input class="input" id="aiProviderApiKey" type="password" maxlength="4096" autocomplete="new-password" aria-describedby="aiProviderCommonKeyHelp" value="${esc(noAuth ? '' : (state.aiProviderDraft?.api_key || ''))}" placeholder="${esc(t('settings.aiProvider.apiKeyPlaceholder'))}" ${noAuth ? 'disabled' : ''}><small>${esc(t('settings.aiProvider.apiKeyHelp'))}</small><label class="ai-provider-key-option"><input type="checkbox" id="aiProviderClearKey" ${state.aiProviderDraft?.clear_api_key ? 'checked' : ''}> ${esc(t('settings.aiProvider.clearKey'))}</label><small>${esc(t('settings.aiProvider.clearKeyHelp'))}</small></div>
    ...
    function readTranslationProviderForm() {
        ...
        return {
            ...
            use_shared_api_key: document.getElementById('translationProviderUseSharedKey')?.checked
                ?? (current.use_shared_api_key ?? Boolean(state.translationProviderSettings?.inherited)),
            api_key: translationProviderFormValue('translationProviderApiKey', current.api_key || ''),
    ...
    function renderTranslationProviderSettings() {
        ...
        const savedSettings = stateValue.settings || {};
        const settings = state.translationProviderDraft || savedSettings;
        const savedSharedKey = savedSettings.use_shared_api_key ?? Boolean(stateValue.inherited);
        const useSharedKey = settings.use_shared_api_key ?? Boolean(stateValue.inherited);
        const individualSecret = stateValue.individual_secret || (!savedSharedKey ? stateValue.secret : {}) || {};
        const individualKeyAvailable = Boolean(individualSecret.available)
            && String(settings.base_url || '').trim().replace(/\/+$/, '') === String(savedSettings.base_url || '').trim().replace(/\/+$/, '')
            && settings.auth_mode === savedSettings.auth_mode;
        const secret = useSharedKey
            ? (state.aiProviderSettings?.secret || (savedSharedKey ? stateValue.secret : {}) || {})
            : { ...individualSecret, available: individualKeyAvailable };
        const isOpenAi = settings.provider_id === 'openai';
        const noAuth = settings.auth_mode === 'none';
        const busy = state.translationProviderSaving || state.translationProviderTesting;
        const sourceLabel = t(`settings.aiProvider.secretSource.${useSharedKey ? 'shared' : (secret.source || 'none')}`);
        const credentialReady = noAuth || secret.available;
        const secretStatus = noAuth
            ? t('settings.aiProvider.authNotRequired')
            : secret.available
        ...
        return `<section class="ai-provider-settings">
            ...
            <form id="translationProviderForm">
                <div class="ai-provider-grid">
                    ...
                    <div class="form-field ai-provider-wide ai-provider-key-settings"><label class="ai-provider-key-option"><input type="checkbox" id="translationProviderUseSharedKey" ${useSharedKey ? 'checked' : ''}> ${esc(t('settings.translationProvider.useSharedApiKey'))}</label><small>${esc(t('settings.translationProvider.useSharedApiKeyHelp'))}</small><label for="translationProviderApiKey">${esc(t('settings.translationProvider.individualApiKey'))}</label><input class="input" id="translationProviderApiKey" type="password" maxlength="4096" autocomplete="new-password" value="${esc(noAuth ? '' : (state.translationProviderDraft?.api_key || ''))}" placeholder="${esc(t('settings.aiProvider.apiKeyPlaceholder'))}" ${noAuth || useSharedKey ? 'disabled' : ''}><small>${esc(t('settings.aiProvider.apiKeyHelp'))}</small>${!useSharedKey && individualSecret.configured ? `<small class="ai-provider-key-notice" role="status">${esc(t(individualKeyAvailable ? 'settings.translationProvider.individualKeyAvailable' : 'settings.translationProvider.individualKeyRetained'))}</small>` : ''}<label class="ai-provider-key-option"><input type="checkbox" id="translationProviderClearKey" ${state.translationProviderDraft?.clear_api_key ? 'checked' : ''}> ${esc(t('settings.aiProvider.clearKey'))}</label><small>${esc(t('settings.translationProvider.clearKeyHelp'))}</small></div>
    ...
    async function saveTranslationProviderSettings() {
        ...
        const body = { ...draft, api_key: draft.use_shared_api_key || draft.auth_mode === 'none' ? '' : draft.api_key };
    ...
    function pluginHasError(plugin) {
        return plugin?.compatibility?.compatible === false || Boolean(plugin?.error);
    }

    function pluginCompatibilityMessage(plugin) {
        const code = plugin.error?.code || plugin.compatibility?.code || '';
        const keys = {
            unsupported_application: 'unsupportedApplication', unknown_plugin: 'unknownPlugin',
            unsupported_plugin_version: 'unsupportedVersion', plugin_api_unavailable: 'apiUnavailable',
            plugin_api_major_mismatch: 'apiVersion', plugin_api_version_too_old: 'apiVersion',
            plugin_api_missing_capabilities: 'missingCapabilities', invalid_manifest: 'invalidManifest', boot_failed: 'bootFailed',
        };
        return t(`plugins.compatibility.${keys[code] || 'unavailable'}`);
    }

    function pluginDetachBlockMessage(plugin) {
        const code = plugin.detach_block_reason;
        if (['current_database_adapter', 'database_adapter_in_use'].includes(code)) return t('plugins.detachDatabaseBlocked');
        if (code === 'plugin_not_in_error') return t('plugins.detachHealthyBlocked');
        return t('plugins.detachBlocked');
    }

    function pluginUpdateCandidate(plugin) {
        const update = plugin?.update;
        // The server supplies only newer releases and applies its version/API policy.
        return update && update.compatibility?.compatible === true
            && typeof update.version === 'string' && update.version !== '' && update.version !== String(plugin.version ?? '')
            && /^[a-f0-9]{64}$/i.test(String(update.sha256 || '')) ? update : null;
    }

    function pluginUpdateBlockMessage(plugin) {
        if (plugin.update_block_reason === 'database_adapter_in_use') {
            return t('plugins.updateDatabaseBlocked');
        }
        if (plugin.update_block_reason === 'unsafe_plugin_path') return t('plugins.updateUnsafePath');
        return t('plugins.updateBlocked');
    }

    function pluginCompatibilityMarkup(plugin) {
        if (!pluginHasError(plugin)) return '';
        const compatibility = plugin.compatibility || {};
        const minimumVersion = compatibility.minimum_supported_version
            || (Array.isArray(compatibility.supported_versions) ? compatibility.supported_versions[0] : compatibility.supported_versions)
            || '';
        const details = [
            [t('plugins.compatibility.pluginVersion'), String(compatibility.plugin_version ?? plugin.version ?? '') || t('plugins.compatibility.unknownVersion')],
            [t('plugins.compatibility.supportedVersions'), minimumVersion || t('plugins.compatibility.noSupportedVersion')],
            [t('plugins.compatibility.appVersion'), compatibility.app_version || t('plugins.compatibility.unknownVersion')],
        ];
        if (compatibility.required_api) details.push([t('plugins.compatibility.requiredApi'), compatibility.required_api]);
        if (compatibility.available_api) details.push([t('plugins.compatibility.availableApi'), compatibility.available_api]);
        if (Array.isArray(compatibility.missing_capabilities) && compatibility.missing_capabilities.length) {
            details.push([t('plugins.compatibility.requiredCapabilities'), compatibility.missing_capabilities.join(', ')]);
        }
        return `<div class="plugin-compatibility-error" role="note"><strong>${esc(t('plugins.compatibility.errorTitle'))}</strong><p>${esc(pluginCompatibilityMessage(plugin))}</p><dl>${details.map(([label, value]) => `<div><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`).join('')}</dl></div>`;
    }
    ...
    function renderPluginSettings() {
        ...
        const installed = state.installedPlugins.map(plugin => {
            ...
            const busy = Boolean(state.pluginOperation);
            const blocked = pluginHasError(plugin);
            const update = pluginUpdateCandidate(plugin);
            const updating = state.pluginOperation === plugin.id && state.dialog === 'plugin-update-confirmation';
            const effectiveEnabled = Boolean(plugin.enabled) && !blocked;
            ...
            const baseStatusLabel = retiredAfterMigration
                ...
                : (effectiveEnabled ? t('plugins.enabled') : t('plugins.disabled'));
            ...
            const control = manualConnection
                ...
                : blocked
                ? `<button type="button" class="toggle plugin-toggle" data-action="toggle-plugin" data-plugin-id="${esc(plugin.id)}" data-enabled="true" aria-label="${esc(t('plugins.enable', { name: pluginName }))}" aria-pressed="false" disabled></button>`
            ...
            const detach = blocked ? `<button type="button" class="btn secondary compact plugin-detach" data-action="detach-plugin" data-plugin-id="${esc(plugin.id)}" ${busy || plugin.can_detach !== true ? 'disabled' : ''}>${esc(t(state.pluginOperation === plugin.id && !updating ? 'plugins.detaching' : 'plugins.detach'))}</button>` : '';
            const updateControl = update ? `<button type="button" class="btn primary compact plugin-update" data-action="update-plugin" data-plugin-id="${esc(plugin.id)}" ${busy || plugin.update_block_reason ? 'disabled' : ''}>${esc(t(updating ? 'plugins.updating' : 'plugins.update'))}</button>` : '';
            const updateSummary = update ? `<div class="plugin-update-summary"><strong>${esc(t('plugins.updateAvailable'))}</strong><span>${esc(plugin.version || t('plugins.compatibility.unknownVersion'))} → ${esc(update.version)}</span>${plugin.update_block_reason ? `<p>${esc(pluginUpdateBlockMessage(plugin))}</p>` : ''}</div>` : '';
            return `<article class="plugin-card${blocked ? ' has-error' : ''}" data-plugin-card-id="${esc(plugin.id)}">
                <span class="plugin-card-icon" aria-hidden="true">${esc(icon)}</span>
                <div class="plugin-card-copy"><strong>${esc(pluginName)}</strong><span>${esc(pluginDescription)}</span><small>version ${esc(plugin.version)} · ${esc(blocked ? t('plugins.disabled') : statusLabel)}</small>${pluginCompatibilityMarkup(plugin)}${updateSummary}${blocked && plugin.can_detach !== true ? `<p class="plugin-detach-blocked">${esc(pluginDetachBlockMessage(plugin))}</p>` : ''}${plugin.operation_error ? `<p class="plugin-operation-error" role="alert">${esc(plugin.operation_error)}</p>` : ''}</div>
                <div class="plugin-card-actions">${control}${updateControl}${detach}</div>
            </article>`;
        }).join('');

        const available = state.availablePlugins.map(plugin => {
            const busy = Boolean(state.pluginOperation);
            return `<article class="plugin-card available${pluginHasError(plugin) ? ' has-error' : ''}" data-plugin-card-id="${esc(plugin.id)}">
                <span class="plugin-card-icon" aria-hidden="true">${esc(plugin.icon || '🧩')}</span>
                <div class="plugin-card-copy"><strong>${esc(plugin.name)}</strong><span>${esc(plugin.description || t('plugins.noDescription'))}</span><small>version ${esc(plugin.version)}</small>${pluginCompatibilityMarkup(plugin)}</div>
                <button type="button" class="btn secondary compact plugin-download" data-action="download-plugin" data-plugin-id="${esc(plugin.id)}" ${busy ? 'disabled' : ''}>${svg('download', 'small')} ${esc(state.pluginOperation === plugin.id ? t('plugins.downloading') : t('plugins.download'))}</button>
            </article>`;
        }).join('');
        ...
        if (state.pluginsLoading) {
            ...
            catalogStatus = `<div class="plugin-status">${esc(t('plugins.catalogNotConfigured'))}</div>`;
        ...
        const runtimeStatus = runtime
            ? `<div class="plugin-status"><strong>${esc(t('plugins.runtime'))}</strong>: ${esc(runtime.deployment_profile)} · ${esc(runtime.runtime)} · ${esc(runtime.os_family)} ${esc(runtime.architecture)} · PHP ${esc(runtime.php_version)} · DB ${esc(runtime.canonical_backend || t('plugins.runtimeUnknown'))}</div>`
            : '';

        return `<section class="plugin-settings">
            <div class="plugin-settings-heading"><div><strong>${esc(t('plugins.title'))}</strong><span>${esc(t('plugins.help'))}</span></div><button type="button" class="btn secondary compact" data-action="refresh-plugins" ${state.pluginsLoading || state.pluginOperation ? 'disabled' : ''}>${esc(t('plugins.refresh'))}</button></div>
            <div class="plugin-status"><a href="https://github.com/BloomingBlooming/openconcept_official/tree/main/plugins" target="_blank" rel="noopener noreferrer">${esc(t('plugins.catalogSource'))}</a></div>
    ...
    function settingsTabDescription(tab) {
        const key = ['general', 'mail', 'ai', 'translation', 'plugins'].includes(tab) ? tab : 'general';
        return t(`settings.tabs.${key}Help`);
    }

    function renderWafCompatibilitySettings() {
        if (!state.capabilities.can_manage_system_settings) return '';
        const busy = state.wafCompatibilityLoading || state.wafCompatibilitySaving;
        const enabled = state.wafCompatibilityDraft ?? state.wafCompatibilityEnabled;
        return `<section class="organization-settings waf-compatibility-settings"><div class="organization-settings-copy"><strong>${esc(t('settings.waf.title'))}</strong><span id="wafCompatibilityHelp">${esc(t('settings.waf.help'))}</span><span>${esc(t('settings.waf.inspectionNotice'))}</span>${state.wafCompatibilityError ? `<span class="waf-settings-error" role="alert">${esc(state.wafCompatibilityError)}</span>` : ''}</div><form id="wafCompatibilityForm" class="organization-settings-form"><label class="waf-compatibility-option"><input type="checkbox" id="wafCompatibilityEnabled" aria-describedby="wafCompatibilityHelp" ${enabled ? 'checked' : ''} ${busy ? 'disabled' : ''}><span>${esc(t('settings.waf.title'))}</span></label><button class="btn primary compact" type="submit" ${busy ? 'disabled' : ''}>${esc(t(state.wafCompatibilitySaving ? 'common.saving' : 'common.save'))}</button></form></section>`;
    }

    async function loadWafCompatibilitySettings() {
        if (!state.capabilities.can_manage_system_settings || state.wafCompatibilityLoading) return;
        state.wafCompatibilityLoading = true;
        state.wafCompatibilityError = '';
        try {
            await api('waf-compatibility-settings');
        } catch (error) {
            state.wafCompatibilityError = error.message;
        } finally {
            state.wafCompatibilityLoading = false;
        }
    }

    async function saveWafCompatibilitySettings() {
        if (!state.capabilities.can_manage_system_settings || state.wafCompatibilitySaving) return;
        const enabled = Boolean(document.getElementById('wafCompatibilityEnabled')?.checked);
        state.wafCompatibilityDraft = enabled;
        state.wafCompatibilitySaving = true;
        state.wafCompatibilityError = '';
        renderApp();
        try {
            if (enabled && !globalThis.crypto?.subtle) {
                throw encryptionFailure('waf_crypto_unavailable', 'common.wafCryptoUnavailable');
            }
            await api('waf-compatibility-settings', {
                method: 'POST', body: { waf_compatibility_enabled: enabled },
            });
            state.wafCompatibilityDraft = null;
            toast(t('settings.waf.saved'));
        } catch (error) {
            state.wafCompatibilityError = error.message;
        } finally {
            state.wafCompatibilitySaving = false;
            if (state.view === 'settings') renderApp();
        }
    }

    function renderInvitationLocaleField() {
        return `<div class="form-field"><label for="inviteLocale" lang="en" translate="no">${esc(t('settings.invitation.language'))}</label><select class="input" id="inviteLocale" name="ui_locale">${availableContentLanguages().map(locale => `<option value="${esc(locale.code)}" ${locale.code === state.i18n.locale ? 'selected' : ''}>${esc(locale.name)}</option>`).join('')}</select><small>${esc(t('settings.invitation.languageHelp'))}</small></div>`;
    }

    function renderMailSettings() {
        if (!state.capabilities.can_manage_system_settings) return '';
        const config = state.mailSettings?.settings || {};
        const field = (key, type = 'text', extra = '') => `<div class="form-field"><label for="mail-${key}">${esc(t(`mailUi.${key}`))}</label><input class="input" id="mail-${key}" name="${key}" type="${type}" value="${esc(config[key] ?? '')}" ${extra}></div>`;
        return `<section class="settings-section-card"><div class="settings-section-heading"><div><strong>${esc(t('mailUi.title'))}</strong><span>${esc(t('mailUi.help'))}</span></div></div>
            ${state.mailSettings?.effective_transport === 'log' && config.transport !== 'log' ? `<p class="plugin-status" role="status">${esc(t('mailUi.transport'))}: ${esc(t('mailUi.transport.log'))}</p>` : ''}
            <form id="mailSettingsForm" class="mail-settings-form">
                <div class="form-field"><label for="mail-transport">${esc(t('mailUi.transport'))}</label><select class="input" id="mail-transport" name="transport">${['smtp', 'mail', 'log'].map(value => `<option value="${value}" ${config.transport === value ? 'selected' : ''}>${esc(t(`mailUi.transport.${value}`))}</option>`).join('')}</select></div>
                ${field('from_email', 'email')}${field('from_name')}${field('reply_to', 'email')}
                ${field('smtp_host')}${field('smtp_port', 'number', 'min="1" max="65535"')}
                <div class="form-field"><label for="mail-smtp_encryption">${esc(t('mailUi.smtp_encryption'))}</label><select class="input" id="mail-smtp_encryption" name="smtp_encryption">${['ssl', 'tls', 'none'].map(value => `<option value="${value}" ${config.smtp_encryption === value ? 'selected' : ''}>${esc(t(`mailUi.encryption.${value}`))}</option>`).join('')}</select></div>
                ${field('smtp_username', 'text', 'autocomplete="off"')}
                <div class="form-field"><label for="mail-smtp_password">${esc(t('mailUi.smtp_password'))}</label><input class="input" id="mail-smtp_password" name="smtp_password" type="password" autocomplete="new-password" value=""><small>${esc(t(state.mailSettings?.password_configured ? 'mailUi.passwordStored' : 'mailUi.passwordEmpty'))}</small></div>
                <label><input type="checkbox" name="clear_password"> ${esc(t('mailUi.clearPassword'))}</label>
                ${field('smtp_timeout', 'number', 'min="3" max="60"')}
                <p>${esc(t('mailUi.testHelp', { email: state.user?.email || '' }))}</p>
                <div id="mailSettingsStatus" role="status">${esc(state.mailSettingsError || '')}</div>
                <div class="mail-settings-actions"><button class="btn primary" type="submit" ${!state.mailSettings ? 'disabled' : ''}>${esc(t('common.save'))}</button><button class="btn secondary" type="button" data-action="test-mail" ${!state.mailSettings ? 'disabled' : ''}>${esc(t('mailUi.test'))}</button></div>
            </form></section>`;
    }
    ...
    async function loadMailSettings() {
        ...
        const sameSession = () => session === notificationSession && actorId === Number(state.user?.id)
            && state.capabilities.can_manage_system_settings;
        try {
            const settings = await api('mail-settings');
            if (!sameSession()) return;
            state.mailSettings = settings;
            state.mailSettingsError = '';
        } catch (error) {
            if (sameSession()) state.mailSettingsError = error.message;
        }
    }

    async function saveMailSettings() {
        const form = document.getElementById('mailSettingsForm');
        if (!form || !state.capabilities.can_manage_system_settings || state.mailSettingsBusy || !form.reportValidity()) return;
        const session = notificationSession;
        const actorId = Number(state.user?.id);
        const sameSession = () => session === notificationSession && actorId === Number(state.user?.id)
            && state.capabilities.can_manage_system_settings;
        const body = Object.fromEntries(new FormData(form));
        body.clear_password = Boolean(form.elements.clear_password.checked);
        body.smtp_port = Number(body.smtp_port);
        body.smtp_timeout = Number(body.smtp_timeout);
        const status = document.getElementById('mailSettingsStatus');
        state.mailSettingsBusy = true;
        form.querySelectorAll('button').forEach(button => { button.disabled = true; });
        status.textContent = t('common.saving');
        try {
            const settings = await api('mail-settings', { method: 'POST', body });
            form.elements.smtp_password.value = '';
            if (!sameSession()) return;
            state.mailSettings = settings;
            state.mailSettingsError = '';
            if (state.view === 'settings') renderApp();
            toast(t('mailUi.saved'));
        } catch (error) {
            if (sameSession()) status.textContent = error.message;
        } finally {
            if (sameSession()) state.mailSettingsBusy = false;
    ...
    async function testMailSettings() {
        const form = document.getElementById('mailSettingsForm');
        if (!form || !state.capabilities.can_manage_system_settings || state.mailSettingsBusy) return;
        const session = notificationSession;
        const actorId = Number(state.user?.id);
        const sameSession = () => session === notificationSession && actorId === Number(state.user?.id)
            && state.capabilities.can_manage_system_settings;
        ...
        state.mailSettingsBusy = true;
        form.querySelectorAll('button').forEach(button => { button.disabled = true; });
        status.textContent = t('settings.sending');
        try {
            const result = await api('mail-test', { method: 'POST', body: {} });
            if (!sameSession()) return;
            status.textContent = result.mail.sent
                ? t(result.mail.transport === 'log' ? 'mailUi.logged' : 'mailUi.testSent')
                : result.mail.error || t('settings.result.mailFailed');
        } catch (error) {
            if (sameSession()) status.textContent = error.message;
        } finally {
            if (sameSession()) state.mailSettingsBusy = false;
    ...
    function renderSettingsPage() {
        ...
        const localeSettings = `<section class="organization-settings"><div class="organization-settings-copy"><strong><label for="uiLocaleSelect" lang="en" translate="no">${esc(t('settings.uiLanguage'))}</label></strong><span id="uiLocaleHelp">${esc(t('settings.uiLanguageHelp'))}</span></div><select class="select" id="uiLocaleSelect" aria-describedby="uiLocaleHelp">${availableContentLanguages().map(locale => `<option value="${esc(locale.code)}" ${locale.code === state.i18n.locale ? 'selected' : ''}>${esc(locale.name)}</option>`).join('')}</select></section>`;
        const themeMode = window.OpenConceptTheme?.get() || 'light';
        const themeSettings = `<section class="organization-settings theme-settings"><div class="organization-settings-copy"><strong><label for="uiThemeSelect">${esc(t('settings.appearance.title'))}</label></strong><span id="uiThemeHelp">${esc(t('settings.appearance.help'))}</span></div><select class="select" id="uiThemeSelect" aria-describedby="uiThemeHelp"><option value="light" ${themeMode === 'light' ? 'selected' : ''}>${esc(t('settings.appearance.light'))}</option><option value="dark" ${themeMode === 'dark' ? 'selected' : ''}>${esc(t('settings.appearance.dark'))}</option></select></section>`;
        const availableTabs = state.capabilities.can_manage_system_settings ? ['general', 'mail', 'ai', 'translation', 'plugins'] : ['general'];
        ...
        const tabs = `<nav class="settings-tabs" role="tablist" aria-label="${esc(t('settings.tabs.label'))}">${tabButton('general', t('settings.tabs.general'))}${state.capabilities.can_manage_system_settings ? `${tabButton('mail', t('settings.tabs.mail'))}${tabButton('ai', t('settings.tabs.ai'))}${tabButton('translation', t('settings.tabs.translation'))}${tabButton('plugins', t('settings.tabs.plugins'))}` : ''}</nav>`;
        const panels = `<div class="settings-panels">
            <section class="settings-panel" id="settingsPanel-general" role="tabpanel" aria-labelledby="settingsTab-general" data-settings-panel="general" ${activeTab === 'general' ? '' : 'hidden'}><section class="settings-section-card"><div class="settings-section-heading"><div><strong>${esc(t('settings.general.title'))}</strong><span>${esc(t('settings.general.help'))}</span></div></div>${localeSettings}${organizationSettings}${renderWafCompatibilitySettings()}${themeSettings}</section></section>
            ${state.capabilities.can_manage_system_settings ? `<section class="settings-panel" id="settingsPanel-mail" role="tabpanel" aria-labelledby="settingsTab-mail" data-settings-panel="mail" ${activeTab === 'mail' ? '' : 'hidden'}>${renderMailSettings()}</section><section class="settings-panel" id="settingsPanel-ai" role="tabpanel" aria-labelledby="settingsTab-ai" data-settings-panel="ai" ${activeTab === 'ai' ? '' : 'hidden'}>${renderRagFlowOverview()}${renderAiProviderSettings()}${renderRagCoreSettings()}</section><section class="settings-panel" id="settingsPanel-translation" role="tabpanel" aria-labelledby="settingsTab-translation" data-settings-panel="translation" ${activeTab === 'translation' ? '' : 'hidden'}>${renderTranslationProviderSettings()}</section><section class="settings-panel" id="settingsPanel-plugins" role="tabpanel" aria-labelledby="settingsTab-plugins" data-settings-panel="plugins" ${activeTab === 'plugins' ? '' : 'hidden'}>${renderPluginSettings()}</section>` : ''}
        </div>`;
    ...
    function activateSettingsTab(tab) {
        const allowedTabs = state.capabilities.can_manage_system_settings
            ? ['general', 'mail', 'ai', 'translation', 'plugins']
            : ['general'];
    ...
    async function togglePlugin(pluginId, enabled) {
        ...
        if (!state.capabilities.can_manage_system_settings || !plugin || state.pluginOperation || state.pluginsLoading
            || state.dialog === 'plugin-enable-confirmation') return;
        if (enabled && pluginHasError(plugin)) {
            plugin.operation_error = pluginCompatibilityMessage(plugin);
            renderApp();
            toast(plugin.operation_error, 'error');
            return;
        }
        if (Boolean(plugin.enabled) === enabled) return;
        if (!enabled) { await applyPluginToggle(plugin, false); return; }

        const confirmation = {
            target: Object.freeze({ plugin_id: pluginId, version: String(plugin.version ?? ''), installation_token: String(plugin.installation_token ?? '') }),
            name: pluginT(plugin.id, 'plugin.name', {}, plugin.name || plugin.id),
            actorId: Number(state.user?.id), session: notificationSession, busy: false, error: '',
        };
        state.dialog = 'plugin-enable-confirmation';
        state.dialogData = confirmation;
        renderApp();
        window.setTimeout(() => {
            if (state.dialog === 'plugin-enable-confirmation' && state.dialogData === confirmation) {
                root.querySelector('.plugin-enable-dialog .dialog-footer [data-action="cancel-enable-plugin"]')?.focus();
            }
        }, 0);
    }

    function cancelPluginEnable() {
        if (state.dialog !== 'plugin-enable-confirmation') return false;
        if (state.dialogData?.busy) return true;
        state.dialog = null;
        state.dialogData = null;
        renderApp();
        return true;
    ...
    function renderPluginEnableDialog() {
        const confirmation = state.dialogData;
        if (!confirmation?.target) return '';
        const disabled = confirmation.busy ? 'disabled' : '';
        return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog plugin-enable-dialog" role="alertdialog" aria-modal="true" aria-labelledby="pluginEnableTitle" aria-describedby="pluginEnableWarning pluginEnableQuestion">
            <header class="dialog-header"><div class="dialog-title" id="pluginEnableTitle">${esc(t('plugins.enableTitle'))}</div><button type="button" class="btn icon-only" data-action="cancel-enable-plugin" aria-label="${esc(t('common.close'))}" ${disabled}>${svg('close', 'small')}</button></header>
            <div class="dialog-body"><p class="plugin-enable-name">${esc(confirmation.name)}</p><p class="dialog-intro plugin-enable-warning" id="pluginEnableWarning">${esc(t('plugins.enableWarning'))}</p><p id="pluginEnableQuestion">${esc(t('plugins.enableQuestion'))}</p>${confirmation.error ? `<p class="plugin-operation-error" role="alert">${esc(confirmation.error)}</p>` : ''}${confirmation.busy ? `<p role="status" aria-live="polite">${esc(t('plugins.enabling'))}</p>` : ''}</div>
            <footer class="dialog-footer"><button type="button" class="btn secondary" data-action="cancel-enable-plugin" ${disabled}>${esc(t('plugins.enableNo'))}</button><button type="button" class="btn primary" data-action="confirm-enable-plugin" ${disabled}>${esc(t('plugins.enableYes'))}</button></footer>
        </section></div>`;
    }

    async function confirmEnablePlugin() {
        if (state.dialog !== 'plugin-enable-confirmation') return;
        const confirmation = state.dialogData;
        if (!confirmation?.target || confirmation.busy || state.pluginOperation) return;
        if (!state.capabilities.can_manage_system_settings || confirmation.session !== notificationSession
            || confirmation.actorId !== Number(state.user?.id)) { cancelPluginEnable(); return; }
        const plugin = state.installedPlugins.find(item => item.id === confirmation.target.plugin_id);
        if (!plugin || plugin.enabled) { cancelPluginEnable(); return; }
        if (String(plugin.version ?? '') !== confirmation.target.version || String(plugin.installation_token ?? '') !== confirmation.target.installation_token) {
            confirmation.error = t('plugins.enableChanged');
            renderApp();
            return;
        }
        if (pluginHasError(plugin)) {
            confirmation.error = plugin.operation_error = pluginCompatibilityMessage(plugin);
            renderApp();
            return;
        }
        await applyPluginToggle(plugin, true, confirmation);
    }

    async function applyPluginToggle(plugin, enabled, confirmation = null) {
        const pluginId = plugin.id;
        const session = notificationSession;
        const actorId = Number(state.user?.id);
        const sameSession = () => session === notificationSession && actorId === Number(state.user?.id);
        state.pluginOperation = pluginId;
        if (confirmation) { confirmation.busy = true; confirmation.error = ''; }
        renderApp();
        try {
            ...
            if (confirmation && state.dialog === 'plugin-enable-confirmation' && state.dialogData === confirmation) {
                state.dialog = null;
                state.dialogData = null;
            }
            if (!enabled) {
                const plugins = { ...state.activePlugins };
                delete plugins[pluginId];
                applyPluginState({ plugins, plugin_ui: state.pluginUi.filter(item => item.id !== pluginId) });
            }
            try { pluginStateChannel?.postMessage({ changed: true }); }
            catch (_) { /* Other tabs also refresh when they become visible. */ }
            renderApp();
            toast(t('plugins.stateChanged', { name: plugin.name, state: enabled ? t('plugins.enabled') : t('plugins.disabled') }));
            window.setTimeout(() => { if (sameSession()) window.location.reload(); }, 350);
        } catch (err) {
            if (!sameSession()) return;
            if (confirmation) { confirmation.busy = false; confirmation.error = err.message || t('plugins.enableFailed'); }
            state.pluginOperation = '';
            renderApp();
            toast(err.message, 'error');
        }
    ...
    function updatePlugin(pluginId) {
        const plugin = state.installedPlugins.find(item => item.id === pluginId);
        const update = pluginUpdateCandidate(plugin);
        if (!state.capabilities.can_manage_system_settings || !plugin || !update || plugin.update_block_reason
            || state.pluginOperation || state.pluginsLoading) return;
        const confirmation = {
            target: Object.freeze({ plugin_id: pluginId, version: String(plugin.version ?? ''),
                installation_token: String(plugin.installation_token ?? ''), target_version: update.version, package_sha256: update.sha256 }),
            name: pluginT(plugin.id, 'plugin.name', {}, plugin.name || plugin.id),
            actorId: Number(state.user?.id), session: notificationSession, busy: false, error: '',
        };
        state.dialog = 'plugin-update-confirmation';
        state.dialogData = confirmation;
        renderApp();
        window.setTimeout(() => {
            if (state.dialog === 'plugin-update-confirmation' && state.dialogData === confirmation) {
                root.querySelector('[data-action="cancel-update-plugin"]')?.focus();
            }
        }, 0);
    }

    function cancelPluginUpdate() {
        if (state.dialog !== 'plugin-update-confirmation') return false;
        if (state.dialogData?.busy) return true;
        state.dialog = null;
        state.dialogData = null;
        renderApp();
    ...
    function renderPluginUpdateDialog() {
        const confirmation = state.dialogData;
        if (!confirmation?.target) return '';
        const disabled = confirmation.busy ? 'disabled' : '';
        const target = confirmation.target;
        return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog plugin-update-dialog" role="alertdialog" aria-modal="true" aria-labelledby="pluginUpdateTitle" aria-describedby="pluginUpdateDescription">
            <header class="dialog-header"><div class="dialog-title" id="pluginUpdateTitle">${esc(t('plugins.updateTitle'))}</div><button type="button" class="btn icon-only" data-action="cancel-update-plugin" aria-label="${esc(t('common.close'))}" ${disabled}>${svg('close', 'small')}</button></header>
            <div class="dialog-body"><p class="dialog-intro" id="pluginUpdateDescription">${esc(t('plugins.confirmUpdate', { name: confirmation.name, from: target.version || t('plugins.compatibility.unknownVersion'), to: target.target_version }))}</p>${confirmation.error ? `<p class="plugin-operation-error" role="alert">${esc(confirmation.error)}</p>` : ''}${confirmation.busy ? `<p class="plugin-update-progress" role="status" aria-live="polite">${esc(t('plugins.updating'))}</p>` : ''}</div>
            <footer class="dialog-footer"><button type="button" class="btn secondary" data-action="cancel-update-plugin" ${disabled}>${esc(t('common.cancel'))}</button><button type="button" class="btn primary" data-action="confirm-update-plugin" ${disabled}>${esc(t(confirmation.busy ? 'plugins.updating' : 'plugins.update'))}</button></footer>
        </section></div>`;
    }

    async function confirmUpdatePlugin() {
        if (state.dialog !== 'plugin-update-confirmation') return;
        const confirmation = state.dialogData;
        if (!confirmation?.target || confirmation.busy || state.pluginOperation || !state.capabilities.can_manage_system_settings) return;
        const { target, name, actorId, session } = confirmation;
        const sameSession = () => session === notificationSession && actorId === Number(state.user?.id);
        if (!sameSession()) { cancelPluginUpdate(); return; }
        const pluginId = target.plugin_id;
        const plugin = state.installedPlugins.find(item => item.id === pluginId);
        state.pluginOperation = pluginId;
        confirmation.busy = true;
        confirmation.error = '';
        if (plugin) plugin.operation_error = '';
        renderApp();
        try {
            const data = await api('update-plugin', { method: 'POST', body: target });
            if (!sameSession()) return;
            notificationRequest++;
            if (data.plugin && typeof data.plugin === 'object') {
                state.installedPlugins = state.installedPlugins.map(item => item.id === pluginId ? { ...item, ...data.plugin, update: null } : item);
            }
            if (state.dialog === 'plugin-update-confirmation' && state.dialogData === confirmation) {
                state.dialog = null;
                state.dialogData = null;
            ...
            state.pluginsLoaded = false;
            try { pluginStateChannel?.postMessage({ changed: true }); }
            catch (_) { /* Reload and polling also synchronize runtime state. */ }
            renderApp();
            toast(t('plugins.updated', { name, version: target.target_version }));
            window.setTimeout(() => { if (sameSession()) window.location.reload(); }, 350);
        } catch (error) {
            if (!sameSession()) return;
            const current = state.installedPlugins.find(item => item.id === pluginId);
            const message = error.message || t('plugins.updateFailed');
            if (current) current.operation_error = message;
    ...
    function detachPlugin(pluginId) {
        const plugin = state.installedPlugins.find(item => item.id === pluginId);
        if (!state.capabilities.can_manage_system_settings || !plugin || !pluginHasError(plugin)
            || plugin.can_detach !== true || state.pluginOperation || state.pluginsLoading) return;
        const confirmation = {
            target: Object.freeze({ plugin_id: pluginId, version: String(plugin.version ?? ''),
                installation_token: String(plugin.installation_token ?? '') }),
            name: pluginT(plugin.id, 'plugin.name', {}, plugin.name || plugin.id),
            actorId: Number(state.user?.id), session: notificationSession, busy: false, error: '',
        };
        state.dialog = 'plugin-detach-confirmation';
        state.dialogData = confirmation;
        renderApp();
        window.setTimeout(() => {
            if (state.dialog === 'plugin-detach-confirmation' && state.dialogData === confirmation) {
                root.querySelector('[data-action="cancel-detach-plugin"]')?.focus();
            }
        }, 0);
    }

    function cancelPluginDetach() {
        if (state.dialog !== 'plugin-detach-confirmation') return false;
        if (state.dialogData?.busy) return true;
        state.dialog = null;
        state.dialogData = null;
        renderApp();
    ...
    function renderPluginDetachDialog() {
        const confirmation = state.dialogData;
        if (!confirmation?.target) return '';
        const disabled = confirmation.busy ? 'disabled' : '';
        return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog plugin-detach-dialog" role="alertdialog" aria-modal="true" aria-labelledby="pluginDetachTitle" aria-describedby="pluginDetachDescription">
            <header class="dialog-header"><div class="dialog-title" id="pluginDetachTitle">${esc(t('plugins.detach'))}</div><button type="button" class="btn icon-only" data-action="cancel-detach-plugin" aria-label="${esc(t('common.close'))}" ${disabled}>${svg('close', 'small')}</button></header>
            <div class="dialog-body"><p class="dialog-intro" id="pluginDetachDescription">${esc(t('plugins.confirmDetach', { name: confirmation.name }))}</p><p class="plugin-detach-version">${esc(t('plugins.compatibility.pluginVersion'))}: ${esc(confirmation.target.version || t('plugins.compatibility.unknownVersion'))}</p>${confirmation.error ? `<p class="plugin-operation-error" role="alert">${esc(confirmation.error)}</p>` : ''}${confirmation.busy ? `<p role="status">${esc(t('plugins.detaching'))}</p>` : ''}</div>
            <footer class="dialog-footer"><button type="button" class="btn secondary" data-action="cancel-detach-plugin" ${disabled}>${esc(t('common.cancel'))}</button><button type="button" class="btn primary" data-action="confirm-detach-plugin" ${disabled}>${esc(t(confirmation.busy ? 'plugins.detaching' : 'plugins.detach'))}</button></footer>
        </section></div>`;
    }

    async function confirmDetachPlugin() {
        if (state.dialog !== 'plugin-detach-confirmation') return;
        const confirmation = state.dialogData;
        if (!confirmation?.target || confirmation.busy || state.pluginOperation || !state.capabilities.can_manage_system_settings) return;
        const { target, name, actorId, session } = confirmation;
        const sameSession = () => session === notificationSession && actorId === Number(state.user?.id);
        if (!sameSession()) { cancelPluginDetach(); return; }
        const pluginId = target.plugin_id;
        const plugin = state.installedPlugins.find(item => item.id === pluginId);
        state.pluginOperation = pluginId;
        confirmation.busy = true;
        ...
        renderApp();
        try {
            const data = await api('detach-plugin', { method: 'POST', body: target });
            if (!sameSession()) return;
            notificationRequest++;
            const plugins = { ...state.activePlugins };
            delete plugins[pluginId];
            applyPluginState(data.plugin_state || { plugins, plugin_ui: state.pluginUi.filter(item => item.id !== pluginId) });
            ...
            state.pluginsLoaded = false;
            try { pluginStateChannel?.postMessage({ changed: true }); }
            catch (_) { /* Polling and reload also synchronize other tabs. */ }
            await loadPlugins(false);
            if (!sameSession()) return;
            renderApp();
            toast(t('plugins.detached', { name }));
            window.setTimeout(() => { if (sameSession()) window.location.reload(); }, 350);
        } catch (error) {
            if (!sameSession()) return;
            const current = state.installedPlugins.find(item => item.id === pluginId);
            if (current) current.operation_error = error.message || t('plugins.detachFailed');
            confirmation.busy = false;
            confirmation.error = error.message || t('plugins.detachFailed');
            state.pluginOperation = '';
            renderApp();
            toast(error.message || t('plugins.detachFailed'), 'error');
        }
    }
    ...
    function renderDialog() {
        if (state.dialog === 'plugin-enable-confirmation') return renderPluginEnableDialog();
        if (state.dialog === 'plugin-update-confirmation') return renderPluginUpdateDialog();
        if (state.dialog === 'plugin-detach-confirmation') return renderPluginDetachDialog();
        ...
        if (state.dialog === 'metadata') {
            ...
            return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog"><header class="dialog-header"><div class="dialog-title">${esc(t('page.properties'))}</div>${close}</header><div class="dialog-body">
                ${childrenOf(state.page.id).length ? `<label class="setting-row setting-subtree-status"><span class="setting-icon">${svg('folder')}</span><span class="setting-meta"><span class="setting-label">${esc(t('page.applyStatusToDescendants'))}</span><span class="setting-help" id="metaStatusDescendantsHelp">${esc(t('page.applyStatusToDescendantsHelp'))}</span></span><input type="checkbox" id="metaStatusDescendants" aria-describedby="metaStatusDescendantsHelp"></label>` : ''}
        ...
        if (state.dialog === 'icon') {
            const groups = [
                ['stationery', ['✏️','✒️','🖋️','🖊️','🖌️','🖍️','📝','📄','📃','📑','📒','📓','📔','📕','📗','📘','📙','📚','🔖','📎','🖇️','📏','📐','✂️']],
                ['office', ['💼','📋','🗒️','📇','📅','📆','🗓️','📌','📍','🧷','🗑️','📦','🧾','✉️','📩','📨','📧','📮','📬','📥','📤','☎️','📞','📊']],
                ['computer', ['💻','🖥️','⌨️','🖱️','🖲️','🖨️','📠','📁','📂','🗂️','🗃️','🗄️','💾','💿','📀','🔌','🔋','📱','📲','🌐','☁️','🔗','⚙️','🧮']],
                ['basic', ['✦','◉','↗','◫','⌂','☕','◷','🌱','💡','🧭','🎯','🪴','🧩','🛠️','🔎','🎨','✅','🤝','🚀','🔐','⚠️','🚨','⏰','🔔']],
            ];
            return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog" role="dialog" aria-modal="true" aria-labelledby="pageIconDialogTitle"><header class="dialog-header"><div class="dialog-title" id="pageIconDialogTitle">${esc(t('page.icon.select'))}</div>${close}</header><div class="dialog-body">${groups.map(([category, choices]) => `<section class="page-icon-group"><h3>${esc(t('page.icon.group.' + category))}</h3><div class="icon-grid">${choices.map(icon => `<button type="button" class="icon-option ${icon === state.page.icon ? 'active' : ''}" data-action="choose-icon" data-value="${esc(icon)}" aria-label="${esc(icon)}" aria-pressed="${icon === state.page.icon}">${esc(icon)}</button>`).join('')}</div></section>`).join('')}</div></section></div>`;
        }
        if (state.dialog === 'inbox') {
            return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog inbox-dialog" role="dialog" aria-modal="true" aria-label="${esc(t('navigation.inbox'))}"><header class="dialog-header"><div class="dialog-title">${esc(t('navigation.inbox'))}${unreadCount() ? `<span class="dialog-count">${unreadCount()}</span>` : ''}</div>${close}</header><div class="dialog-body inbox-body">${renderInboxBody()}</div><footer class="dialog-footer inbox-footer"><button type="button" class="btn secondary" data-action="refresh-notifications" ${state.inboxBusy || state.inboxOpeningId || state.inboxLoading ? 'disabled' : ''}>${esc(t('inbox.refresh'))}</button><button type="button" class="btn secondary" data-action="mark-read" ${unreadCount() && !state.inboxBusy && !state.inboxOpeningId && !state.inboxLoading ? '' : 'disabled'}>${esc(t('inbox.markAllRead'))}</button></footer></section></div>`;
        }
        if (state.dialog === 'about-openconcept') {
            return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog about-dialog" role="dialog" aria-modal="true" aria-labelledby="aboutOpenConceptTitle"><header class="dialog-header"><div class="dialog-title" id="aboutOpenConceptTitle">${esc(t('about.title'))}</div>${close}</header><div class="dialog-body about-body"><div class="about-brand">${brandLogo('about-logo', 'OpenConcept')}<strong>OpenConcept</strong></div><p>CopyRight 2026 : Shinna Co.,Ltd, in Ho Chi Minh City</p><div class="about-version">${esc(t('about.version', { version: appVersion }))}</div></div><footer class="dialog-footer"><button type="button" class="btn primary" data-action="close-dialog">${esc(t('common.close'))}</button></footer></section></div>`;
        }
        ...
        if (state.dialog === 'invitation-result') {
            ...
            const delivered = result.mail_sent && result.mail_transport !== 'log';
            ...
            return `<div class="dialog-backdrop" data-action="backdrop"><section class="dialog"><header class="dialog-header"><div class="dialog-title">${esc(t(result.mail_transport === 'log' && result.mail_sent ? 'mailUi.transport.log' : titleKey))}</div>${close}</header><div class="dialog-body"><div class="invite-result-icon ${delivered ? 'success' : 'warning'}">${svg(delivered ? 'check' : 'bell')}</div><p class="dialog-intro result-copy">${esc(result.mail_sent ? (result.mail_transport === 'log' ? t('mailUi.logged') : t(successKey, { email: result.email })) : t('settings.result.mailFailed'))}</p>${result.temporary_password ? `<div class="temporary-password"><code>${esc(result.temporary_password)}</code><button class="btn secondary compact" data-action="copy-temporary-password">${svg('copy', 'small')} ${esc(t('common.copy'))}</button></div><p class="security-note">${esc(t('settings.result.passwordOneTime'))}</p>` : ''}${result.mail_error ? `<div class="mail-warning">${esc(result.mail_error)}</div>` : ''}</div><footer class="dialog-footer"><button class="btn primary" data-action="close-dialog">${esc(t('common.done'))}</button></footer></section></div>`;
        }
        return '';
    }
        ...
        const messages = state.aiMessages.map(message => {
            ...
            const sources = (message.sources || []).map((source, index) => {
                const content = `<span class="ai-source-index">${index + 1}</span>
                    ...
                    ${svg('chevron', 'tiny')}`;
                if (source.source_scope === 'knowledge_history' && source.history_reference) {
                    return `<a class="ai-source" href="history.php?record=${encodeURIComponent(source.history_reference)}" target="_blank" rel="noopener">${content}</a>`;
                }
    ...
    function syncAiSearchExtensions() {
        ...
        mountedAiSearchExtensions.forEach(id => {
            if (!host || !isPluginActive(id)) unmountAiSearchExtension(id);
        });
        if (!host) {
            return;
        }
    ...
    function notifyAiSearchExtensions(eventName, payload) {
        aiSearchExtensions.forEach((extension, id) => {
            if (!isPluginActive(id) || !mountedAiSearchExtensions.has(id)) return;
    ...
    async function openPage(id, updateHash = true) {
        ...
        try {
            ...
            revealPageInSidebar(data.page.id);
            renderApp();
            scheduleExtensionMaintenance();
            return true;
    ...
    async function showSettings(tab = 'general', updateHash = true) {
        ...
        if (state.capabilities.can_manage_system_settings) {
            await Promise.all([loadPlugins(false), loadAiProviderSettings(false), loadTranslationProviderSettings(false), loadWafCompatibilitySettings(), loadMailSettings()]);
    ...
    async function refreshSummary() {
        const inboxRequest = notificationRequest;
        const inboxWasRefreshing = Boolean(notificationInFlight);
        const data = await api('bootstrap');
        state.user = data.user;
        state.pages = (Array.isArray(data.pages) ? data.pages : []).map(normalizePageAccessDepartments);
        state.trash = (Array.isArray(data.trash) ? data.trash : []).map(normalizePageAccessDepartments);
        ...
        if (!inboxWasRefreshing && inboxRequest === notificationRequest && !state.inboxBusy && !state.inboxLoading) {
            if (state.inboxFilter === 'all' && state.inboxPage === 1) {
                applyNotificationData({ notifications: data.notifications, counts: data.notification_counts,
                    pagination: data.notification_pagination, unread_count: data.unread_count });
    ...
    function updateNotificationChrome() {
        ...
        if (markAll) markAll.disabled = count === 0 || Boolean(state.inboxBusy || state.inboxOpeningId || state.inboxLoading);
        const refresh = root.querySelector('.inbox-footer [data-action="refresh-notifications"]');
        if (refresh) refresh.disabled = Boolean(state.inboxBusy || state.inboxOpeningId || state.inboxLoading);
    }

    function applyNotificationData(data) {
        state.notifications = Array.isArray(data.notifications) ? data.notifications.slice(0, 10) : [];
        const counts = data.counts || {};
        state.notificationCounts = { all: Math.max(0, Number(counts.all) || 0),
            unread: Math.max(0, Number(counts.unread ?? data.unread_count) || 0),
            archived: Math.max(0, Number(counts.archived) || 0) };
        const pagination = data.pagination || {};
        state.inboxPagination = { page: Math.max(1, Number(pagination.page) || 1), per_page: 10,
            total: Math.max(0, Number(pagination.total) || 0), total_pages: Math.max(1, Number(pagination.total_pages) || 1) };
        state.inboxPage = Math.min(state.inboxPagination.page, state.inboxPagination.total_pages);
    }

    async function refreshNotifications(showError = false, { force = false, foreground = false } = {}) {
        if (!state.user || state.inboxBusy && !force || state.inboxLoading && !foreground && !force) return false;
        ...
        try {
            const query = `&filter=${encodeURIComponent(filter)}&page=${page}`;
            const data = await api('notifications', { query });
            if (request !== notificationRequest || !state.user || filter !== state.inboxFilter || page !== state.inboxPage) return false;
            applyNotificationData(data);
            state.inboxError = '';
            applyPluginState(data.plugin_state);
            return true;
        } catch (error) {
            if (request !== notificationRequest || !state.user) return false;
            state.inboxError = error.message || '';
            if (showError) toast(error.message || t('inbox.loadFailed'), 'error');
            return false;
        } finally {
            if (notificationInFlight === request) notificationInFlight = 0;
            if (request === notificationRequest && state.user) {
    ...
    async function mutateNotification(action, body) {
        if (!state.user || state.inboxBusy) return false;
        const session = notificationSession;
        const operation = `${action}:${Number(body.id) || 'all'}`;
        state.inboxBusy = operation;
        state.inboxLoading = false;
        state.inboxError = '';
        notificationRequest++;
        updateNotificationChrome();
        try {
            ...
            if (session !== notificationSession || !state.user) return false;
            await refreshNotifications(false, { force: true });
            return true;
        } catch (error) {
            if (session === notificationSession && state.user) {
                state.inboxError = error.message || '';
                toast(error.message || t('inbox.updateFailed'), 'error');
            }
            return false;
        } finally {
            if (session === notificationSession && state.inboxBusy === operation) {
                state.inboxBusy = '';
                updateNotificationChrome();
            }
        }
    ...
    async function setNotificationRead(id, isRead) {
        ...
        if (!notification || state.inboxBusy || state.inboxLoading) return false;
        if (Boolean(Number(notification.is_read)) === Boolean(isRead)) return true;
        if (Number(notification.is_archived)) return false;
        return mutateNotification('notification-read', { id: Number(id), is_read: Boolean(isRead) });
    }

    async function manageNotification(id, action) {
        if (state.inboxOpeningId || state.inboxLoading) return false;
        const notification = state.notifications.find(item => Number(item.id) === Number(id));
        if (!notification) return false;
        if (action === 'notification-archive' && (!Number(notification.is_read) || Number(notification.is_archived))) return false;
        if (action === 'notification-delete' && !Number(notification.is_archived)) return false;
        if (!['notification-archive', 'notification-delete'].includes(action)) return false;
        const success = await mutateNotification(action, { id: Number(id) });
        if (success) toast(t(action === 'notification-archive' ? 'inbox.archivedNotice' : 'inbox.deletedNotice'));
        return success;
    }

    async function selectInboxFilter(filter) {
        if (state.inboxBusy || state.inboxOpeningId || !['all', 'unread', 'archived'].includes(filter)) return;
        notificationNavigation++;
        state.inboxFilter = filter;
        state.inboxPage = 1;
        state.notifications = [];
        state.inboxPagination = { page: 1, per_page: 10, total: 0, total_pages: 1 };
        await refreshNotifications(false, { foreground: true });
    }

    async function selectInboxPage(page) {
        if (state.inboxBusy || state.inboxOpeningId || !Number.isSafeInteger(page) || page < 1
            || page > state.inboxPagination.total_pages || page === state.inboxPage) return;
        notificationNavigation++;
    ...
    async function openInboxNotification(id) {
        if (state.inboxBusy || state.inboxOpeningId || state.inboxLoading) return;
        const found = state.notifications.find(item => Number(item.id) === Number(id));
        if (!found) return;
        // An unread item can leave this page as soon as marking it read completes.
        const notification = { ...found };
        const navigation = ++notificationNavigation;
        const session = notificationSession;
        state.inboxOpeningId = Number(id);
        updateNotificationChrome();
        try {
            if (!Number(notification.is_read) && !Number(notification.is_archived)
                && !await setNotificationRead(id, true)) return;
            if (session !== notificationSession || navigation !== notificationNavigation || state.dialog !== 'inbox') return;
            state.dialog = null;
            if (notification.extension_action && window.OpenConceptPluginApi) {
                renderApp();
                await window.OpenConceptPluginApi.openNotification(Number(id));
            } else if (Number(notification.page_id)) {
                await openPage(Number(notification.page_id));
                if (['comment', 'reply', 'mention'].includes(notification.type) && state.page) {
                    state.panel = 'comments';
                    renderApp();
                }
            } else renderApp();
        } catch (error) {
            if (session === notificationSession) toast(error.message, 'error');
        } finally {
            if (session === notificationSession && state.inboxOpeningId === Number(id)) {
                state.inboxOpeningId = 0;
                updateNotificationChrome();
            }
        }
    }
    ...
    async function savePage() {
        ...
        const statusDescendantsRequest = pageAtRequest._statusDescendantsRequest;
        const applyStatusToDescendants = statusDescendantsRequest?.status === requestedAccessState.status;
        try {
            ...
            if (applyStatusToDescendants) payload.apply_status_to_descendants = true;
            const data = await api('save-page', { method: 'POST', body: payload });
            if (pageAtRequest._statusDescendantsRequest === statusDescendantsRequest) delete pageAtRequest._statusDescendantsRequest;
            ...
            if (applyStatusToDescendants && !summaryRefreshFailed) {
                const sidebar = root.querySelector('.sidebar');
                if (sidebar) {
                    rememberSidebarScroll();
                    sidebar.outerHTML = renderSidebar();
            ...
            state.saveRetryBlocked = err.retryable === false || ((isValidationFailure || err.code === 'invalid_server_response') && !hasNewerEdits);
            state.saveFailure = state.saveRetryBlocked ? 'validation' : (isValidationFailure ? null : 'retryable');
            state.saveErrorMessage = err.message;
            toast(err.message, 'error');
    ...
    function updateSaveIndicator(mode) {
        ...
        el.title = mode === 'error' ? state.saveErrorMessage : '';
    ...
    async function submitInvitation() {
        ...
        try {
            const data = await api('invite-member', { method: 'POST', body: {
                ...
                ui_locale: form.elements.ui_locale.value,
                department,
    ...
    root.addEventListener('change', async event => {
        if (event.target.id === 'wafCompatibilityEnabled') {
            state.wafCompatibilityDraft = Boolean(event.target.checked);
        }
        ...
        if (event.target.id === 'translationProviderType' || event.target.id === 'translationProviderAuthMode' || event.target.id === 'translationProviderUseSharedKey') {
            const preserveKeySelectionPosition = event.target.id === 'translationProviderUseSharedKey';
            const settingsScrollTop = preserveKeySelectionPosition
                ? (root.querySelector('.settings-page-scroller')?.scrollTop || 0)
                : 0;
            const draft = readTranslationProviderForm();
            ...
            state.translationProviderNotice = '';
            renderApp();
            if (preserveKeySelectionPosition) {
                const scroller = root.querySelector('.settings-page-scroller');
                if (scroller) scroller.scrollTop = settingsScrollTop;
                document.getElementById('translationProviderUseSharedKey')?.focus({ preventScroll: true });
            }
            return;
        }
        ...
        if (event.target.id === 'uiThemeSelect') {
            window.OpenConceptTheme?.set(event.target.value);
            return;
        }
    ...
    root.addEventListener('submit', event => {
        ...
        if (event.target.id === 'mailSettingsForm') {
            event.preventDefault();
            saveMailSettings();
            return;
        }
        if (event.target.id === 'wafCompatibilityForm') {
            event.preventDefault();
            saveWafCompatibilitySettings();
        }
    ...
    root.addEventListener('click', async event => {
        ...
        if (action === 'open-page') { event.preventDefault(); event.stopPropagation(); if (Date.now() < state.suppressTreeClickUntil) return; state.mobileSidebar = false; await openPage(target.dataset.id); }
        ...
        else if (action === 'toggle-sidebar-more') {
            state.sidebarMoreOpen = !state.sidebarMoreOpen;
            const items = root.querySelector('#sidebarMoreItems');
            if (items) items.hidden = !state.sidebarMoreOpen;
            const label = t(state.sidebarMoreOpen ? 'sidebar.hideMore' : 'sidebar.showMore');
            target.setAttribute('aria-expanded', String(state.sidebarMoreOpen));
            target.setAttribute('aria-label', label);
            target.title = label;
        }
        else if (action === 'toggle-tree') { event.stopPropagation(); toggleTreePage(target.dataset.id); root.querySelector(`.page-tree-main [data-action="toggle-tree"][data-id="${Number(target.dataset.id)}"]`)?.focus({ preventScroll: true }); }
        else if (action === 'expand-all-tree') setAllTreeExpanded(true);
        else if (action === 'collapse-all-tree') setAllTreeExpanded(false);
            ...
            if (state.inboxBusy || state.inboxOpeningId) return;
            notificationNavigation++;
            state.dialog = 'inbox';
            state.inboxFilter = 'all';
            state.inboxPage = 1;
        ...
        else if (action === 'retry-page-save') await retryPageSave();
        ...
        else if (action === 'confirm-enable-plugin') await confirmEnablePlugin();
        else if (action === 'cancel-enable-plugin') cancelPluginEnable();
        else if (action === 'detach-plugin') await detachPlugin(target.dataset.pluginId);
        else if (action === 'update-plugin') updatePlugin(target.dataset.pluginId);
        else if (action === 'confirm-update-plugin') await confirmUpdatePlugin();
        else if (action === 'cancel-update-plugin') cancelPluginUpdate();
        else if (action === 'confirm-detach-plugin') await confirmDetachPlugin();
        else if (action === 'cancel-detach-plugin') cancelPluginDetach();
        else if (action === 'close-dialog' || action === 'backdrop' && event.target === target) {
            if (state.dialog === 'plugin-enable-confirmation') { cancelPluginEnable(); return; }
            if (state.dialog === 'plugin-update-confirmation') { cancelPluginUpdate(); return; }
            if (state.dialog === 'plugin-detach-confirmation') { cancelPluginDetach(); return; }
            if (state.dialog === 'inbox') notificationNavigation++;
            ...
            if (document.getElementById('metaStatusDescendants')?.checked && childrenOf(state.page.id).length) {
                state.page._statusDescendantsRequest = { status: requestedStatus };
            } else {
                delete state.page._statusDescendantsRequest;
            }
            ...
            state.page.category = document.getElementById('metaCategory').value.trim() || t('page.defaultCategory');
            ...
            syncPageTags(state.page);
            state.dialog = null; scheduleSave(true); renderApp();
        }
        ...
        else if (action === 'extension-replace-file') chooseFileReplacement(Number(target.dataset.id));
        else if (action === 'extension-file-action') {
            const file = state.files.find(item => Number(item.id) === Number(target.dataset.id));
            const descriptor = file?.extension_actions?.[Number(target.dataset.index)];
            if (descriptor) await window.OpenConceptPluginApi.openAction(descriptor);
        }
        ...
        else if (action === 'notification') await openInboxNotification(Number(target.dataset.id));
        else if (action === 'toggle-notification-read') await setNotificationRead(Number(target.dataset.id), target.dataset.read === '1');
        else if (action === 'archive-notification') await manageNotification(Number(target.dataset.id), 'notification-archive');
        else if (action === 'delete-notification') await manageNotification(Number(target.dataset.id), 'notification-delete');
        else if (action === 'inbox-filter') await selectInboxFilter(target.dataset.filter);
        else if (action === 'inbox-page') await selectInboxPage(Number(target.dataset.page));
        else if (action === 'refresh-notifications') { if (await refreshNotifications(true, { foreground: true })) toast(t('inbox.refreshed')); }
        else if (action === 'mark-read') {
            if (!state.inboxOpeningId && !state.inboxLoading && unreadCount()
                && await mutateNotification('mark-notifications', {})) toast(t('inbox.allMarkedRead'));
        }
        else if (action === 'test-mail') { testMailSettings(); }
    ...
    document.addEventListener('keydown', event => {
        ...
        if (state.dialog === 'plugin-enable-confirmation' && event.key === 'Tab') {
            const buttons = [...root.querySelectorAll('.plugin-enable-dialog button:not([disabled])')];
            ...
            if (!event.shiftKey && (document.activeElement === last || !buttons.includes(document.activeElement))) {
        ...
        if (event.key === 'Escape') {
            if (state.dialog === 'plugin-enable-confirmation') { event.preventDefault(); cancelPluginEnable(); }
            else if (state.dialog === 'plugin-update-confirmation') { event.preventDefault(); cancelPluginUpdate(); }
            else if (state.dialog === 'plugin-detach-confirmation') { event.preventDefault(); cancelPluginDetach(); }
            else if (state.editorFind.open) closeEditorFind();
