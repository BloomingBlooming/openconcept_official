...
/** API requirements apply to every plugin; release minimums constrain only the listed IDs. */
final class PluginCompatibility
{
    private const MINIMUMS = [
        // Core 3 retains Plugin API 1 and the existing independent plugin minimums.
        '3.0.0' => [
    ...
    public function __construct(
        private readonly string $appVersion = '3.0.0',
    ...
    public function check(array $plugin, bool $apiAvailable = true): array
    {
        ...
        if (!is_string($plugin['id'] ?? null) || strlen($id) > 64
            || preg_match('/^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/D', $id) !== 1
            || !is_array($requirements) || preg_match(self::VERSION_PATTERN, $version) !== 1) {
        ...
        $status['code'] = match (true) {
            $minimum !== null && self::compareVersions($version, $minimum) < 0 => 'unsupported_plugin_version',
