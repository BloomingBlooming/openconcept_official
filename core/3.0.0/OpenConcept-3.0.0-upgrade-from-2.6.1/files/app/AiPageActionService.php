...
final class AiPageActionService
{
    ...
    public function __construct(PDO $pdo, ?OpenAIClient $client = null, ?string $applicationUrl = null)
    {
        ...
        $this->i18n->registerPackage('ai-page-actions', dirname(__DIR__) . '/locales/ai-page-actions');
    }
    ...
    public function handle(
        ...
        if ($operation === 'none') {
            ...
            return $this->result($question, $answer ?: $this->message('cannotExecute'), $page, $response, []);
        }
        if ($page === null) {
            return $this->result(
                $question,
                $this->message('openPage'),
                null,
                $response,
                []
            );
        ...
        if (in_array($operation, ['rename_page', 'create_page', 'update_focused_block', 'insert_after_focus'], true)) {
            ...
            $preview = [
                'heading' => $this->message('changeHeading'),
                'summary' => '',
                'before' => '',
                'after' => '',
            ];
            ...
            if ($operation === 'rename_page') {
                if ($normalizedPlan['title'] === '') {
                    return $this->result($question, $this->message('titleRequired'), $page, $response, []);
                }
                $preview['summary'] = $this->message('renameSummary');
                ...
                if ($normalizedPlan['title'] === '' || $plannedBlocks === []) {
                    return $this->result($question, $this->message('createRequired'), $page, $response, []);
                }
                $preview['heading'] = $this->message('createHeading');
                $preview['summary'] = $this->message('createSummary');
                $preview['before'] = $this->message('destination', ['title' => (string) $page['title']]);
                $preview['after'] = $normalizedPlan['title'] . "\n" . blocksPlainText($plannedBlocks);
            } else {
                $blockId = (string) ($editorContext['block_id'] ?? '');
                if ($blockId === '') {
                    return $this->result($question, $this->message('focusRequired'), $page, $response, []);
                }
                $blocks = json_decode((string) ($page['blocks_json'] ?? '[]'), true);
                $blocks = is_array($blocks) ? $blocks : [];
                $index = $this->blockIndex($blocks, $blockId);
                if ($index < 0) {
                    return $this->result($question, $this->message('blockChanged'), $page, $response, []);
                }
                ...
                if ($operation === 'update_focused_block') {
                    ...
                    if (!in_array($type, ['paragraph', 'lead', 'heading1', 'heading2', 'heading3', 'bullet', 'number', 'todo', 'quote', 'code', 'callout'], true)) {
                        return $this->result($question, $this->message('blockUnsupported'), $page, $response, []);
                    }
                    $preview['summary'] = $this->message('replaceSummary');
                    $preview['after'] = $normalizedPlan['content'] === '' ? $this->message('emptyContent') : $normalizedPlan['content'];
                } else {
                    $plannedBlocks = $this->plannedBlocks($normalizedPlan['blocks'], (string) ($page['plain_text'] ?? ''));
                    if ($plannedBlocks === []) {
                        return $this->result($question, $this->message('insertRequired'), $page, $response, []);
                    }
                    $preview['summary'] = $this->message('insertSummary');
            ...
            $result = $this->result(
                ...
                $operation === 'create_page'
                    ? $this->message('createPrepared')
                    : $this->message('changePrepared'),
                $page,
                $response,
                []
            );
            ...
            $answer = $answer ?: $this->message('focusTitleMoved');
            ...
            $answer = $answer ?: $this->message('focusBlockMoved');
        } else {
            return null;
        }
    ...
    public function executePending(array $pending, array $user, string $expectedPasswordFingerprint): array
    {
        ...
        if ($operation === 'rename_page') {
            ...
            $answer = $this->message('renamed', ['title' => $plan['title']]);
        } elseif ($operation === 'create_page') {
            ...
            $answer = $this->message('created', ['title' => $created['title']]);
...
SQL);
    ...
    ): array
    {
        ...
        $blocks[] = ['id' => $this->blockId(), 'type' => 'heading2', 'content' => $this->message('sourcesHeading')];
        $blocks[] = [
            'id' => $this->blockId(),
            'type' => 'paragraph',
            'content' => '<a href="' . htmlspecialchars($sourceUrl, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '" target="_blank" rel="noopener noreferrer">' . htmlspecialchars((string) $source['title'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</a>' . htmlspecialchars($this->message('sourcePageSuffix'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'),
        ];
...
SQL);
            ...
            $insert->execute([
                ...
                'none',
                $status,
                cleanText((string) ($source['category'] ?? ''), 80) ?: $this->message('page.defaultCategory', [], 'core'),
                $inheritedTags,
                $inheritedTags,
    ...
    private function insertRevision(array $page, array $user): void
    {
        $meta = [
            ...
            'category' => (string) ($page['category'] ?? $this->message('page.defaultCategory', [], 'core')),
        ...
        foreach (array_slice($planned, 0, 60) as $raw) {
            ...
            if ($type === 'quote') {
                ...
                if ($normalizedQuote === '' || $normalizedSource === '' || !str_contains($normalizedSource, $normalizedQuote)) {
                    $type = 'paragraph';
                    $text = $this->message('summaryText', ['text' => $text]);
                }
            }
            $blocks[] = [
                'id' => $this->blockId(),
    ...
    private function focusDescription(array $context): string
    {
        return match ((string) ($context['target_type'] ?? 'none')) {
            'title' => $this->message('focusTitle'),
            'block' => $this->message('focusBlock', ['text' => utf8Slice((string) ($context['text'] ?? ''), 80)]),
            'table_cell' => $this->message('focusTableCell', ['row' => (int) ($context['row'] ?? 0) + 1, 'column' => (int) ($context['column'] ?? 0) + 1]),
            default => $this->message('focusUnknown'),
        };
    }
    ...
    private function result(string $question, string $answer, ?array $page, array $response, array $actions): array
    {
        ...
        $sources = $pageId > 0 ? [[
            ...
            'title' => $pageTitle,
            'summary' => $this->message('sourceSummary'),
    ...
    private function selectActorLocale(array $user): void
    {
        $this->locale = $this->i18n->selectLocale(is_string($user['ui_locale'] ?? null) ? $user['ui_locale'] : 'en-US');
    }

    /** Format only application-owned text; do not reinterpret placeholder-like input values. */
    private function message(string $key, array $parameters = [], string $domain = 'ai-page-actions'): string
    {
        $template = $this->i18n->catalog($domain, $this->locale)[$key] ?? $key;
        $replacements = [];
