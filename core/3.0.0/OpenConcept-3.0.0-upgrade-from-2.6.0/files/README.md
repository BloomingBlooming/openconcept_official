# OpenConcept 3.0.0 公開配布版

V3.0.0はV2.6.1以降の差分と未配布のV2.6.2・V2.7.0を累積した版です。送信メールアカウントの管理画面、招待・ログイン・初回パスワード変更、API・プラグイン・通知・出力の5言語対応、プロフィール写真の圧縮を含みます。履歴カレンダーとフロートNavi 1.2.3の状態保持も継承します。

Version 3.0.0 consolidates changes since 2.6.1, including the unreleased 2.6.2 and 2.7.0 work. It includes outgoing email settings, five-language invitations, sign-in, first-password changes, API/plugin messages, notifications and exports, plus profile photo compression. It retains the localized history calendar and persistent Float Navi 1.2.3 preferences. User-written content remains unchanged when the interface language changes.

アプリのダークモード、カバー13種類（なしを含む）、アイコン96種類、受信トレイ下の三点アイコンによるAI検索・記録と履歴の開閉を含みます。フロートNavi 1.2.3は個別プラグインとして導入・更新できます。

Includes app-wide dark mode, 13 cover choices including No cover, 96 page icons, and the three-dot toggle below Inbox for AI search and Records and history. Float Navi 1.2.3 is installed or updated as a separate plugin.

既公開の図面管理0.9.3・フロートNavi 1.2.2はV3.0.0でも利用できます。今回の修正を含む図面管理0.9.4・フロートNavi 1.2.3は独立配布で、本体パッケージには同梱しません。

Previously published Drawing Manager 0.9.3 and Float Navi 1.2.2 remain compatible with V3.0.0. The updated Drawing Manager 0.9.4 and Float Navi 1.2.3 are distributed separately and are not bundled in the Core package.

このフォルダーは、不特定多数への配布を目的としたOpenConceptの実行パッケージです。個別環境のホスト名、Database接続情報、APIキー、メールアカウント、SSH設定、利用者データ、ログ、添付ファイルは含まれていません。

V2.6.1では、プラグインIDを本体へ事前登録する制限を解除しました。新しいIDでも、マニフェスト・要求API・必要な機能の条件を満たせば管理者が有効化できます。既存プラグインの最低対応版、設定、データ、ON/OFFを保持し、Core DBスキーマ世代6とPlugin API 1.0.0を維持します。

Version 2.6.1 removes the requirement to register new plugin IDs in Core. Administrators can enable a new ID when its manifest, required API version, and capabilities meet the application requirements. Existing minimum plugin versions, settings, data, and enabled states are preserved. Core schema generation 6 and Plugin API 1.0.0 remain unchanged.

ページツリー・検索結果・お気に入りは「アイコン＋状態マーク＋ページ名」で表示します。管理者・編集者・投稿者には承認待ち「♦」（黄色）、下書き「■」（白）、非公開「✖」（赤）を表示し、公開ページはマークなし。ツールチップは「（状態）ページ名」です。フォルダーのプロパティーでチェックして状態を保存すると、フォルダー自身と配下すべての状態を一括変更できます。記録と履歴のページ送り（10・25・50件）と、プラグイン有効化時の確認も追加しました。

Sidebar status marks appear between the icon and title without a colon: yellow ♦ for pending approval, white ■ for drafts, red ✖ for private pages, and no mark for published pages. Marks are visible to system/content administrators, editors, and authors; tooltips show the status in parentheses before the full title. Folder properties can apply one status to the folder and all active descendants. Records and history add pagination with 10/25/50 results per page, and plugin activation adds confirmation.

詳しくは [変更点と利用方法](docs/release-notes-3.0.0.md)、[Plugin API](docs/plugin-api-reference.md)、[プラグイン開発](docs/plugin-development.md) を参照してください。V2.6.0の記録と履歴・正本出力を継続利用できます。正本アーカイブは資格情報や環境設定を含まないため、環境全体バックアップも継続します。

See the [release notes](docs/release-notes-3.0.0.md) and [Plugin API reference](docs/plugin-api-reference.md). Records, history, and portable knowledge archives from v2.6.0 remain available. Knowledge archives omit credentials and environment settings; continue complete environment backups as well.


新規設置にはこの公開パッケージを使用します。既存環境の更新には、更新元・更新先のバージョンに適合する検証済みアップグレードパッケージと、その更新手順を使用してください。データや秘密設定を保持して更新します。

Use this public package for a new installation. To update an existing installation, use a verified upgrade package that supports both your installed version and the target version, and follow its upgrade instructions. Preserve data and private configuration during the upgrade.
...
- PDOおよびPDO SQLite
- OpenSSLとPHP session
- プロフィール写真の縮小・圧縮にはGDのJPEG・PNG・WebP対応
- AI・外部接続を使う場合はcURL
