...
final class FileReaderService
{
    ...
    private function recoverFailedJobs(): void
    {
        ...
        foreach ($jobs['items'] as $job) {
            ...
            $this->api->transaction(function () use ($key): void {
                ...
                $this->notifyReview($candidate['filename'], $key, $reference);
            });
        }
    ...
    private function notifyReview(string $filename, string $key, array $reference): void
    {
        $parameters = ['filename' => $filename];
        $target = ['key' => $key] + $reference;
        if (method_exists($this->api, 'notifyAdminsLocalized')) {
            $this->api->notifyAdminsLocalized('file_reading_review', 'review.notification', $parameters, $target, 'review', 'review:' . $key);
            return;
        }
        // Older Core versions continue to receive the established notification contract.
        $this->api->notifyAdmins('file_reading_review', $this->tr('review.notification', $parameters), $target, 'review', 'review:' . $key);
    }
