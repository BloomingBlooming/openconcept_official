...
require_once __DIR__ . '/PageSubtreeStatus.php';
...
require_once __DIR__ . '/ProfilePhoto.php';
...
function stopForCanonicalDatabaseFailure(DatabaseUnavailableException $exception): never
{
    ...
    require_once __DIR__ . '/DatabaseFailurePageRenderer.php';
    $failureRenderer = new DatabaseFailurePageRenderer();
    ...
    $diagnostic = $failureRenderer->diagnostic($category);
    ...
    if (str_contains($accept, 'application/json') || str_ends_with($script, '/api.php')) {
        ...
        echo json_encode([
            'error' => $selectionFailure
                ? $failureRenderer->text('selection.message')
                : $diagnostic['message'],
            ...
            'policy' => $selectionFailure
                ? $failureRenderer->text('selection.policy')
                : $diagnostic['action'],
            'recovery' => $safeRecovery === [] ? null : $safeRecovery,
        ], JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR);
    } else {
        header('Content-Type: text/html; charset=utf-8');
        echo $failureRenderer->html($category, $exception->backend(), $exception->failureCode(), $safeRecovery);
    }
    exit;
}
...
if (is_dir(dirname(__DIR__) . '/locales/page-export')) {
    $i18n->registerPackage('page-export', dirname(__DIR__) . '/locales/page-export');
}
require_once __DIR__ . '/SystemMessageLocalizer.php';
SystemMessageLocalizer::loadCore();
...
(new KnowledgeSearchBridge($pdo))->register();
...
{
    if (isset($GLOBALS['i18n']) && class_exists(SystemMessageLocalizer::class, false)) {
        $actor = $GLOBALS['user'] ?? null;
        $data = SystemMessageLocalizer::response($data, uiLocale(is_array($actor) ? $actor : null), $GLOBALS['i18n']);
    }
...
function requireUser(PDO $pdo): array
{
    ...
    if (!$user) {
        jsonResponse(['error' => localizedAuthError('ログインが必要です。')], 401);
    }
    return $user;
}
...
function requireCsrf(): void
{
    ...
    if (!is_string($token) || !hash_equals((string) ($_SESSION['csrf'] ?? ''), $token)) {
        jsonResponse(['error' => localizedAuthError('セッションの確認に失敗しました。再読み込みしてください。', $GLOBALS['user'] ?? null)], 419);
    }
}
...
function requireRole(array $user, array $roles): void
{
    if (!in_array($user['role'], $roles, true)) {
        jsonResponse(['error' => localizedMemberError('この操作を行う権限がありません。', $user)], 403);
    }
}
...
function updateMemberRole(PDO $pdo, array $actor, int $memberId, string $role): array
{
    ...
    try {
        ...
        if ($previousRole !== $role) {
            ...
            $messageKey = $role === 'admin'
                ? 'notification.adminGranted'
                : ($previousRole === 'admin'
                    ? 'notification.adminRevoked'
                    : 'notification.roleChanged');
            createLocalizedNotification($pdo, $memberId, 'account_role_changed', $messageKey, [], null);
            if ($previousRole === 'admin' || $role === 'admin') {
                ...
                foreach ($currentAdministratorIds as $administratorId) {
                    ...
                    createLocalizedNotification(
                        ...
                        'notification.administratorChanged', ['name' => (string) $member['name']],
                        null
                    );
                }
            }
            ...
            foreach ($pdo->query("SELECT id FROM users WHERE active = 1 AND role = 'admin' ORDER BY id")->fetchAll(PDO::FETCH_COLUMN) as $administratorId) {
                ...
                if ($administratorId !== $actorId) {
                    createLocalizedNotification(
                        ...
                        'notification.administratorSuspended', ['name' => (string) $member['name']],
                        null
                    );
                }
            }
...
SQL);
        ...
        createLocalizedNotification(
            ...
            $memberId,
            'account_reactivated',
            $role === 'admin' ? 'notification.adminReactivated' : 'notification.reactivated', [],
            null
        );
...
/** Only installed, exact language-pack codes are accepted in shared login URLs. */
function requestedUiLocale(): ?string
{
    global $i18n;
    $requested = $_GET['lang'] ?? null;
    return is_string($requested) && I18n::isValidLocale($requested)
        && $i18n->selectLocale($requested) === $requested ? $requested : null;
}

/** Translate existing authentication validation messages at the UI boundary. */
function localizedAuthError(string $message, ?array $user = null): string
{
    global $i18n;
    $key = [
        'ログインが必要です。' => 'auth.loginRequired',
        'セッションの確認に失敗しました。再読み込みしてください。' => 'auth.sessionVerificationFailed',
        'ログイン状態を確認できません。もう一度ログインしてください。' => 'auth.sessionChanged',
        'ログイン状態が変更されました。もう一度ログインしてください。' => 'auth.sessionChanged',
        '確認用パスワードが一致しません。' => 'passwordChange.mismatch',
        'パスワードは12文字以上にしてください。' => 'passwordChange.tooShort',
        '英大文字・英小文字・数字・記号のうち3種類以上を使用してください。' => 'passwordChange.tooWeak',
        '現在のパスワードとは異なるパスワードを設定してください。' => 'passwordChange.mustDiffer',
        '現在のパスワードが正しくありません。' => 'passwordChange.currentIncorrect',
        '続行するにはパスワードを変更してください。' => 'passwordChange.required',
        '利用可能な画面言語を選択してください。' => 'auth.invalidLanguage',
        '画面言語を更新できませんでした。' => 'auth.languageUpdateFailed',
        '初期セットアップは完了しています。' => 'setup.alreadyComplete',
        '有効なユーザーが存在するため初期化できません。' => 'setup.activeUsersExist',
        '管理者名を入力してください。' => 'setup.nameRequired',
        '正しいメールアドレスを入力してください。' => 'setup.emailInvalid',
        'このメールアドレスは既に使用されています。' => 'setup.emailInUse',
    ][$message] ?? null;
    return $key === null ? $message : $i18n->translate($key, [], uiLocale($user));
}

/** Localize member onboarding failures without changing the legacy service contracts. */
function localizedMemberError(string $message, array $user): string
{
    global $i18n;
    $key = [
        'この操作を行う権限がありません。' => 'settings.memberError.permission',
        '氏名と正しいメールアドレスを入力してください。' => 'settings.memberError.inviteDetails',
        '部署を選択するか、新しい部署名を入力してください。' => 'settings.memberError.department',
        'このメールアドレスのアカウントは停止されているため、再登録できません。' => 'settings.memberError.emailSuspended',
        'このメールアドレスは既に登録されています。' => 'settings.memberError.emailRegistered',
        'メンバーを招待できませんでした。' => 'settings.memberError.inviteFailed',
        '再送するメンバーを確認してください。' => 'settings.memberError.resendTarget',
        'このメンバーは初回ログイン待ちではありません。画面を更新してください。' => 'settings.memberError.notPending',
        '招待を再送できませんでした。' => 'settings.memberError.resendFailed',
        '復活するアカウントと権限を確認してください。' => 'settings.memberError.reactivateTarget',
        '復活できる停止中アカウントが見つかりません。' => 'settings.memberError.reactivateMissing',
        'アカウントを復活できませんでした。' => 'settings.memberError.reactivateFailed',
        'パスワードを再設定するメンバーを確認してください。' => 'settings.memberError.resetTarget',
        'パスワードを再設定できるメンバーが見つかりません。' => 'settings.memberError.resetMissing',
        'パスワードを再設定できませんでした。' => 'settings.memberError.resetFailed',
    ][$message] ?? null;
    return $key === null ? localizedAuthError($message, $user) : $i18n->translate($key, [], uiLocale($user));
}
...
/** Generate a known application message in the recipient's saved locale. */
function localizedNotificationMessage(PDO $pdo, int $userId, string $key, array $parameters = [], string $domain = 'system-messages'): string
{
    return SystemMessageLocalizer::message($key, $parameters, notificationRecipientLocale($pdo, $userId), $domain);
}

function notificationRecipientLocale(PDO $pdo, int $userId): string
{
    $statement = $pdo->prepare('SELECT ui_locale FROM users WHERE id = ?');
    $statement->execute([$userId]);
    return (string) ($statement->fetchColumn() ?: 'en-US');
}

/** The existing record store preserves parameters without changing notification rows or schema. */
function createLocalizedNotification(PDO $pdo, int $userId, string $type, string $key, array $parameters, ?int $pageId, string $domain = 'system-messages', bool $refresh = false): int
{
    $message = localizedNotificationMessage($pdo, $userId, $key, $parameters, $domain);
    $id = $refresh ? createOrRefreshNotification($pdo, $userId, $type, $message, $pageId) : createNotification($pdo, $userId, $type, $message, $pageId);
    $records = new PluginApiRepository($pdo);
    $previous = $records->get('core:notifications', 'localization', (string) $id);
    $records->put('core:notifications', 'localization', (string) $id, [
        'domain' => $domain, 'key' => $key, 'parameters' => $parameters,
        'source_sha256' => hash('sha256', cleanText($message, 500)),
    ], $previous === null ? 0 : (int) $previous['revision']);
    return $id;
}
    ...
    try {
        ...
        foreach ($administratorIds as $administratorId) {
            createOrRefreshNotification(
                ...
                SystemMessageLocalizer::translate($message, notificationRecipientLocale($pdo, (int) $administratorId)),
                null
            );
        }
        return true;
...
function notifyCommentParticipants(PDO $pdo, array $actor, array $page, string $body, ?int $parentId): array
{
    ...
    $actorName = cleanText((string) ($actor['name'] ?? ''), 120);
    $pageTitle = cleanText((string) ($page['title'] ?? ''), 240);
    foreach ($recipients as $recipientId => $type) {
        $locale = notificationRecipientLocale($pdo, (int) $recipientId);
        $parameters = [
            'name' => $actorName ?: SystemMessageLocalizer::message('notification.member', [], $locale),
            'title' => $pageTitle ?: SystemMessageLocalizer::message('page.untitled', [], $locale, 'core'),
        ];
        createLocalizedNotification($pdo, (int) $recipientId, $type, 'notification.' . $type, $parameters, $pageId);
    }
...
{
    return ProfilePhoto::inspect($temporaryPath, $originalName);
}
...
function deleteUnusedProfilePhoto(PDO $pdo, string $storedName): void
{
    ...
    $thumbnail = uploadStoragePath() . DIRECTORY_SEPARATOR . ProfilePhoto::CACHE_DIRECTORY . DIRECTORY_SEPARATOR . $storedName;
    if (is_file($thumbnail) && !@unlink($thumbnail)) {
        error_log('OpenConcept profile thumbnail cleanup failed: ' . $thumbnail);
    }
}
...
function fileReferencesById(array $pages): array
{
    ...
    foreach ($pages as $page) {
        ...
        foreach (array_keys($fileIds) as $fileId) {
            $references[$fileId][] = [
                'page_id' => $pageId,
                'page_title' => cleanText((string) ($page['title'] ?? ''), 240) ?: SystemMessageLocalizer::message('page.untitled', [], uiLocale(is_array($GLOBALS['user'] ?? null) ? $GLOBALS['user'] : null), 'core'),
            ];
        }
    }
    return $references;
...
): array
{
    ...
    try {
        ...
        // Removing a file from current use must not destroy the evidence linked
        // by earlier page revisions. Bytes and their access provenance survive.
        KnowledgeFileVersions::retain($pdo, $file, uploadStoragePath(), (int) $user['id'], 'removed_from_current');
