...
foreach (['calendar','calendarFor','previousMonth','nextMonth','today','clearDate','closeCalendar','dateFormat','invalidDate'] as $key) {
    $historyLabels[$key] = $i18n->translate('knowledge.' . $key, [], $historyLocale);
}
$historyBoot = ['csrf' => $_SESSION['csrf'], 'locale' => $historyLocale, 'labels' => $historyLabels, 'admin' => $historyUser['role'] === 'admin',
...
<link rel="stylesheet" href="assets/knowledge-date-picker.css?v=<?= substr(hash_file('sha256', __DIR__ . '/assets/knowledge-date-picker.css'), 0, 12) ?>">
...
<?php foreach (['from', 'to'] as $dateField): ?>
<div class="knowledge-date-field"><label for="knowledge-<?= $dateField ?>"><?= $h($historyLabels[$dateField]) ?></label><div class="knowledge-date-input">
<input id="knowledge-<?= $dateField ?>" name="<?= $dateField ?>" type="text" inputmode="numeric" placeholder="YYYY-MM-DD" maxlength="10" autocomplete="off" aria-describedby="knowledge-date-format">
<button type="button" data-knowledge-calendar="<?= $dateField ?>" aria-label="<?= $h(str_replace('{field}', $historyLabels[$dateField], $historyLabels['calendarFor'])) ?>" aria-haspopup="dialog" aria-expanded="false"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M16 3v4M8 3v4M3 11h18"/></svg></button>
</div></div>
<?php endforeach; ?>
...
<p id="knowledge-date-format" class="knowledge-note"><?= $h($historyLabels['dateFormat']) ?></p>
...
</main><script id="knowledge-boot" type="application/json"><?= json_encode($historyBoot, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR) ?></script><script src="assets/knowledge-date-picker.js?v=<?= substr(hash_file('sha256', __DIR__ . '/assets/knowledge-date-picker.js'), 0, 12) ?>" defer></script><script src="assets/knowledge-history.js?v=<?= $h($historyAssetVersions['js']) ?>" defer></script></body></html>
