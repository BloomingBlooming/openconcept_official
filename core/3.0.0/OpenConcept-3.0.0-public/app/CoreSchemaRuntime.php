...
require_once __DIR__ . '/PluginApiSchema.php';
...
final class CoreSchemaRuntime
{
    ...
    private const GENERATION6_INTENT = 'database.schema_generation6_intent';
    ...
    private ?CoreSchemaCatalog $generation5Catalog = null;
    ...
    private function inspectPreflight(bool $writesAllowed): ?array
    {
        ...
        if ($version === $this->catalog->schemaVersion()) {
            if ($this->setting(CoreSchemaMySqlGeneration5Upgrade::INTENT_SETTING) !== null
                || $this->setting(self::GENERATION6_INTENT) !== null) {
        ...
        if ($version === 5 && $this->catalog->schemaVersion() === 6) {
            $this->assertGeneration6SourceOrProgress();
            return null;
        }
    ...
    private function assertLegacyBaseEligibility(): void
    {
        ...
        $knownNonBase = array_merge(self::RAG_TABLES, self::ROUTE_TABLES, self::LEGACY_RAG_TABLES, PluginApiSchema::TABLES);
    ...
    public function boot(bool $writesAllowed): array
    {
        ...
        try {
            ...
            if ($globalVersion === 4 && $this->catalog->schemaVersion() >= 5) {
                ...
                if ($this->catalog->schemaVersion() === 6) {
                    $generationStages[] = $generation5['migration'] ?? [];
                    $globalVersion = 5;
                } elseif ($generationStages === []) {
                    return $generation5;
                } else {
            ...
            if ($globalVersion === 5 && $this->catalog->schemaVersion() === 6) {
                $generation6 = $this->upgradeSealedGeneration5ToGeneration6();
                if ($generationStages !== []) {
                    $generation6['migration']['stages'] = array_merge($generationStages, [$generation6['migration']]);
                    $generation6['migration']['from_schema_version'] = $generationStages[0]['from_schema_version'];
                    $generation6['migration']['atomic'] = false;
                    $generation6['migration']['physical_atomic'] = false;
                    $generation6['migration']['activation_atomic'] = false;
                }
                return $generation6;
            }
            $this->verifier->verifyTables($this->pdo, $this->baseTables());
            ...
            if ($this->catalog->schemaVersion() >= 6) {
                PluginApiSchema::migrate($this->pdo);
            }
    ...
    private function requiresMySqlGeneration5Upgrade(): bool
    {
        ...
        return in_array($this->storedVersion(self::VERSION_SETTING), [3, 4, 5], true);
    }
    ...
    private function assertSealedGeneration4UpgradeSourceOrProgress(): void
    {
        ...
        if ((string) $this->pdo->getAttribute(PDO::ATTR_DRIVER_NAME) === 'mysql') {
            (new CoreSchemaMySqlGeneration5Upgrade($this->pdo, $catalog, $this->generation5Catalog()))->preflight();
            return;
        }
    ...
    private function generation5Catalog(): CoreSchemaCatalog
    {
        return $this->catalog->schemaVersion() === 5 ? $this->catalog : ($this->generation5Catalog ??= CoreSchemaCatalog::loadHistorical(5));
    }
    ...
    private function upgradeSealedGeneration4ToGeneration5(): array
    {
        ...
        $target = $this->generation5Catalog();
        $driver = (string) $this->pdo->getAttribute(PDO::ATTR_DRIVER_NAME);
        if ($driver === 'mysql') {
            $upgrade = new CoreSchemaMySqlGeneration5Upgrade($this->pdo, $source, $target);
            return $upgrade->upgrade(
                function () use ($target): void {
                    $this->writeAttestationValuesFor($target);
                },
                function () use ($target): void {
                    $this->assertAttestationFor($target);
                }
            );
        }
        ...
        $targetVerifier = new CoreSchemaVerifier($target);
        $this->pdo->beginTransaction();
        try {
            if ($driver === 'pgsql') {
    ...
    /** MySQL CREATE TABLE is resumable only after a sealed predecessor and durable intent. */
    private function assertGeneration6SourceOrProgress(): void
    {
        $source = $this->generation5Catalog();
        $this->assertFingerprintFor($source);
        $this->assertRagVersion();
        $present = $this->presentTables(PluginApiSchema::TABLES);
        $intent = $this->setting(self::GENERATION6_INTENT);
        if ((string) $this->pdo->getAttribute(PDO::ATTR_DRIVER_NAME) !== 'mysql'
            && ($intent !== null || $present !== [])) {
            throw new CoreSchemaException(['reason' => 'canonical_generation6_unexpected_partial_state']);
        }
        if ($intent === null && $present === []) {
            (new CoreSchemaVerifier($source))->verify($this->pdo);
            return;
        }
        if ($intent !== $this->catalog->artifactHash()) {
            throw new CoreSchemaException(['reason' => 'canonical_generation6_intent_invalid']);
        }
        $report = (new CoreSchemaVerifier($source))->verifyTables($this->pdo, $source->tableNames((string) $this->pdo->getAttribute(PDO::ATTR_DRIVER_NAME)));
        if ($report['unclassified_relations'] !== []) {
            throw new CoreSchemaException(['reason' => 'canonical_generation6_unclassified_relation']);
        }
        $this->verifier->verifyOperationalTriggers($this->pdo);
        if ($present !== []) { $this->verifier->verifyTables($this->pdo, $present); }
    }

    private function upgradeSealedGeneration5ToGeneration6(): array
    {
        $this->assertGeneration6SourceOrProgress();
        if ($this->pdo->inTransaction()) { throw new CoreSchemaException(['reason' => 'canonical_generation_upgrade_inside_transaction']); }
        $driver = (string) $this->pdo->getAttribute(PDO::ATTR_DRIVER_NAME);
        if ($driver === 'mysql') {
            $this->pdo->beginTransaction();
            try {
                $this->writeSetting(self::GENERATION6_INTENT, $this->catalog->artifactHash());
                $this->pdo->commit();
        ...
        $this->pdo->beginTransaction();
        try {
            if ($driver === 'pgsql') { $this->lockPostgreSqlCoreTables($this->generation5Catalog()->tableNames('pgsql')); }
            $this->assertGeneration6SourceOrProgress();
            if ($driver !== 'mysql') { PluginApiSchema::migrate($this->pdo); }
            $this->verifier->verify($this->pdo);
            $this->writeAttestationValuesFor($this->catalog);
            $statement = $this->pdo->prepare('DELETE FROM system_settings WHERE setting_key = ?');
            $statement->execute([self::GENERATION6_INTENT]);
        ...
        return $this->verifier->verify($this->pdo) + ['migration' => [
            'from_schema_version' => 5, 'to_schema_version' => 6,
            'atomic' => $driver !== 'mysql', 'physical_atomic' => $driver !== 'mysql',
            'activation_atomic' => true, 'crash_resumable' => true,
            'added_tables' => PluginApiSchema::TABLES,
        ]];
    }
